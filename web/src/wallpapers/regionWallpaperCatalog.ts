export type WallpaperRegion = "demacia" | "bandle-city"
...
export function decodeRegionWallpaperCatalog(value: unknown): RegionWallpaperCatalog {
  ...
  const candidates = row.candidates.map((candidate, index) => {
    ...
    if (item.region !== "demacia" && item.region !== "bandle-city" || item.status !== "research-candidate" || typeof item.id !== "string") throw new Error(`candidate[${index}] identity is invalid`)
    ...
    return {
      id: item.id,
      region: item.region as WallpaperRegion,
...
export const regionWallpaperCatalog = decodeRegionWallpaperCatalog({
  ...
  candidates: [{
    ...
    region: "bandle-city",
    status: "research-candidate",
    label: { "zh-CN": "班德尔城", en: "Bandle City" },
    description: { "zh-CN": "官方动态壁纸候选 · 本地预览", en: "Official wallpaper candidate · local preview" },
    poster: "/assets/wallpapers/candidates/bandle-city-poster.webp",
    webm: "/assets/wallpapers/candidates/bandle-city.webm",
    mp4: "/assets/wallpapers/candidates/bandle-city.mp4",
    intrinsicWidth: 1920,
    intrinsicHeight: 1080,
    fps: 25,
    durationSeconds: 15.04,
    focalPoint: { x: 0.5, y: 0.5 },
    sourceDigest: "17da832a35c669df551663a2a363e43b825895ba1a1b7648a4d07694a0eddb6c",
    rights: "unverified",
  }],
})
