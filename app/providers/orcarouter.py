...
_INCOMPLETE_FINISH_REASONS = {"length", "content_filter"}


class OrcaRouterProvider:
    """Translate RiftCoach chat contracts to the OrcaRouter gateway.

    OrcaRouter is an OpenAI-compatible gateway that routes each request to a
    suitable model, so the response ``model`` is the routed model rather than
    the requested ``orcarouter/*`` namespace, and routed reasoning models may
    return ``reasoning_content``. Both are accepted here while all structural
    response checks still fail closed.
    """
    ...
    def __init__(self, *, client: Any, model: str) -> None:
        ...
        if model != ORCAROUTER_MODEL:
            raise ValueError(
                f"OrcaRouter adapter requires exact model {ORCAROUTER_MODEL!r}."
            )
        self._client = client
    ...
    def chat(self, request: ChatRequest) -> ChatResponse:
        ...
        try:
            ...
            # Reasoning content is expected: the gateway routes across models,
            # some of which expose chain-of-thought in this field.
            ...
            routed_model = getattr(raw_response, "model", None)
            if not isinstance(routed_model, str) or not routed_model.strip():
                raise ProviderResponseError(
                    provider=self.provider_name,
                    code="invalid_chat_response",
                )
...
class _ToolAliasMap:
    """One request-local reversible mapping for gateway-safe tool names."""
