    ...
    def build_offline_coach_runtime(self, *, runs_root, provider, knowledge_provider, context_builder=None):
        ...
        factory = RuntimeExecutionFactory(
            ...
            evaluator_factory=lambda runtime: evaluator_type(
                ...
                **({"inference_audit": "scope_v3" if contract is SCOPE_V3_COACH_CONTRACT else "scope_v2" if contract is SCOPE_V2_COACH_CONTRACT else "scope" if contract is SCOPE_COACH_CONTRACT else "coverage" if contract is COVERAGE_COACH_CONTRACT else "v3" if contract is ANCHOR_COACH_CONTRACT else "v2" if contract is CLAIM_COACH_CONTRACT else True} if contract in (INFERENCE_COACH_CONTRACT, CLAIM_COACH_CONTRACT, ANCHOR_COACH_CONTRACT, COVERAGE_COACH_CONTRACT, SCOPE_COACH_CONTRACT, SCOPE_V2_COACH_CONTRACT, SCOPE_V3_COACH_CONTRACT) else {}),
