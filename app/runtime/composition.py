    ...
    def build_offline_coach_runtime(self, *, runs_root, provider, knowledge_provider, context_builder=None):
        ...
        factory = RuntimeExecutionFactory(
            ...
            evaluator_factory=lambda runtime: evaluator_type(
                ...
                **({"inference_audit": "evidence_v6" if contract is EVIDENCE_V6_COACH_CONTRACT else "evidence_v5" if contract is EVIDENCE_V5_COACH_CONTRACT else "evidence_v4" if contract is EVIDENCE_V4_COACH_CONTRACT else "evidence_v3" if contract is EVIDENCE_V3_COACH_CONTRACT else "capacity_v1" if contract is CAPACITY_COACH_CONTRACT else "evidence_v2" if contract is EVIDENCE_V2_COACH_CONTRACT else "evidence_v1" if contract is EVIDENCE_COACH_CONTRACT else "fact_v1" if contract is FACT_INFERENCE_COACH_CONTRACT else "scope_v5" if contract is FEEDBACK_COACH_CONTRACT else "scope_v4" if contract in (SCOPE_V4_COACH_CONTRACT, EXPANDED_COACH_CONTRACT) else "scope_v3" if contract is SCOPE_V3_COACH_CONTRACT else "scope_v2" if contract is SCOPE_V2_COACH_CONTRACT else "scope" if contract is SCOPE_COACH_CONTRACT else "coverage" if contract is COVERAGE_COACH_CONTRACT else "v3" if contract is ANCHOR_COACH_CONTRACT else "v2" if contract is CLAIM_COACH_CONTRACT else True} if contract in (EVIDENCE_V6_COACH_CONTRACT, EVIDENCE_V5_COACH_CONTRACT, EVIDENCE_V4_COACH_CONTRACT, EVIDENCE_V3_COACH_CONTRACT, CAPACITY_COACH_CONTRACT, EVIDENCE_V2_COACH_CONTRACT, EVIDENCE_COACH_CONTRACT, FACT_INFERENCE_COACH_CONTRACT, FEEDBACK_COACH_CONTRACT, EXPANDED_COACH_CONTRACT, INFERENCE_COACH_CONTRACT, CLAIM_COACH_CONTRACT, ANCHOR_COACH_CONTRACT, COVERAGE_COACH_CONTRACT, SCOPE_COACH_CONTRACT, SCOPE_V2_COACH_CONTRACT, SCOPE_V3_COACH_CONTRACT, SCOPE_V4_COACH_CONTRACT) else {}),
