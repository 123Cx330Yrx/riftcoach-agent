...
def build_grounded_evaluation_prompt(facts, report, *, user_utterance, knowledge, compact_json=False):
    prompt = build_secure_evaluation_prompt(facts, report, user_utterance=user_utterance, knowledge=knowledge, compact_json=compact_json)
...
class GroundedChatEvaluationAdapter(SecureChatEvaluationAdapter):
    ...
    def evaluate(self, request):
        if self.inference_audit == "context_v1":
            from app.evaluation.golden_context_runtime import evaluate
        ...
        if self.inference_audit == "evidence_v8":
            from app.evaluation.golden_evidence_runtime_v8 import evaluate
            return evaluate(self, request)
        if self.inference_audit == "evidence_v7":
            from app.evaluation.golden_evidence_runtime_v7 import evaluate
            return evaluate(self, request)
        if self.inference_audit == "evidence_v6":
            from app.evaluation.golden_evidence_runtime_v6 import evaluate
            return evaluate(self, request)
        if self.inference_audit == "evidence_v5":
            from app.evaluation.golden_evidence_runtime_v5 import evaluate
            return evaluate(self, request)
        if self.inference_audit == "evidence_v4":
            from app.evaluation.golden_evidence_runtime_v4 import evaluate
            return evaluate(self, request)
        if self.inference_audit == "evidence_v3":
            from app.evaluation.golden_evidence_runtime_v3 import evaluate
            return evaluate(self, request)
        if self.inference_audit == "capacity_v1":
            from app.evaluation.golden_capacity_runtime import evaluate
            return evaluate(self, request)
        if self.inference_audit == "evidence_v2":
            from app.evaluation.golden_evidence_runtime_v2 import evaluate
            return evaluate(self, request)
        if self.inference_audit == "evidence_v1":
            from app.evaluation.golden_evidence_runtime import evaluate
            return evaluate(self, request)
        ...
        if self.inference_audit:
            ...
            if self.inference_audit == "scope":
                from app.evaluation.golden_inference_scope import audit_prompt, inference_response_contract, EvaluationResponseModelV16
                EvaluationResponseModelV13 = EvaluationResponseModelV16
            if self.inference_audit == "scope_v2":
                from app.evaluation.golden_inference_scope_v2 import audit_prompt, inference_response_contract, EvaluationResponseModelV17
                EvaluationResponseModelV13 = EvaluationResponseModelV17
            if self.inference_audit == "scope_v3":
                from app.evaluation.golden_inference_scope_v3 import audit_prompt, inference_response_contract, EvaluationResponseModelV17
                EvaluationResponseModelV13 = EvaluationResponseModelV17
        ...
        def call(text, step):
            ...
            latest_response = _chat_response(self.runtime, system_prompt=self.system_prompt, user_prompt=text,
        ...
        if self.inference_audit == "scope":
            ...
            contextual["validate_context"] = lambda p: validate_scope(p, request.report, facts["inference_facts"])
        if self.inference_audit == "scope_v2":
            from app.evaluation.golden_inference_scope_v2 import validate_scope_v2
...
class GroundedCoachReviser(ChatCoachReviser):
    ...
    def revise(self, request):
        if self.inference_audit == "context_v1":
            from app.evaluation.golden_context_runtime import revise
            return revise(self, request)
        if self.inference_audit == "evidence_v8":
            from app.evaluation.golden_evidence_runtime_v8 import revise
            return revise(self, request)
        if self.inference_audit == "evidence_v7":
            from app.evaluation.golden_evidence_runtime_v7 import revise
            return revise(self, request)
        if self.inference_audit == "evidence_v6":
            from app.evaluation.golden_evidence_runtime_v6 import revise
            return revise(self, request)
        if self.inference_audit == "evidence_v5":
            from app.evaluation.golden_evidence_runtime_v5 import revise
            return revise(self, request)
        if self.inference_audit == "evidence_v4":
            from app.evaluation.golden_evidence_runtime_v4 import revise
            return revise(self, request)
        if self.inference_audit == "evidence_v3":
            from app.evaluation.golden_evidence_runtime_v3 import revise
            return revise(self, request)
        if self.inference_audit == "capacity_v1":
            from app.evaluation.golden_capacity_runtime import revise
            return revise(self, request)
        if self.inference_audit == "evidence_v2":
            from app.evaluation.golden_evidence_runtime_v2 import revise
            return revise(self, request)
        if self.inference_audit == "evidence_v1":
            from app.evaluation.golden_evidence_runtime import revise
            return revise(self, request)
        if self.inference_audit == "fact_v1":
