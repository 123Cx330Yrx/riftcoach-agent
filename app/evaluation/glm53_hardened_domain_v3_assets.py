"""No-I/O admission for the fresh GLM-5.3 hardened V3 assets."""
...
PROTOCOL_PATH = Path(
    "data/evaluation/glm53_flash_hardened_domain_protocol_v3.json"
)
DATASET_PATH = Path("data/evaluation/glm53_flash_hardened_domain_heldout_v3.json")
...
PROTOCOL_ID = "glm53-flash-hardened-domain-observation-v3"
PROTOCOL_VERSION = "3.0.0"
DATASET_ID = "glm53-flash-hardened-domain-heldout-v3"
DATASET_VERSION = "3.0.0"
SNAPSHOT_ID = "glm53-flash-hardened-context-v3"
...
CASE_IDS = (
    "revision_form_control_67",
    "revision_user_data_boundary_71",
    "revision_knowledge_data_boundary_79",
)
...
class HardenedDomainV3Protocol(_FrozenModel):
...
class GLM53HardenedDomainV3AssetAdmission(_FrozenModel):
    """Public-safe identities for V3; no question or marker text is exposed."""

    schema_version: Literal["1.0"] = "1.0"
    admission_id: NonBlankText = "glm53-hardened-domain-v3-assets-admission"
    protocol_id: Literal[PROTOCOL_ID] = PROTOCOL_ID
    protocol_version: Literal[PROTOCOL_VERSION] = PROTOCOL_VERSION
    ...
    def validate_identity(self) -> "GLM53HardenedDomainV3AssetAdmission":
        if self.case_ids != CASE_IDS:
            raise ValueError("hardened V3 case order is not canonical")
        if len(self.artifact_sha256) != 7:
            raise ValueError("hardened V3 admission must bind seven artifacts")
        if len(set(self.artifact_sha256)) != len(self.artifact_sha256):
            raise ValueError("hardened V3 artifact digests must be unique")
        if len(set(self.forbidden_marker_sha256)) != len(
            self.forbidden_marker_sha256
        ):
            raise ValueError("hardened V3 marker digests must be unique")
        return self
...
def admit_hardened_domain_v3_assets(
    *,
    ...
    protocol = HardenedDomainV3Protocol.model_validate_json(
    ...
    if (
        ...
        raise ValueError("V3 protocol does not match the budget proof")
    ...
    if (dataset.dataset_id, dataset.dataset_version) != (
        ...
        raise ValueError("unexpected hardened V3 Dataset identity")
    if tuple(row.case_id for row in dataset.cases) != CASE_IDS:
        raise ValueError("hardened V3 Dataset case IDs drifted")
    if any(
        ...
        raise ValueError("V3 Dataset quality or resource walls drifted")
    ...
    if (snapshot.snapshot_id, snapshot.snapshot_sha256) != (
        ...
        raise ValueError("V3 Dataset and Context snapshot identities differ")
    ...
    if (
        ...
        raise ValueError("V3 input plan revision or Context contract drifted")
    ...
    if expected_commitments != actual_commitments:
        raise ValueError("V3 input plan Context commitments differ from snapshot")
    ...
    if rebuilt != snapshot:
        raise ValueError("frozen V3 Context snapshot cannot be rebuilt")
    if (
        ...
        raise ValueError("V3 budget proof does not bind the frozen assets")
    ...
    if (
        ...
        raise ValueError("V3 assets reuse consumed RQ-227 or V2 identity")
    if len(set(markers)) != len(markers):
        raise ValueError("V3 forbidden markers must be unique")
    ...
    return GLM53HardenedDomainV3AssetAdmission(
