"""Materialized offline requests. These builders are not registered for live I/O."""
import json
...
from app.evaluation.golden_fact_operations import OperationWire
...
POLICY = """Offline fact/inference candidate v2:
...
每个阿拉伯数字都用numeric_bindings绑定。binding只含op、operands、token；operands为[证据键,字段路径]数组。
value取一个原值；difference为第一个减第二个；percent只对单局kill_participation/damage_share/gold_share乘100；
ratio_percent为第一个除第二个再乘100，不代表百分比变化。二元操作必须同指标；除零不合法。
token逐字取quote数字，按其小数位四舍五入ROUND_HALF_UP，最多6位。原数值错误时列unsupported及issue，禁止改quote骗过校验。
混合数字与稳定/能力结论的句子必须按inference审查，不能只引用正确数字部分；逐段检查没有列进claims的其余断言。
inference的scope为selected_sample/beyond_sample/ambiguous/question_or_negation，scope_anchor取原句1到20字符。
...
def response_contract():
    # Explicitly unissued candidate identity: this is not a new live version.
    return contract_for_model(name="offline_fact_candidate", version="0.0.2", output_model=OperationWire)
