...
from app.evaluation.golden_evidence_requests_v8 import evaluation_request, revision_request, POLICY
from app.evaluation.golden_evidence_scope_v8 import EvidenceWire, EvidenceEvaluation, expand_evidence, normalize_json
...
INFERENCE_POLICY_ID = "golden-evidence-scope-v8"
...
def inference_component_fingerprints(skill):
    ...
    for name in ("golden_fact_candidate", "golden_numeric_evidence_v2", "golden_numeric_evidence_v3", "golden_numeric_evidence_v4", "golden_evidence_scope_v3", "golden_evidence_scope_v5", "golden_evidence_requests_v8", "golden_scope_diagnostics", "golden_evidence_diagnostics_v6", "golden_evidence_diagnostics_v7", "golden_evidence_diagnostics_v8", "golden_evidence_scope_v8", "golden_evidence_runtime_v8"):
        values[name] = Path(__file__).with_name(name + ".py").read_text(encoding="utf-8")
    for key, value in values.items():
        rows.append(ComponentFingerprint(component_id=key, source="app.evaluation.golden_evidence_runtime_v8:" + key,
