...
from app.evaluation.golden_evidence_requests import evaluation_request, revision_request, POLICY
from app.evaluation.golden_evidence_scope import EvidenceWire, EvidenceEvaluation, expand_evidence
from app.evaluation.golden_fact_candidate import fact_pack
from app.evaluation.golden_evidence_scope import collect_diagnostics
...
INFERENCE_POLICY_ID = "golden-evidence-scope-v1"
INFERENCE_POLICY = POLICY


def inference_response_contract():
    return contract_for_model(name="coach_evaluation", version="1.12.0", output_model=EvidenceWire)
...
def inference_component_fingerprints(skill):
    ...
    for name in ("golden_fact_candidate", "golden_numeric_evidence", "golden_evidence_scope", "golden_evidence_requests", "golden_scope_diagnostics", "golden_evidence_runtime"):
        values[name] = Path(__file__).with_name(name + ".py").read_text(encoding="utf-8")
    for key, value in values.items():
        rows.append(ComponentFingerprint(component_id=key, source="app.evaluation.golden_evidence_runtime:" + key,
