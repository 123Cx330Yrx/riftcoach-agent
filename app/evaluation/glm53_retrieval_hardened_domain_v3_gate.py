...
DEFAULT_RETRIEVAL_OUTPUT = Path(
    "data/evaluation/results/provider_capabilities/"
    "zhipu_glm53_flash_retrieval_hardened_domain_v3_rq237_v1.json"
)
DEFAULT_RETRIEVAL_RUNS_ROOT = Path("data/runs/evaluation/glm53_retrieval_hardened_domain_v3")
...
class RetrievalV3CaseResource(_FrozenModel):
    ...
    def validate_total(self) -> "RetrievalV3CaseResource":
        if self.total_tokens != self.input_tokens + self.output_tokens:
            raise ValueError("V3 case token total is inconsistent")
        return self


class RetrievalV3ResourceSnapshot(_FrozenModel):
    ...
    @model_validator(mode="after")
    def validate_totals(self) -> "RetrievalV3ResourceSnapshot":
        if self.total_tokens != self.input_tokens + self.output_tokens:
...
class RetrievalV3ControlSnapshot(_FrozenModel):
    global_stop: SafeCodeText | None = None
    provider_stop: SafeCodeText | None = None
    provider_error_code: SafeCodeText | None = None


class RetrievalV3DomainAdmission(_FrozenModel):
    ...
    asset_admission: GLM53RetrievalHardenedDomainV3AssetAdmission
    ...
    @model_validator(mode="after")
    def validate_identity(self) -> "RetrievalV3DomainAdmission":
...
class RetrievalV3DomainGateResult(_FrozenModel):
    schema_version: Literal[SCHEMA_VERSION] = SCHEMA_VERSION
    protocol_id: Literal[PROTOCOL_ID] = PROTOCOL_ID
    experiment_id: Sha256Text
    run_timestamp_utc: datetime
    admission: RetrievalV3DomainAdmission
    resources: RetrievalV3ResourceSnapshot
    control: RetrievalV3ControlSnapshot
    protocol_calls: Literal[PROTOCOL_MAX_CALLS] = PROTOCOL_MAX_CALLS
    protocol_total_tokens: int = Field(ge=0)
    ...
    @model_validator(mode="after")
    def validate_result(self) -> "RetrievalV3DomainGateResult":
...
class RetrievalV3PreparedRun:
    admission: RetrievalV3DomainAdmission
    assets: GLM53RetrievalHardenedDomainV3AssetAdmission
    dataset: DomainEvaluationDataset
    input_plan: LoadedDomainCaseInputPlan


@dataclass(frozen=True)
class RetrievalV3PreflightStatus:
    status: Literal["pending_public_ci", "pending_protocol_evidence"]
    implementation_sha: str
    public_ci_sha: str
    asset_admission: GLM53RetrievalHardenedDomainV3AssetAdmission | None
...
@dataclass(frozen=True)
class RetrievalV3DomainGateOptions:
    ...
    protocol_result: Path = DEFAULT_RETRIEVAL_PROTOCOL_RESULT
    output: Path = DEFAULT_RETRIEVAL_OUTPUT
    runs_root: Path = DEFAULT_RETRIEVAL_RUNS_ROOT


def build_retrieval_v3_domain_preflight(
    *,
    ...
    protocol_result_path: str | Path = DEFAULT_RETRIEVAL_PROTOCOL_RESULT,
) -> RetrievalV3PreparedRun:
    ...
    assets = admit_retrieval_hardened_domain_v3_assets(
        project_root=root,
    ...
    draft = RetrievalV3DomainAdmission.model_construct(**admission_data)
    admission_data["experiment_id"] = _admission_identity(draft)
    return RetrievalV3PreparedRun(
        admission=RetrievalV3DomainAdmission(**admission_data),
        assets=assets,
        dataset=dataset,
...
def run_retrieval_v3_domain_gate(
    *,
    admission: RetrievalV3DomainAdmission,
    dataset: DomainEvaluationDataset,
...
) -> RetrievalV3DomainGateResult:
    if not isinstance(admission, RetrievalV3DomainAdmission):
        raise TypeError("admission must be a RetrievalV3DomainAdmission")
    ...
    return RetrievalV3DomainGateResult(
...
def run_retrieval_cli(
    options: RetrievalV3DomainGateOptions,
    *,
...
) -> RetrievalV3PreflightStatus | RetrievalV3PreparedRun | RetrievalV3DomainGateResult:
    ...
    if current_sha != public_sha or options.confirm_public_ci_success is not True:
        return RetrievalV3PreflightStatus(
    ...
    assets = admit_retrieval_hardened_domain_v3_assets(project_root=root, confirm_rules_frozen=True, protocol_path=options.protocol_plan, dataset_path=options.dataset, input_plan_path=options.input_plan, snapshot_path=options.snapshot, budget_report_path=options.budget_report)
    try:
        prepared = build_retrieval_v3_domain_preflight(project_root=root, implementation_sha=current_sha, public_ci_sha=public_sha, confirm_public_ci_success=True, protocol_plan_path=options.protocol_plan, dataset_path=options.dataset, input_plan_path=options.input_plan, snapshot_path=options.snapshot, budget_report_path=options.budget_report, protocol_result_path=options.protocol_result)
    except (FileNotFoundError, ValueError):
        return RetrievalV3PreflightStatus(status="pending_protocol_evidence", implementation_sha=current_sha, public_ci_sha=public_sha, asset_admission=assets, reason="fresh exact-SHA G53-3-L protocol evidence is required")
    ...
    with tempfile.TemporaryDirectory(prefix="glm53-hardened-domain-v3-", dir=str(runs_root.parent)) as temporary:
        ...
        result = run_retrieval_v3_domain_gate(admission=prepared.admission, dataset=prepared.dataset, provider=provider, case_executor=executor, confirm_real_call=True)
    output.write_bytes(canonical_retrieval_v3_result_bytes(result))
    return result


def canonical_retrieval_v3_result_bytes(result: RetrievalV3DomainGateResult) -> bytes:
    if not isinstance(result, RetrievalV3DomainGateResult):
        raise TypeError("result must be a RetrievalV3DomainGateResult")
...
def _resource_snapshot(state: BoundedRevisionBudgetState) -> RetrievalV3ResourceSnapshot:
    snap = state.snapshot()
    rows = tuple(RetrievalV3CaseResource(case_id=case_id, calls_used=value["calls_used"], input_tokens=value["input_tokens"], output_tokens=value["output_tokens"], total_tokens=value["total_tokens"], latency_ms=value["latency_ms"]) for case_id, value in snap["cases"].items())
    return RetrievalV3ResourceSnapshot(calls_used=snap["calls_used"], input_tokens=snap["input_tokens"], output_tokens=snap["output_tokens"], total_tokens=snap["total_tokens"], latency_ms=snap["latency_ms"], case_resources=rows, stop_code=_safe_code(snap["stop_code"]) if snap["stop_code"] else None, provider_error_code=_safe_code(snap["provider_error_code"]) if snap["provider_error_code"] else None)
...
def _admission_identity(admission: RetrievalV3DomainAdmission) -> str:
...
def _parse_args(argv: Sequence[str] | None = None) -> RetrievalV3DomainGateOptions:
    ...
    parser.add_argument("--protocol-result", type=Path, default=DEFAULT_RETRIEVAL_PROTOCOL_RESULT)
    parser.add_argument("--output", type=Path, default=DEFAULT_RETRIEVAL_OUTPUT)
    parser.add_argument("--runs-root", type=Path, default=DEFAULT_RETRIEVAL_RUNS_ROOT)
    values = parser.parse_args(argv)
    return RetrievalV3DomainGateOptions(confirm_real_call=values.confirm_real_call, public_ci_sha=values.public_ci_sha, confirm_public_ci_success=values.confirm_public_ci_success, implementation_sha=values.implementation_sha, preflight_only=values.preflight_only, max_calls=values.max_calls, protocol_plan=values.protocol_plan, dataset=values.dataset, input_plan=values.input_plan, snapshot=values.snapshot, budget_report=values.budget_report, protocol_result=values.protocol_result, output=values.output, runs_root=values.runs_root)


def main(argv: Sequence[str] | None = None) -> int:
    result = run_retrieval_cli(_parse_args(argv))
    if isinstance(result, RetrievalV3PreflightStatus):
        print(f"status={result.status} external_provider_calls=0 held_out_executed=false")
    elif isinstance(result, RetrievalV3PreparedRun):
...
__all__ = [
    ...
    "DEFAULT_RETRIEVAL_PROTOCOL_RESULT",
    "RetrievalV3DomainAdmission",
    "RetrievalV3DomainGateOptions",
    "RetrievalV3DomainGateResult",
    "RetrievalV3PreparedRun",
    "RetrievalV3PreflightStatus",
    "build_retrieval_v3_domain_preflight",
    "canonical_retrieval_v3_result_bytes",
    "main",
    "run_retrieval_cli",
