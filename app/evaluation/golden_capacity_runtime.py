...
from app.evaluation.golden_evidence_requests_v2 import evaluation_request as base_evaluation_request, revision_request as base_revision_request, POLICY


def evaluation_request(*args, **kwargs):
    return replace(base_evaluation_request(*args, **kwargs), max_tokens=32768, timeout_s=300)


def revision_request(*args, **kwargs):
    return replace(base_revision_request(*args, **kwargs), max_tokens=32768, timeout_s=300)

from app.evaluation.golden_evidence_scope_v2 import EvidenceWire, EvidenceEvaluation, expand_evidence
from app.evaluation.golden_fact_candidate import fact_pack
from app.evaluation.golden_evidence_scope_v2 import collect_diagnostics
...
INFERENCE_POLICY_ID = "golden-evidence-scope-v2"
...
def inference_component_fingerprints(skill):
    ...
    for name in ("golden_fact_candidate", "golden_numeric_evidence_v2", "golden_evidence_scope_v2", "golden_evidence_requests_v2", "golden_scope_diagnostics", "golden_evidence_runtime_v2", "golden_capacity_runtime"):
        values[name] = Path(__file__).with_name(name + ".py").read_text(encoding="utf-8")
    for key, value in values.items():
        rows.append(ComponentFingerprint(component_id=key, source="app.evaluation.golden_evidence_runtime_v2:" + key,
