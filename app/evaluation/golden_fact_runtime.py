...
from app.evaluation.golden_fact_requests import evaluation_request, revision_request, POLICY
from app.evaluation.golden_fact_operations import OperationWire, OperationEvaluation, expand_operations
...
INFERENCE_POLICY_ID = "golden-fact-inference-v1"
INFERENCE_POLICY = POLICY.replace("Offline fact/inference candidate v2:", "Fact/inference evaluation v1:")


def inference_response_contract():
    return contract_for_model(name="coach_evaluation", version="1.11.0", output_model=OperationWire)
...
def evaluate(adapter, request):
    ...
    try:
        ...
        decoded = decode_structured_response(response=response, contract=inference_response_contract(),
            output_model=OperationWire, repair=repair,
            validate_context=lambda _: expand_operations(latest.content, request.report, pack))
    except _SecurityStop as stop:
        return adapter._result(stop.payload, verdict=EvaluationVerdict.FAIL)
    payload = expand_operations(decoded.response.content, request.report, pack)
...
def revise(adapter, request):
    ...
    canonical = OperationEvaluation.model_validate(_evaluation_payload(request.evaluation))
...
def inference_component_fingerprints(skill):
    ...
    for name in ("golden_fact_candidate", "golden_fact_operations", "golden_fact_requests", "golden_fact_diagnostics", "golden_scope_diagnostics", "golden_fact_runtime"):
        values[name] = Path(__file__).with_name(name + ".py").read_text(encoding="utf-8")
    for key, value in values.items():
        rows.append(ComponentFingerprint(component_id=key, source="app.evaluation.golden_fact_runtime:" + key,
