...
from app.runtime.coach_contract import CoachContractSnapshot, LATENCY_COACH_CONTRACT
...
def run_golden_slice(
    ...
    *,
    ...
    if config.with_provider:
        real_evidence = verify_real_evidence(LATENCY_COACH_CONTRACT.snapshot(),
    ...
    journal = GoldenCallJournal(ROOT / "data/runs/golden_slice_reservations" / config.run_id,
        identity={"run_id": config.run_id, "implementation_sha": sha,
                  ...
                  "coach_contract": LATENCY_COACH_CONTRACT.snapshot().model_dump(mode="json"),
...
def _run_reserved_golden_slice(config, *, gate, journal, implementation_sha, environ, opgg_fetcher):
    ...
    if config.with_provider:
        ...
        provider = ZhipuProvider.from_candidate_profile(
            client=OpenAI(
                ...
                timeout=LATENCY_COACH_CONTRACT.descriptor()["request_timeout_s"],
                max_retries=0,
            ),
        ...
        application = build_coach_application(
            ...
            coach_contract=LATENCY_COACH_CONTRACT,
    ...
    return GoldenSliceReceipt(
        ...
        coach_contract=LATENCY_COACH_CONTRACT.snapshot() if config.with_provider else None,
        source_counts={
