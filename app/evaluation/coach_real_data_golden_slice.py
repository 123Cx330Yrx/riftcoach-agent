...
from app.runtime.coach_contract import CoachContractSnapshot, COMPACT_COACH_CONTRACT
...
def run_golden_slice(
    ...
    *,
    ...
    if config.with_provider:
        real_evidence = verify_real_evidence(COMPACT_COACH_CONTRACT.snapshot(),
    ...
    journal = GoldenCallJournal(ROOT / "data/runs/golden_slice_reservations" / config.run_id,
        identity={"run_id": config.run_id, "implementation_sha": sha,
                  ...
                  "coach_contract": COMPACT_COACH_CONTRACT.snapshot().model_dump(mode="json"),
...
def _run_reserved_golden_slice(config, *, gate, journal, implementation_sha, environ, opgg_fetcher):
        ...
        try:
            provider = None if config.provider_transport else ZhipuProvider.from_candidate_profile(
                client=OpenAI(
                    ...
                    timeout=COMPACT_COACH_CONTRACT.descriptor()["request_timeout_s"],
                    max_retries=0,
            ...
            application = build_coach_application(
                ...
                coach_contract=COMPACT_COACH_CONTRACT,
    ...
    return GoldenSliceReceipt(
        ...
        coach_contract=COMPACT_COACH_CONTRACT.snapshot() if config.with_provider else None,
        provider_transport=config.provider_transport,
        source_counts={
