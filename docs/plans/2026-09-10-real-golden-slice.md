...
提交前验证：bridge新增三项确定时钟/中途异常/调用前隐私计数测试，28项通过；相邻比例回归134项通过，最后关闭失败计时修正后桥接28项复验通过。Luna独立审查后澄清事件为本地接受时间、阶段包含其中落盘开销；语法、治理和diff检查通过，无新增真实调用。


### 7422396实测定位与简练报告修正（2026-09-10）

7422396a18b824c6a1bab9124c62c6d49269053e / Actions34473254289三job成功（3018 passed/152 skipped/127 subtests、PostgreSQL208）。新鲜协议3请求1008/108通过；黄金golden_20260910_stream_7422396三请求，前两次完整工具回合输入15926/输出454，第三请求90秒父截止无报告。第三请求首reasoning6.125s、首正文40.531s，最后持久前缀89.437s仍有正文，3584事件、正文3316字符/reasoning4656字符，max相邻接受事件间隔328ms，advance83.046s、本地processing250ms，progress写入189ms（与阶段重叠）。该前缀不显示持续长停顿，支持生成过程未能在预算内完成，不能还原旧失败或判断供应商内部原因。终态/Usage/EOF缺失，close未知；最后563ms可能未落盘，不补造关闭或输出用量。新增6请求，累计60次，五次超时请求用量未知；本批已知输入16934/输出562。Bundle8dabebee60d545e95cc242bdc3195212bf95dff8dad74f2897d8ac6bf8e19fa1。

继续修正：新增独立COMPACT_COACH_CONTRACT1.3.6、Skill0.5.6/Program2.3.6，继承1.3.5全部确定性/逐局/位置/来源输入。仅给生成与修订增加1600–2200字符软目标、每项事实只展开一次、重点风险和可验证行动、精简对照表；七节结构、适用知识引用、OP.GG事实到具体条件建议、版本/段位/区域边界和未知训练目标不得删减。必要事实优先于长度；不硬截断、不为超软长度独立触发修订，保留修订70%最低比例及85分/一次修订。评测同步知晓软目标但不能降低原证据与准确性标准。high8192/90/120/480及输入/调用/工具预算不变，旧1.3.5与默认1.2不修改。下一步离线接线/旧身份/9回合质量拒绝测试、公共同SHA与新鲜协议后一次新版完整报告观察。效果尚未验证，8E/Training/live Workbench不前移。


独立人工回执绑定Trace456e200e873f84037eefccafa5f1e31f8546ac97823f38517d4c1b112d89f706，Trace确认4次knowledge.search成功，Runtime118421ms。实际来源仍Riot0/static1/official_patch1/OP.GG2工具+2初始化+2目录；receipt中5为比赛条目不是Riot调用数。新1.3.6 SHA d20fc775e7be126d373163a7d72cf71be329e28614aeb1eacaaf5e560b1f59f3；生成/评估/修订全贯穿简练策略，默认1.2及旧1.3.5摘要冻结。保存五局+本次Bundle的合成完整回放最大input ceiling54918，初始15536/最终27344units，9请求均含同一来源digest和位置策略，8工具/一次修订仍低分拒绝。事实投影输入未删，长度仅软目标，效果待新实测。


1.3.6提交前验证：131项比例回归通过（包括新增3项策略贯穿/旧身份冻结/预算检查和新增事实逐字段等价参数），主Agent修正测试对JSON转义策略的检查口径；先前批量编辑造成的局部语法错误已在执行前修复。保存实数+来源Bundle回放通过；语法、治理、diff检查通过。无额外真实请求。
...
运行：在仓库执行python -m scripts.export_showmaker_observation --manual-review <人工副本绝对路径> --output <新归档路径>，在web启动现有开发服务器，打开/?view=observation后选择导出的JSON；不需要外服账号。截图位于用户outputs/showmaker-workbench-desktop.png及showmaker-workbench-mobile.png。练习刷新清空，归档来源时间不刷新；该页面不承担个人训练持久化或authenticated API/DB消费，无法据此宣布8E完成。面试表述可说“已实现有来源校验的ShowMaker观摩归档消费与位置隔离”，不可说“生产闭环全部完成”。
