...
## 7. v4 实际结果与 prompt 复盘（2026-08-27）

v4 已实际调用一次并下载成功，但用户审查拒绝。九宫格显示：0–3 秒基本保持静态，4–9 秒主要是中央平台
变成一个大面积发光圆顶，随后又收回；左 Rift、右星图/地形、道路和 near/mid/far 环境没有达到目标的持续
全幕运动。数值也支持这一判断：center 每 0.5 秒 MAD `0.014625`，明显高于 left `0.005851` 与 right
`0.004653`；first→last SSIM `0.994464` 只是说明它回到了相近外观，不能证明中间的运动自然或全局。

这次 prompt 不是随机写的，但确实存在设计错误：

1. 同时要求“保持平台几何不变”和“platform response / central breathing swell”，正向语义给了模型把平台
   变成显著圆顶的捷径，negative 没能抵消它。
2. `fluid depth / breathe / continuously drift` 是抽象结果词，没有说明每个区域“哪些已有材质、沿什么表面、
   以什么可观察轨迹运动”，模型于是选择最容易制造差异的中心体积变化。
3. 单一静态首帧没有区域 mask 或中间关键帧，无法约束左/右/环境与中心同级；12 秒只给了更多时间让中心事件
   主导，而没有增加可控的空间证据。
4. 我把全幕目标压进一条较长 prompt，又叠加了很多禁止项，结果在“源图稳定”和“明显运动”之间偏向了局部
   保守变化；这不是靠再加形容词就能修好。

因此 v4 不进入 runtime，也不立即重抽 v5。下一次必须先完成方法层裁决：是采用带区域/时间控制的真实视频编辑，
还是将全局环境拆成可控的确定性材质层并只让生成模型负责有机局部；若仍保留首帧生成，也要先证明每个运动
载体的可观察行为和区域控制，而不是继续堆 abstract adjectives。
