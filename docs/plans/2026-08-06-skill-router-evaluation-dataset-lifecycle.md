...
## 9. development 与 holdout 结果

双 Skill development v2 已按门禁正式运行：23 条全部精确匹配，三类准确率均为
`1.0`，错误选择率为 `0.0`，没有逐条 mismatch。该结果只用于接受并冻结开发规则，
不作为自然语言泛化证明。

independent holdout v1 随后只运行一次：12 条中 11 条精确匹配，唯一失败把“分析
一下我最近键盘的表现”误选为 `recent-form-review`。该句满足当前字面合同，但目标
实体属于设备而不是 LoL 对局，因此分类为确定性 Router 的域语义局限。

失败已原样保存，没有用于添加“键盘”排除词或修改规则。若未来根据该失败调节规则，
必须退休 holdout v1 并创建新的未污染版本。5C-5 至此完成；下一检查点 5C-6 将比较
确定性域信号、排除词、澄清机制和模型兜底，但不默认采用模型。
