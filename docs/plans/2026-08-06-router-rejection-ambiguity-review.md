...
> **后续演进说明（2026-08-07）**：本文记录 5C-4 当时的阶段快照，当时 Catalog
> 只有一个真实 Skill，因此第 3.3 节和第 6 节描述的是旧单 Skill 边界。5C-5 前加入
> `single-match-review` 后，近期与单局信号不再互相排除：只命中一个范围时选择对应
> Skill，同时命中两个范围时返回 `ambiguous`。当前规则、真实候选数量和最终 5C
> 结论以 `2026-08-07-skill-router-v1-exit-review.md` 为准；保留本文旧描述是为了呈现
> 设计如何演进，不应把它误读为当前 Manifest。
