...
本轮离线完成第二个问题的可执行考场；真实 GLM 尚未进入这个考场，第三个问题属于
后续 `5D-7`。
...
离线控制器必须先提交到 GitHub，并让公开 CI 对精确代码 SHA 通过。之后才能按 RQ-027
运行一次累计上限为 7、领域剩余上限为 4 的真实 GLM recent-form 切片。真实结果无论
成功还是失败都原样保存，不临场改 Prompt 追分。

只有这次受控真实运行完成并复核后，才能决定 `5D-6b` 是否收尾。随后 `5D-7` 才会用
多案例数据集系统评测 Prompt、Context、工具选择、事实/引用、注入、质量、延迟与成本。
