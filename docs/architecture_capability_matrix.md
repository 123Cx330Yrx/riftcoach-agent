...
| A09 | Prompt/Context Engineering | Harness Prompt V0、SKILL.md 指令；5D-2 已实现 trust-typed Context Builder，5D-3 已实现逐轮 Context 门禁；5D-7 已冻结双层语义身份、Evaluation 1.1、安全 blocking policy、隔离 held-out 与独立输入计划，并以当前无领域 Provider 准入完成评测门审查 | 阶段 5D-5E | 保留真实模型完整链路、真实注入和性能限制；5D-exit-review 先审查整体，5E 再加统一 Trace | Prompt 版本、上下文优先级、Token 预算、漂移拒绝、用户/RAG 注入、回归和消融测试 | 部分完成 |
...
| A11 | AgentRuntime V1 | 5D-1/2 已建立执行与 Context 边界，5D-3 已编译 Manifest 权限/预算并加入有界停止，5D-4 已产生可审计 draft/evidence，5D-5 已通过唯一 ReviewHarness 组合为 typed terminal output；5D-6/7 已补结构化输出、最小真实协议和领域评测门 | 阶段 5D-5E | 先做 5D-exit-review；5E 统一 run/stream/event/trace/usage；阶段 6 持久 Session，阶段 8 取消、快照和恢复 | 统一 run/stream、事件、Trace、Usage 和终止原因 | 部分完成 |
...
| Q02 | Trace 与 Observability | Harness Artifact、工具指标、Usage 基础；5D-1 统一安全 run ID；ADR-0027 双层安全错误 provenance 已在 Provider stop/calibration 离线切片实现 | 阶段 5E | 本切片公开验证后，5E 再增加统一 Trace、日志、告警；G53 API 可用后沿用该合同 | run_id 串联 Prompt、模型、工具、证据、耗时和决策 | 部分完成 |
