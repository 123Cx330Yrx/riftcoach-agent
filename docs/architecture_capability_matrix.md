...
| A15 | 标准 MCP 与动态 Meta | 7-1…7-5 已公共闭环；实现 `a88fbc4/32483521108`、官方 SDK Client→RiftCoach stdio、RiftCoach Client→OP.GG Streamable HTTP 与不可覆盖 evidence `fac6fe0/32484257736` 均通过；8D `a274b7f/32598480400` 又完成 typed EvidenceBundle；8E ADR-0057 body-free diagnostic 已由 RQ-087 live replay 定位 `Mid.rank_prev_patch` AST `Name`，ADR-0058 nullable rank-history 精确 JSON `null` 修复由 `83fde7d/32615340228` 公共闭环 | 阶段 7 | 其他 OP.GG 工具仍按真实用例逐项 Adapter；实时刷新、正式 Coach/UI 与公网 Server/Auth/TLS 属 8E；修复后最终 live replay 前仍不得声称真实两源 bundle 已通过 | initialize、tools/list、tools/call、断线、版本与 owner 边界测试；EvidenceBundle provenance/join/conflict/expiry/schema 测试；schema-drift 诊断不得保存正文；null allowlist 正/负例 | V1 与 8D typed fusion complete；8E preflight 进行中 |
