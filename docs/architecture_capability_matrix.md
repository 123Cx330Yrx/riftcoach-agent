...
| A11 | AgentRuntime V1 | 5D 已通过退出审查：执行/Context 边界、Manifest 权限预算、可审计 draft/evidence、唯一 ReviewHarness、typed terminal output、结构化输出与领域评测门均已形成 | 阶段 5D-5E | 当前进入 5E 入口设计，统一 run/stream/event/trace/usage；阶段 6 持久 Session，阶段 8 取消、快照和恢复 | 统一 run/stream、事件、Trace、Usage 和终止原因 | 部分完成 |
...
| Q02 | Trace 与 Observability | Harness Artifact、工具指标、Usage 基础；5D-1 统一安全 run ID；ADR-0027 双层安全错误 provenance 已实现；5D 退出审查确认这些信号完整但分散 | 阶段 5E | 当前进入 5E，增加统一 Event/Trace/Usage；G53 API 可用后沿用安全错误合同 | run_id 串联 Prompt、模型、工具、证据、耗时和决策 | 部分完成 |
