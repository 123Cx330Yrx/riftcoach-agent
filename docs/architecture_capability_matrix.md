...
| A15 | 标准 MCP 与动态 Meta | 7-1…7-5 已公共闭环；实现 `a88fbc4/32483521108`、官方 SDK Client→RiftCoach stdio、RiftCoach Client→OP.GG Streamable HTTP 与不可覆盖 evidence `fac6fe0/32484257736` 均通过；8D `a274b7f/32598480400` 又完成 typed EvidenceBundle；8E 已增加 body-free schema-drift diagnostic 接缝（ADR-0057） | 阶段 7 | 其他 OP.GG 工具仍按真实用例逐项 Adapter；实时刷新、正式 Coach/UI 与公网 Server/Auth/TLS 属 8E；live drift 字段级裁决前保持 degraded/unavailable | initialize、tools/list、tools/call、断线、版本与 owner 边界测试；EvidenceBundle provenance/join/conflict/expiry/schema 测试；schema-drift 诊断不得保存正文 | V1 与 8D typed fusion complete；8E preflight 进行中 |
