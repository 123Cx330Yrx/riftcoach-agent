...
| A14 | API 与任务持久化 | 5P/6A/8C 已公开完成；8E Batch B `e844bdd/32622696087` 完成 profile/routing；Batch C `7975dc3/32629160732` 完成 0011 append-only Evidence revision、expiry view、owner-scoped Evidence/Product API 与 cursor SSE | 阶段 5P 提供本地同步切片，阶段 6 加 SQL、异步组合、安全与生命周期 | 8E 后续继续 Auth/前端/备份/部署；refresh scheduler 只由独立 writer Bad Case 触发 | receipt/path/Schema/SHA/终态交叉校验；PostgreSQL migration/revision/tamper/concurrency、HTTP 四态/错误、SSE reconnect、Linux composition smoke | V1 与 8E Batch B/C 公共闭环；后续产品化待完成 |
...
| Q02 | Trace 与 Observability | 5E Runtime Trace 已公开；8C 完成 body-free durable task event；8E Batch C `7975dc3/32629160732` 公共闭环 Last-Event-ID cursor SSE、keepalive、terminal/error close，仍不复制 Trace | 阶段 5E | 8E 后续增加正式指标、容量和部署监控；G53 沿用安全错误合同 | run_id 串联版本、模型、工具、证据、耗时、决策、event cursor、reconnect 和恢复结果 | 8C 与 Batch C SSE 公共闭环；生产容量待后续 |
...
| Q10 | 前端可解释性与可访问性 | 8E Batch B/C 已公共闭环 profile/routing 与 evidence/SSE/四态 DTO；Batch D design `88a5ab6/32631766013` 和 fixture React implementation `f7ebedd/32636771507` 均公共闭环，覆盖七场景、forbidden-field、Evidence Drawer、Training 与持久截图 | Stage 8 8E 首个正式 Web 纵向切片 | 下一步先设计真实 API/SSE 接线及缺失 Summary/report projection；之后按 RQ-093 继续电影感入口、真实 Rift Timeline 与完整 Training，Auth/部署另设门 | desktop/tablet/mobile、键盘/Escape/焦点返回、reduced-motion、axe critical/serious 0、no-remote-I/O、状态/数据边界和人工截图 QA | Batch D 同 SHA unit 35/e2e 12/typecheck/build、公共 pytest 1752 与三 job 全绿；真实数据接线待实现 |
