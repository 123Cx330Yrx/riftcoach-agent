...
| A16 | Multi-Agent 与 DAG | 8A strict gate 已由 `12ad835/32567642315` 公共闭环；RQ-082 授权的 8B evaluation-only 三路 runner、角色/工具/Context、digest Artifact、真实 Harness 与不可覆盖 lifecycle 已本地 TDD 通过，正式 holdout 仍为 0 次 | 阶段 8 Advanced | 实现 SHA exact-SHA CI 后才运行 development + 唯一 holdout；只在相对普通并行有增量收益且质量/安全/成本门全过时采用，无收益可 reject，DAG/Agentic Retrieval 继续 deferred | Bad Case、同切片对照、消融、Scripted 成本/延迟、失败隔离、不可覆盖 holdout、结果 validator 和最终 ADR | 8A complete；8B implementation local/pending public CI+holdout |
