...
| ID | 能力 | 当前基础 | V1 负责阶段 | 后续深化 | 验收证据 | 状态 |
|---|---|---|---|---|---|---|
| Q01 | 端到端 Evaluation | 报告事实评测、RAG holdout、路由评测；5D-7 已建立分层合同/实验身份、7 场 secure offline development、3 场未运行 held-out、生产案例 Executor/CLI 与资源/错误控制；DeepSeek 最小真实协议已通过，但未进入真实领域 E2E | 阶段 5C 增加路由 Eval，5D 增加 Prompt Eval | 生产装配 exact-SHA CI 后单次运行冻结 held-out；阶段 8 固定产品回归集和消融 | 数字忠实度、引用、路由、工具选择、实验身份、注入漏判、失败归因与发布安全 | 部分完成 |
