...
| A14 | API 与任务持久化 | 5P/6A/8C 已公开完成；8E Batch B `e844bdd/32622696087` 完成 profile/routing；Batch C 已本地实现 0011 append-only Evidence revision、expiry view、owner-scoped Evidence/Product API 与 cursor SSE | 阶段 5P 提供本地同步切片，阶段 6 加 SQL、异步组合、安全与生命周期 | 8E 后续继续 Auth/前端/备份/部署；refresh scheduler 只由独立 writer Bad Case 触发 | receipt/path/Schema/SHA/终态交叉校验；PostgreSQL migration/revision/tamper/concurrency、HTTP 四态/错误、SSE reconnect、Linux composition smoke | V1 与 8E Batch B 公共闭环；Batch C 本地完成、等待 exact-SHA 公共门 |
...
| Q02 | Trace 与 Observability | 5E Runtime Trace 已公开；8C 完成 body-free durable task event；8E Batch C 已本地增加 Last-Event-ID cursor SSE、keepalive、terminal/error close，仍不复制 Trace | 阶段 5E | 8E 后续增加正式指标、容量和部署监控；G53 沿用安全错误合同 | run_id 串联版本、模型、工具、证据、耗时、决策、event cursor、reconnect 和恢复结果 | 8C 公共闭环；Batch C SSE 本地完成、等待公共门 |
...
| Q10 | 前端可解释性与可访问性 | 尚无正式产品前端；Stage 8 entry design 冻结五模块/MotionSites 门；8E Batch B 已公共闭环 profile/routing，Batch C 已本地冻结 evidence/SSE/四态 DTO | Stage 8 8E 首个正式 Web 纵向切片 | Batch C exact-SHA 公共闭环后，Batch D 做静态 fixture screen、响应式、状态与动效，不提前接 Auth | 桌面/移动截图、键盘操作、reduced-motion、loading/empty/error/degraded 状态和数据边界展示 | 后端产品合同本地就绪；正式前端待 Batch D |
