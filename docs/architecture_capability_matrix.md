...
| A09 | Prompt/Context Engineering | Harness Prompt V0、SKILL.md 指令；5D-2 已实现 trust-typed Context Builder，5D-3 已实现逐轮 Context 门禁；5D-7 已冻结双层语义身份、Evaluation 1.1、安全 blocking policy、隔离 held-out 与独立输入计划；V2 首轮真实输入 3241 tokens；三阶段真实本地 request envelope 长度 6666/7774/6266 已进入校准 fixture | 阶段 5D-5E | 先以 development 校准 V3 资源合同，不把 tokenizer-free 投影冒充官方 Token；5E 加 Trace，阶段 6 加 Memory，阶段 7 加 Meta，阶段 8 做 Compaction | Prompt 版本、上下文优先级、Token 预算、漂移拒绝、用户/RAG 注入、回归和消融测试 | 部分完成 |
| A10 | 结构化模型输出 | 5D-6a 已建立 Provider-neutral 合同；Zhipu 与 DeepSeek V4 Pro 均已真实通过最小 3-call structured/tool 协议，DeepSeek 协议结果为 1428 tokens；V2 首例第一次响应可规范化，但第二轮前由 Token 门停止；ADR-0025 已证明当前门不可达，尚未到结构化 Evaluation | 阶段 5D | 先完成 V3 development 资源校准，再决定是否建立新门验证结构化 Evaluation；不重跑 V2 | 合法、缺字段、额外字段、截断、非 JSON、Schema 漂移、Thinking 预算、调用预算、可达性和修复上限测试 | 部分完成 |
...
| ID | 能力 | 当前基础 | V1 负责阶段 | 后续深化 | 验收证据 | 状态 |
|---|---|---|---|---|---|---|
| Q01 | 端到端 Evaluation | 报告事实评测、RAG holdout、路由评测；5D-7 已建立分层合同/实验身份、secure offline development、生产 Executor/CLI 与资源/错误控制；V2 已真实单次运行并公开归档，但首例第二轮前由 Token 门停止；ADR-0025 已把资源不可达与模型质量 unknown 分开，并持久化可重建的 no-I/O 裁决 | 阶段 5C 增加路由 Eval，5D 增加 Prompt Eval | 先公开验证裁决，再做 V3 development 资源校准；阶段 8 固定产品回归集和消融 | 数字忠实度、引用、路由、工具选择、实验身份、注入漏判、失败归因、预算可达性与发布安全 | 部分完成 |
...
| Q07 | 性能、Token 与成本 | Skill 预算、Tool 超时、Token Usage 基础；5D-2/3 Context 门禁和 D5 实验账本已实现；DeepSeek 协议为 3 calls/1428 tokens/约 `$0.00221496`；V2 首例为 1 call/3440 tokens/`$0.00506616`/12125 ms；ADR-0025 精确给出 next-call 4464 门槛，并用三阶段长度校准 fixture 暴露原 Fake Usage 失真 | 阶段 5E 定义运行预算，阶段 6 定义 API SLO | 用 development 校准新资源合同，保持精确 Usage/投影/unknown 分层；5E 统一 Trace，阶段 8 增加监控和成本告警 | p50/p95、Token、工具次数、模型成本、预算可达性和超预算停止 | 部分完成 |
