...
| A11 | AgentRuntime V1 | 5D 控制链及 5E-1 至 5E-4 均已公开完成；两个真实 Skill 共用同步 `run()`、进程内 `stream()`、typed output、完整 Trace/Usage、安全失败映射与 exit matrix；5F-1/5F-2 已公开完成 Pi 0.84.2 审计与隔离 spike；5F-3 又由 `3d9a081/32025522606` 公开证明 Harness/成功 Trace 可适配，但 Context 单位、extended terminal 与 live timing 硬 parity 失败 | 阶段 5D-5E | 5F-4 未进入；5F-5 决定 partial-adopt/reject；阶段 6 持久 Session，阶段 8 取消、快照和恢复 | 统一 run/stream、事件、Trace、Usage、终止原因、退出审查，以及 Pi batch/Usage/Trace/sidecar 差异矩阵 | 已完成 |
