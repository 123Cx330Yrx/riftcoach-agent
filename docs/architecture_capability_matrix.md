...
| A14 | API 与任务持久化 | 5P/6A/8C 已公开完成；8E Batch B `e844bdd/32622696087` 又完成 owner-scoped `GET /player-profiles`、Conversation opaque selection、legacy required region 与逐请求 routing | 阶段 5P 提供本地同步切片，阶段 6 加 SQL、异步组合、安全与生命周期 | 8E Batch C 再提供 EvidenceBundle store/refresh/expiry、SSE/四态产品 DTO 与部署接缝 | receipt/path/Schema/SHA/终态交叉校验；PostgreSQL migration、HTTP 幂等、event replay、profile owner isolation/去重、routing fingerprint、Linux composition smoke | V1 与 8E Batch B 公共闭环；Batch C 待实现 |
...
| Q10 | 前端可解释性与可访问性 | 尚无正式产品前端；Stage 8 entry design 已冻结五模块与 MotionSites 资源门；8E Batch B `e844bdd/32622696087` 已先公共闭环可选玩家档案、self/observed/region 语义和逐请求路由，保持“先合同后动效” | Stage 8 8E 首个正式 Web 纵向切片 | Batch C 先冻结 evidence/SSE/四态 DTO，Batch D 再做静态 fixture screen、响应式与动效 | 桌面/移动截图、键盘操作、reduced-motion、loading/empty/error/degraded 状态和数据边界展示 | 后端选择合同公共完成，正式前端待实现 |
