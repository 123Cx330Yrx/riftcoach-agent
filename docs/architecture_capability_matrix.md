...
| A03 | 模型 Provider 抽象 | 统一 ChatRequest/Response、Registry、能力协商；Zhipu/DeepSeek 最小协议已有真实证据但领域未准入；V2 `admitted=false`；ADR-0025/0026 已公开冻结资源 Bad Case 与校准设计；V3 development 双 profile、8 个生产请求 envelope、Fake replay 与 no-I/O admission 已本地实现，真实 Usage/质量仍 unknown | 阶段 3 | 完成 exact-SHA CI 后单独确认最多 8-call 真实校准；旧/V2 held-out 均不重跑 | 同一领域案例、Tool Calling、结构化输出、错误合同、调用/Token/金额停止与可达性证明 | 部分完成 |
...
| A10 | 结构化模型输出 | 5D-6a 已建立 Provider-neutral 合同；Zhipu 与 DeepSeek V4 Pro 均已真实通过最小协议；V2 第二轮前由不可达 Token 门停止；V3 development 已用现有 production Executor 构造 Evaluation + 一次 repair 请求形状并公开 body-free 快照，尚无真实四阶段 Usage | 阶段 5D | exact-SHA CI 后单独确认 development Usage replay，再决定是否建立 V3 质量门；不重跑 V2 | 合法、缺字段、额外字段、截断、非 JSON、Schema 漂移、Thinking 预算、调用预算、可达性和修复上限测试 | 部分完成 |
...
| ID | 能力 | 当前基础 | V1 负责阶段 | 后续深化 | 验收证据 | 状态 |
|---|---|---|---|---|---|---|
| Q01 | 端到端 Evaluation | 报告事实评测、RAG/路由评测与 5D-7 分层领域合同已建立；V2 因资源合同不可达而未测出质量；V3 development 已离线实现资源校准请求集、Fake 控制和预算裁决，把 Usage 测量与质量准入继续隔离 | 阶段 5C 增加路由 Eval，5D 增加 Prompt Eval | 先 exact-SHA 公开冻结，再单独确认真实 Usage replay；阶段 8 固定产品回归集和消融 | 数字忠实度、引用、路由、工具选择、实验身份、注入漏判、失败归因、预算可达性与发布安全 | 部分完成 |
