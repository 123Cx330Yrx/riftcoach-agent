...
| A09 | Prompt/Context Engineering | Harness Prompt V0、SKILL.md 指令；5D 已完成 trust-typed Context/预算与 Evaluation 1.1；5E Trace 有 prompt profile 字段；5P 已绑定 verified Prompt Program；6B-8 run-scoped Memory Context decorator/body-free manifest/terminal turns 已由 `aacc11a/32403187972` 公共闭环 | 5D-5P，6B-8 | 阶段 7-8 再加入 Meta、Compaction 与隔离上下文 | Prompt 版本、组件摘要、上下文优先级、whole-record ceiling、owner/role 隔离、manifest、注入、回归和消融测试 | V1 已完成，后续深化 |
...
| A13 | Session 与长期 Memory | RQ-060 至 RQ-071 已冻结 PostgreSQL 单一真源、claimed/observed、Conversation、Candidate、typed targets、Plan/Progress、Context 与 terminal turns；entry design、6B-1 至 6B-8 与历史教学/工程证据门均已公共闭环；6B-8 由 `aacc11a/32403187972` 验证 selector/0008/writer/package | 阶段 6 | 当前 6B-9 实现 owner export、三 scope 删除、retention/purge/补偿和 exit review；Redis/语义索引只由真实 Bad Case 触发，verified 仅在正式 Auth + 安全 RSO callback + `/accounts/me` PUUID 精确匹配后另行采用 | 无 CN 路由、claimed/observed、verified 不可创建、两 owner/Conversation/同 PUUID 隔离、Task binding、typed version、Plan/Progress、stable Context、terminal publication、查看/更正/导出/删除/补偿测试 | 进行中（6B-8 public pytest 1465/112 skips、PostgreSQL 157 passed、package schema 1.5/external calls 0；6B-9 设计/TDD） |
...
| Q05 | 数据生命周期与隐私 | 本地缓存不提交；6A-6 已公开验证 task/run 7/90/30 天保留与 hidden-before-cleanup；6B-1 至 6B-8 已建立 Memory 数据面但其 owner export/三 scope 删除/retention 仍待 6B-9 | 阶段 6 由 6B-9 完成 Memory 的查看/导出/更正/删除/补偿 | 阶段 8 加备份、恢复和公开隐私说明 | 原始比赛、Run、Memory 的保留、更正、导出、删除失败补偿测试 | 部分完成（6B-9 进行中） |
