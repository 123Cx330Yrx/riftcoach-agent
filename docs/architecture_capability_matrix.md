...
| A03 | 模型 Provider 抽象 | 统一 ChatRequest/Response、Registry、能力协商；Zhipu Adapter 真实最小 structured/tool 协议已准入，但 recent-form 真实领域运行在统一响应形成前失败并被拒绝；ADR-0017 已选择 DeepSeek V4 Flash 为有界候选 | 阶段 3 | 5D-7 D5 先离线实现独立 DeepSeek Adapter、安全错误归因与预算控制器，再经 exact-SHA CI 决定是否运行协议/领域门 | 同一领域案例、Tool Calling、结构化输出、错误合同、调用/Token/金额停止 | 部分完成 |
...
| A09 | Prompt/Context Engineering | Harness Prompt V0、SKILL.md 指令；5D-2 已实现 trust-typed Context Builder，5D-3 已实现逐轮 Context 门禁；5D-7 Batch B 已冻结双层语义身份，Batch C 又以用户/RAG canary 驱动 7 场零外部调用的真实本地控制流；D1-D2 已实现 Evaluation 1.1 安全上下文与 blocking policy，D3 已创建隔离 held-out，D4 已冻结候选与同任务实验条件 | 阶段 5D-5E | 5D-7 D5 先实现零调用实验入口，公开 CI 后才考虑有限真实实验；5E 加运行 Trace，阶段 6 加 Memory，阶段 7 加 Meta，阶段 8 做 Compaction | Prompt 版本、上下文优先级、Token 预算、漂移拒绝、用户/RAG 注入、回归和消融测试 | 部分完成 |
| A10 | 结构化模型输出 | 5D-6a 已建立 Provider-neutral 合同；Zhipu 真实 3-call structured/tool 协议通过，但真实领域运行未到 Evaluation，尚无领域结构化质量证据；DeepSeek 候选尚未实现 | 阶段 5D | 5D-7 D5 先离线验证 DeepSeek JSON/tool 映射和错误边界，再以独立协议门决定能否进入同任务领域评测 | 合法、缺字段、额外字段、截断、非 JSON、Schema 漂移、Thinking 预算、调用预算和修复上限测试 | 部分完成 |
...
| A12 | 多模型选择与降级 | Provider Registry 已有；ADR-0017 只选定 DeepSeek V4 Flash 实验候选，Adapter、任务级选择和自动降级均未实现 | 阶段 5F 或真实业务触发点 | 按质量、能力、成本选择，不按厂商数量堆叠 | 同一评测集、故障降级、成本和延迟对照 | 部分完成 |
...
| ID | 能力 | 当前基础 | V1 负责阶段 | 后续深化 | 验收证据 | 状态 |
|---|---|---|---|---|---|---|
| Q01 | 端到端 Evaluation | 报告事实评测、RAG holdout、路由评测；5D-7 Batch A/B 建立分层合同与实验身份，Batch C 新增 7 场 `offline_executable` development 基线并保留 1 个 unsafe-publication Bad Case；D1-D2 已完成 1.1 安全离线基线，D3 已创建 3 场 held-out 但未运行，D4 已冻结 DeepSeek/GLM 同任务采用门 | 阶段 5C 增加路由 Eval，5D 增加 Prompt Eval | 5D-7 D5 先完成离线 Adapter/控制器与错误归因，再决定有限真实结果；阶段 8 固定产品回归集和消融 | 数字忠实度、引用、路由、工具选择、实验身份、注入漏判、失败归因与发布安全 | 部分完成 |
...
| Q03 | Prompt/上下文注入防护 | 工具白名单、Schema、data-only sections、累积预算和实际 ToolExecutionRecord 证据；Batch C 已验证一个 Evaluation 漏判后的 unsafe publication；D1-D2 已以 1.1.0 接收最小用户/RAG 安全上下文并由 Harness 阻断安全 issue，D3 已创建隔离 held-out，D4 已冻结任一 unsafe publication 的全局停止规则 | 阶段 5D 建立不可信输入边界 | 5D-7 D5 先离线证明停止控制器和公开工件脱敏，之后才可能运行 held-out/真实模型；阶段 6 覆盖会话，阶段 7 覆盖外部 MCP/Tool 内容 | 恶意用户输入、恶意文档、恶意工具结果、评测漏判和越权测试 | 部分完成 |
...
| Q07 | 性能、Token 与成本 | Skill 预算、Tool 超时、Token Usage 基础；5D-2 初始 preflight 与 5D-3 累计消息门禁已实现；D4 已冻结每场 4000 total tokens、每请求 1024 output、15/12 call 和 GLM ¥0.50 / DeepSeek $0.05 停止线 | 阶段 5E 定义运行预算，阶段 6 定义 API SLO | 5D-7 D5 离线实现预算 ledger，有限真实运行校准 Usage；阶段 8 增加监控和成本告警 | p50/p95、Token、工具次数、模型成本和超预算停止 | 部分完成 |
...
修订 blocking policy，D3 已创建 3 场隔离 held-out 但未运行；D4 已由 ADR-0017 选择
DeepSeek V4 Flash 为唯一有界候选并冻结成本/停止规则。下一步只做 D5 离线 Adapter、
错误归因和控制器 TDD，不能把候选选择或 Scripted Provider 结果当成真实模型质量。
