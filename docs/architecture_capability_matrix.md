...
| A14 | API 与任务持久化 | 5P 同步切片与 6A-1 至 6A-5 已公开；POST 202、owner-scoped task/run/report、lifespan composition 和 DB/Alembic readiness 已由 `2492951/32106378542` 真库公共验证；Worker executable packaging 尚未完成 | 阶段 5P 提供本地同步切片，阶段 6 加 SQL 与异步组合 | 阶段 8 扩展 lease、取消、恢复与迟到结果治理 | receipt/path/Schema/SHA/终态交叉校验；PostgreSQL migration、HTTP 幂等、并发 claim、鉴权、隔离和中断测试 | 部分完成 |
...
| Q04 | 应用安全 | `.env` 隔离、日志脱敏、离线赛后合规；6A-5 已公开验证 trusted ActorContext、owner-scoped 查询、越权 404 与 production 缺 Auth fail-closed；正式 Auth、CORS/限流/HTTPS 仍未实现 | 阶段 6 建立鉴权、限流、CORS 与用户隔离 | 阶段 8 部署威胁模型、安全扫描和响应流程 | 密钥扫描、权限、限流、数据越权、CORS 和依赖审计 | 部分完成 |
...
| Q08 | 可靠性与故障恢复 | Harness/Tool/Artifact 基础与 6A-1/2/3/4 已公开；6A-4 的 receipt-proven success reconciliation、无证据 recovery-required、人工恢复 CAS 与迟到结果拒绝已由真实 PostgreSQL CI 验证；自动 reclaim 仍未实现 | 阶段 6 增加持久状态、幂等、短事务与有证据 reconciliation | 阶段 8 增加取消、lease/heartbeat/fencing、检查点、自动恢复和备份 | DB/Artifact 故障、并发 claim、进程中断、重复请求、人工恢复和迟到结果测试 | 部分完成 |
