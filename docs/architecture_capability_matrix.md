...
| A15 | 标准 MCP 与动态 Meta | 内部 Tool Runtime，不冒充 MCP；入口设计已由 ADR-0047 冻结 Adapter-first 边界与 OP.GG 条件准入 | 阶段 7 | OP.GG、官方补丁等通过领域 Adapter 分层 | initialize、tools/list、tools/call、断线和版本边界测试；真实外部 Server/Client 互操作 | 已规划（入口设计完成，7-1 待授权） |
