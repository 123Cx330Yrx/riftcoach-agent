...
| A03 | 模型 Provider 抽象 | 统一 ChatRequest/Response、Registry、能力协商；Zhipu/DeepSeek 最小协议有真实证据但领域未准入；DeepSeek V3 calibration 不完整且已由 ADR-0027 关闭 | 阶段 3 | G53-0 审计；任何未来真实门先补高层分类 + allowlisted 安全细分错误 provenance；旧结果均不重跑 | 同一领域案例、Tool Calling、结构化输出、错误合同、调用/Token/金额停止与可达性证明 | 部分完成 |
...
| A10 | 结构化模型输出 | 5D-6a 已建立 Provider-neutral 合同；Zhipu 与 DeepSeek V4 Pro 均真实通过最小协议；DeepSeek V3 首请求规范化失败且当前候选已关闭 | 阶段 5D | G53 按 thinking/structured/tool 新合同隔离审计；宽泛错误不用于猜根因 | 合法、缺字段、额外字段、截断、非 JSON、Schema 漂移、Thinking 预算、调用预算、可达性和修复上限测试 | 部分完成 |
...
| A12 | 多模型选择与降级 | Provider Registry 已有；DeepSeek V4 Pro 只通过最小协议，当前 V3 领域候选已关闭；尚无领域/产品准入、任务级选择或自动降级 | 5D 完成候选采用决策；模型分层为 5P 后横向采用门，默认等待阶段 6 真实业务证据 | G53 先隔离审计；未来仍按 ADR-0019 比较模型分层，5F 只做 Runtime SDK 实验 | 新鲜同任务评测、故障降级、unsafe publication、成本和 p50/p95 延迟对照 | 部分完成 |
...
| Q02 | Trace 与 Observability | Harness Artifact、工具指标、Usage 基础；5D-1 统一安全 run ID；ADR-0027 新增未来真实门的双层安全错误 provenance 前置条件 | 阶段 5E | G53 真实门前先做最小安全细分错误；5E/阶段 8 再增加统一 Trace、日志、告警 | run_id 串联 Prompt、模型、工具、证据、耗时和决策 | 部分完成 |
...
| Q07 | 性能、Token 与成本 | Skill/Tool/Context 预算和实验账本已实现；DeepSeek 协议 3 calls/1428 tokens，V2 1 call/3440 tokens，V3 calibration 1 call/0 normalized Usage 后关闭 | 阶段 5E 定义运行预算，阶段 6 定义 API SLO | G53 使用独立预算；未知计费保持 unknown；5E 统一 Trace | p50/p95、Token、工具次数、模型成本、预算可达性和超预算停止 | 部分完成 |
