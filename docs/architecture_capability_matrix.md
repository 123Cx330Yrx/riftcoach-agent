...
| A13 | Session 与长期 Memory | RQ-060 至 RQ-070 已冻结 PostgreSQL 单一真源、claimed/observed 关系、固定 subject Conversation、独立异步 Player Link、typed Candidate gate 与三类 typed target；entry design、6B-1 至 6B-5 与历史教学/工程证据门均已公共闭环；6B-6 pure/materializer/0006/version writer/query/package 已本地实现 | 阶段 6 | 6B-6 等待实现 SHA 的 PostgreSQL/package 公共门；Training Plan/Progress、Memory Context、Redis/语义索引只由后续检查点/真实 Bad Case 触发，verified 仅在正式 Auth + 安全 RSO callback + `/accounts/me` PUUID 精确匹配后另行采用 | 无 CN 路由前置拒绝、claimed/observed 权限差异、verified 当前不可创建、两用户/两会话/同 PUUID 隔离、conversation subject 不可变、task 继承、迟到结果、Riot ID 改名/PUUID 重指向、bootstrap 幂等/失败、typed target scope/role、version conflict、source candidate exactly-once、查看/历史/更正、过期、删除和补偿测试 | 进行中（6B-6 本地完整 1402 passed/100 PostgreSQL-Docker skips，待 exact-SHA 公共补证） |
