...
| A08 | Skill Router | 5C-1 至 5C-5 已完成；development 23/23，holdout 11/12，设备域假朋友 Bad Case 已原样保存 | 阶段 5C | 5C-6 比较确定性域边界、澄清和模型兜底；只在证据证明收益后采用 | 正例、负例、歧义、未支持、误路由、版本快照和拒绝测试 | 部分完成 |
...
当前唯一下一步是 5C-6 Model Fallback Decision。5C 仍然没有执行 Skill、调用 Tool
或调用 LLM；完成 5C-6 之前不得进入 5D。
