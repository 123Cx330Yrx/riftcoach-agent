...
| A15 | 标准 MCP 与动态 Meta | 内部 Tool Runtime，不冒充 MCP；ADR-0047 已冻结 Adapter-first；7-1 pure version/capability/catalog/call/schema/error contract 已本地完成 | 阶段 7 | OP.GG、官方补丁等通过领域 Adapter 分层 | initialize、tools/list、tools/call、断线和版本边界测试；真实外部 Server/Client 互操作 | 进行中（7-1 待公共闭环；transport/OP.GG/Server 未开始） |
