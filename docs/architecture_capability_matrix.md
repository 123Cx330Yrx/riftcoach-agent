...
| A03 | 模型 Provider 抽象 | 统一 ChatRequest/Response、Registry、能力协商；Zhipu Adapter 真实最小 structured/tool 协议已准入但领域未准入；独立 DeepSeek V4 Pro Adapter 已以 3/3 calls 通过真实最小协议，旧真实领域 held-out 首例暴露多 ToolCall 边界并 fail closed；ADR-0022 已公开验证多调用传输；Fresh-Gate 1/3/4 已公开冻结兼容合同、新资产和 V2 readmission/生产 CLI，同 SHA prepare-only 已准入，领域仍未准入且未注册为默认 | 阶段 3 | 下一步单独确认 V2 真实调用；旧 held-out 不重跑 | 同一领域案例、Tool Calling、结构化输出、错误合同、调用/Token/金额停止 | 部分完成 |
...
| Q03 | Prompt/上下文注入防护 | 工具白名单、Schema、data-only sections、累积预算和实际 ToolExecutionRecord 证据；Batch C 已验证一个 Evaluation 漏判后的 unsafe publication；D1-D2 已以 1.1.0 阻断安全 issue；新三案例注入身份与 V2 接缝均已公开冻结，Fake Provider 输出已验证不泄漏 marker/Key，但不证明真实模型抗注入 | 阶段 5D 建立不可信输入边界 | 另行确认冻结 held-out 的真实模型验证；阶段 6 覆盖会话，阶段 7 覆盖外部 MCP/Tool 内容 | 恶意用户输入、恶意文档、恶意工具结果、评测漏判和越权测试 | 部分完成 |
...
prepare-only 已通过。下一步必须经单独确认才可运行真实 V2 held-out，不能把低层协议、
Fake Provider、prepare-only 或离线修复当成真实模型报告质量。
