...
| A13 | Session 与长期 Memory | RQ-060 至 RQ-071 已冻结 PostgreSQL 单一真源、claimed/observed 关系、固定 subject Conversation、独立异步 Player Link、typed Candidate gate、三类 typed target 与 6B-7 Candidate-backed Plan/Artifact Progress 设计；entry design、6B-1 至 6B-6 与历史教学/工程证据门均已公共闭环；6B-6 由 `5531c81/32387026797` 验证 pure/materializer/0006/version writer/query/package | 阶段 6 | 当前 6B-7 实现 Training Plan/Progress；其 exact-SHA 公共闭环后才进入 6B-8 Memory Context，再进入 6B-9 lifecycle/export；Redis/语义索引只由真实 Bad Case 触发，verified 仅在正式 Auth + 安全 RSO callback + `/accounts/me` PUUID 精确匹配后另行采用 | 无 CN 路由前置拒绝、claimed/observed 权限差异、verified 当前不可创建、两用户/两会话/同 PUUID 隔离、conversation subject 不可变、task 继承、迟到结果、Riot ID 改名/PUUID 重指向、bootstrap 幂等/失败、typed target scope/role、version conflict、source candidate exactly-once、Plan active uniqueness、Artifact-grounded Progress、趋势非因果、查看/历史/更正、过期、删除和补偿测试 | 进行中（6B-6 public pytest 1402/100 skips、PostgreSQL 142 passed、package external calls 0；6B-7 设计/TDD） |
