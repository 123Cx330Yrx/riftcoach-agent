...
| A10 | 结构化模型输出 | 5D-6a 已建立 Provider-neutral 合同与严格 Pydantic 门禁；5D-6b 真实 P2 简单 JSON 通过，P3 暴露默认 Thinking 耗尽输出预算，受控复核进行中 | 阶段 5D | 先用 disabled-thinking 复核复杂 Schema，再做生产 Adapter 与领域切片；仅在真实阻断时比较一个候选 | 合法、缺字段、额外字段、截断、非 JSON、Schema 漂移、Thinking 预算和修复上限测试 | 部分完成 |
...
生产 Provider SDK 映射尚未实现。真实微探针已得到 P1/P2 通过、P3 默认 Thinking
耗尽 1024 输出 tokens、P4 返回一个 ToolCall 但未通过旧参数精确合同的部分证据；
这不是完整 Provider 准入。
统一 `run/stream/event/trace/usage` 表面继续属于 5E。

唯一下一步仍为 5D-6b：先完成 disabled-thinking 与 Tool JSON Schema 参数验收的受控
微探针，再决定是否进入生产 Adapter 离线 TDD。不得提前进入 5D-7，或把部分真实
协议观察/Fake Provider 合同测试称为完整 Provider 准入。
