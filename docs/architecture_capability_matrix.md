...
| A09 | Prompt/Context Engineering | Harness Prompt V0、SKILL.md 指令；5D 已完成 trust-typed Context/预算与 Evaluation 1.1；5E Trace 有 prompt profile 字段；5P 已绑定 verified Prompt Program；6B-8 run-scoped Memory Context decorator/body-free manifest/terminal turns 已由 `aacc11a/32403187972` 公共闭环 | 5D-5P，6B-8 | 8D 已开始把 Meta/Riot/static/patch 收敛为 data-only EvidenceBundle；正式 Coach/UI projection 留 8E | Prompt 版本、组件摘要、上下文优先级、whole-record ceiling、owner/role 隔离、manifest、Meta provenance、注入、回归和消融测试 | V1 已完成；8D typed fusion 本地进行中 |
...
| A14 | API 与任务持久化 | 5P 与 6A 已公开完成；8C 已完成 0010、durable event/replay、lease/fencing、cancel、checkpoint/recovery 与 package event query | 阶段 5P 提供本地同步切片，阶段 6 加 SQL、异步组合、安全与生命周期 | 8E 再提供 SSE/正式产品 DTO 与部署接缝 | receipt/path/Schema/SHA/终态交叉校验；PostgreSQL migration、HTTP 幂等、并发 claim、event replay、owner 隔离、CORS、删除、性能与 Linux composition smoke | V1 已完成；8C `2df5349/32587659678` 公共闭环 |
| A15 | 标准 MCP 与动态 Meta | 7-1…7-5 已公共闭环；实现 `a88fbc4/32483521108`、官方 SDK Client→RiftCoach stdio、RiftCoach Client→OP.GG Streamable HTTP 与不可覆盖 evidence `fac6fe0/32484257736` 均通过 | 阶段 7 | RQ-084 已启动 8D；ADR-0055、strict source/join/conflict/gap、canonical EvidenceBundle digest 与 no-I/O adapter 已本地实现，公共闭环待完成；其他 OP.GG 工具仍逐项 Adapter | initialize、tools/list、tools/call、断线、版本与 owner 边界测试；EvidenceBundle provenance/join/conflict/expiry/schema 测试 | V1 已完成；8D 本地进行中 |
...
| Q02 | Trace 与 Observability | 5E Runtime Trace 已公开；8C 又完成与 Trace 分层的 body-free durable task event、SHA identity、owner cursor replay 和 recovery result | 阶段 5E | 8E 增加生产指标与 SSE replay；G53 沿用安全错误合同 | run_id 串联版本、模型、工具、证据、耗时、决策、event cursor 和恢复结果 | V1 已完成；8C `2df5349/32587659678` 公共闭环 |
