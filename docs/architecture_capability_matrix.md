...
| A14 | API 与任务持久化 | 5P 已公开完成 product compiler、Prompt Program、Application、receipt/query 与薄 FastAPI no-I/O 切片；ADR-0038/6A design/implementation plan 已本地冻结 PostgreSQL polling task、短事务、幂等/ownership、reconciliation、NFR/安全/真库测试和 6A-1..7，但待 entry exact-SHA CI 且尚无 SQL 产品实现 | 阶段 5P 提供本地同步切片，阶段 6 加 SQL | 阶段 8 扩展 lease、取消、恢复与迟到结果治理 | receipt/path/Schema/SHA/终态交叉校验；PostgreSQL migration、HTTP 幂等、并发 claim、鉴权、隔离和中断测试 | 部分完成 |
...
| Q08 | 可靠性与故障恢复 | Harness 降级、Tool 重试/熔断、Artifact 哈希；6A 已设计 PostgreSQL durable task/幂等/原子 claim 和保守 hard-crash 人工恢复，尚无产品实现 | 阶段 6 增加持久状态、幂等、短事务与有证据 reconciliation | 阶段 8 增加取消、lease/heartbeat/fencing、检查点、自动恢复和备份 | DB/Artifact 故障、并发 claim、进程中断、重复请求、人工恢复和迟到结果测试 | 部分完成 |
