...
| A09 | Prompt/Context Engineering | Harness Prompt V0、SKILL.md 指令；5D 已完成 trust-typed Context/预算与 Evaluation 1.1；5E Trace 有 prompt profile 字段；5P-2 已实现 verified Prompt Program manifest/catalog/resolver、component fingerprint drift gate 与 production composition identity | 5D-5P | 5P-3/阶段 6-8 再加入 Application Service 上下文、Memory、Meta、Compaction 与隔离上下文 | Prompt 版本、组件摘要、上下文优先级、Token 预算、漂移拒绝、注入、回归和消融测试 | 部分完成 |
