...
| A09 | Prompt/Context Engineering | Harness Prompt V0、SKILL.md 指令；5D-2 已实现 trust-typed Context Builder，5D-3 已实现逐轮 Context 门禁；5D-7 已冻结双层语义身份、Evaluation 1.1、安全 blocking policy、隔离 held-out 与独立输入计划；Fresh-Gate 1 已让三个合成 development case 分别经真实 Router/Boundary/ContextBuilder 形成 body-free 摘要，并由 V1.1 plan 按序绑定，且已公开 CI | 阶段 5D-5E | 下一步为新 held-out 生成实际逐案例 Context commitment 并再次冻结；5E 加 Trace，阶段 6 加 Memory，阶段 7 加 Meta，阶段 8 做 Compaction | Prompt 版本、上下文优先级、Token 预算、漂移拒绝、用户/RAG 注入、回归和消融测试 | 部分完成 |
| A10 | 结构化模型输出 | 5D-6a 已建立 Provider-neutral 合同；Zhipu 与 DeepSeek V4 Pro 均已真实通过最小 3-call structured/tool 协议，DeepSeek 结果为 1428 tokens、无停止；真实领域首例在 Agent ToolCall 响应规范化阶段失败，因此 Evaluation 仍未运行；多 ToolCall 修复与 Fresh-Gate 1 兼容合同/no-I/O TDD 均已公开验证 | 阶段 5D | 先冻结新鲜 Dataset，再验证真实结构化 Evaluation；不以旧题、低层协议或合成 Executor 替代质量证据 | 合法、缺字段、额外字段、截断、非 JSON、Schema 漂移、Thinking 预算、调用预算和修复上限测试 | 部分完成 |
