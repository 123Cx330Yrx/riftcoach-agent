...
| A11 | AgentRuntime V1 | 5D 控制链及 5E-1 至 5E-4 均已公开完成；两个真实 Skill 共用同步 `run()`、进程内 `stream()`、typed output、完整 Trace/Usage、安全失败映射与 exit matrix；5F-1 至 5F-3 已公开完成 Pi 0.84.2 审计/隔离/Harness 对照，Context 单位、extended terminal 与 live timing 硬 parity 失败；5F-5 本地裁决产品拒绝 Pi，只冻结保留 evaluation-only 资产 | 阶段 5D-5E | 5F-4 未进入；ADR-0037 的可执行评测资产待公共闭环后冻结，产品继续 Python Runtime；阶段 6 持久 Session，阶段 8 取消、快照和恢复 | 统一 run/stream、事件、Trace、Usage、终止原因、退出审查，以及 Pi batch/Usage/Trace/sidecar 差异矩阵与采用/归档门 | 已完成 |
...
| A14 | API 与任务持久化 | 5P-1 至 5P-5 已公开完成严格产品 compiler、Prompt Program、Application、receipt/query 与薄 FastAPI no-I/O 纵向切片；5P-6 exit review 由 `8c8acc6/32010604551` 完成公共闭环；尚无真实外部 Provider/API 部署 | 阶段 5P 提供本地同步切片，阶段 6 加 SQL | 阶段 8 扩展恢复与运行治理 | receipt immutability、路径/Schema/SHA/终态交叉校验、Application 顺序/错误、四端点/OpenAPI/no-I/O；后续 HTTP 幂等、并发、鉴权、隔离和恢复测试 | 部分完成 |
...
| Q07 | 性能、Token 与成本 | 既有预算/实验账本保留；Task D 已本地聚合 Agent/Evaluation/revision 的共享 Provider Usage，并按 `llm.chat` 最坏 retry/repair/revision 计算副作用前 event budget；未配置价格仍为 null | 阶段 5E 定义运行预算，阶段 6 定义 API SLO | exact-SHA 验证 Task D；G53 使用独立预算；阶段 6 增加真实 p50/p95 与产品成本数据 | p50/p95、Token、工具次数、模型成本、预算可达性和超预算停止 | 部分完成 |
