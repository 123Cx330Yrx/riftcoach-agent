...
| A11 | AgentRuntime V1 | 5D 控制链已通过退出审查；5E-1 已公开；5E-2 Task A 已公开；Task B 的共享 Observed Provider、AgentLoop 业务 Tool/terminal 与 observation fail-fast 已本地完成，待公共 CI；Task C-D/run 未实现 | 阶段 5D-5E | 5E-2 Task C-D 接 Harness observer 并形成统一 run；5E-3 验证 stream parity；阶段 6 持久 Session，阶段 8 取消、快照和恢复 | 统一 run/stream、事件、Trace、Usage 和终止原因 | 部分完成 |
...
| Q02 | Trace 与 Observability | 5E-1 和 Task A 已公开；Task B 已在本地接入真实 Provider Adapter 边界、Agent 业务 Tool 与 Agent terminal 观察，并固定 observation fail-fast/默认关闭兼容，待公共 CI；Harness observer 与同步 run 尚未实现 | 阶段 5E | 5E-2 Task C-D 接持久化后 Harness 观察与同步 run；5E-3 验证流式同源；G53 沿用安全错误合同 | run_id 串联 Prompt、模型、工具、证据、耗时和决策 | 部分完成 |
...
| Q07 | 性能、Token 与成本 | 既有预算/实验账本保留；5E-1 已区分 Usage completeness；Task A 已令 Usage 必填；Task B 的共享 Provider decorator 已本地记录 Agent/Evaluation/repair/revision 的单次 Token 与连续 ordinal，待公共 CI 和 Task D 聚合 | 阶段 5E 定义运行预算，阶段 6 定义 API SLO | 5E-2 Task C-D 形成完整 run Usage/Trace；G53 使用独立预算；未知计费继续保持 null | p50/p95、Token、工具次数、模型成本、预算可达性和超预算停止 | 部分完成 |
