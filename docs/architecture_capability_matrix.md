...
| A14 | API 与任务持久化 | 5P/6A/8C 已公开完成；8E Batch B `e844bdd/32622696087` 完成 profile/routing；Batch C `7975dc3/32629160732` 完成 0011 Evidence/Product/SSE；RQ-096 又本地完成 owner-scoped latest locator、Recent Summary 与 typed Evidence HTTP composition | 阶段 5P 提供本地同步切片，阶段 6 加 SQL、异步组合、安全与生命周期 | 8E 后续继续 Auth/备份/部署；refresh scheduler 只由独立 writer Bad Case 触发 | receipt/path/Schema/SHA/终态交叉校验；PostgreSQL migration/revision/tamper/concurrency、locator owner/order、HTTP 四态/错误、SSE reconnect、Linux composition smoke | V1 与 Batch B/C 公共闭环；live integration 本地完成/公共 CI 待闭环 |
...
| Q02 | Trace 与 Observability | 5E Runtime Trace 已公开；8C 完成 body-free durable task event；8E Batch C `7975dc3/32629160732` 公共闭环 cursor SSE；RQ-096 前端本地完成 duplicate/mismatch/terminal/reconnect 与 authoritative reload，仍不复制 Trace | 阶段 5E | 8E 后续增加正式指标、容量和部署监控；G53 沿用安全错误合同 | run_id 串联版本、模型、工具、证据、耗时、决策、event cursor、reconnect、stream close 和恢复结果 | 8C/Batch C 公共闭环；live browser lifecycle 本地完成，生产容量待后续 |
...
| Q10 | 前端可解释性与可访问性 | Batch D fixture React 已由 `f7ebedd/32636771507` 公共闭环；RQ-096 本地完成 default-live API/SSE、exact decoder、identity race guard、verified report plain-text、真实 Training 与 live/fixture mode disclosure；RQ-094 视觉组合保持 `Rift Awakening → Esports Intelligence` | Stage 8 8E 首个正式 Web 纵向切片 | 继续电影感入口、真实 Rift Timeline、完整 Training、Auth/部署，并在 8F 前完成 fusion→Training→UI golden slice；完整 Markdown 仅在 bundle/安全门重新评估 | desktop/tablet/mobile、键盘/Escape/焦点返回、reduced-motion、axe critical/serious 0、no-remote-I/O、identity race/SSE cleanup、invalid-selection cleanup、stream byte-limit cancel、server-list URL selection、escaped report、状态/数据边界和人工截图 QA | live integration 本地 unit 66/e2e 17/typecheck/build、JS gzip 122.01 kB、完整 pytest 1939/真库 200/Linux package 全绿；exact-SHA 公共三 job 待闭环 |
