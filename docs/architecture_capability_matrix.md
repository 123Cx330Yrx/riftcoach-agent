...
| A03 | 模型 Provider 抽象 | 统一 ChatRequest/Response、Registry、能力协商；Zhipu Adapter 真实最小 structured/tool 协议已准入但领域未准入；独立 DeepSeek V4 Pro Adapter 已完成离线映射/错误 TDD与 exact-SHA/no-I/O 门，未真实调用或注册为默认 | 阶段 3 | 5D-7 下一步以最多 3-call 真实协议门决定能否进入同任务领域评测 | 同一领域案例、Tool Calling、结构化输出、错误合同、调用/Token/金额停止 | 部分完成 |
...
| A09 | Prompt/Context Engineering | Harness Prompt V0、SKILL.md 指令；5D-2 已实现 trust-typed Context Builder，5D-3 已实现逐轮 Context 门禁；5D-7 Batch B 已冻结双层语义身份，Batch C 又以用户/RAG canary 驱动 7 场零外部调用的真实本地控制流；D1-D2 已实现 Evaluation 1.1 安全上下文与 blocking policy，D3 已创建隔离 held-out，D4 冻结同任务条件，D5 已实现并公开验证零调用实验入口 | 阶段 5D-5E | 先运行有限真实协议门，通过后才可运行 held-out；5E 加运行 Trace，阶段 6 加 Memory，阶段 7 加 Meta，阶段 8 做 Compaction | Prompt 版本、上下文优先级、Token 预算、漂移拒绝、用户/RAG 注入、回归和消融测试 | 部分完成 |
| A10 | 结构化模型输出 | 5D-6a 已建立 Provider-neutral 合同；Zhipu 真实 3-call structured/tool 协议通过但领域未到 Evaluation；DeepSeek V4 Pro 的 JSON/tool/finish/usage 已离线 Fake SDK TDD，尚无真实响应 | 阶段 5D | 以独立最多 3-call DeepSeek 协议门决定能否进入同任务领域评测 | 合法、缺字段、额外字段、截断、非 JSON、Schema 漂移、Thinking 预算、调用预算和修复上限测试 | 部分完成 |
...
| A12 | 多模型选择与降级 | Provider Registry 已有；DeepSeek V4 Pro 独立 Adapter 已离线实现，但只作为 5D-7 单一实验候选，尚无真实准入、任务级选择或自动降级 | 5D 完成单候选准入；模型分层为 5P 后横向采用门，默认等待阶段 6 真实业务证据 | 按 ADR-0019 比较 Pro-only、Flash-only 与 Flash 默认/Pro 有界升级；5F 只做 Pi/Claude Agent SDK Runtime 采用实验 | 新鲜同任务评测、故障降级、unsafe publication、成本和 p50/p95 延迟对照 | 部分完成 |
...
| Q03 | Prompt/上下文注入防护 | 工具白名单、Schema、data-only sections、累积预算和实际 ToolExecutionRecord 证据；Batch C 已验证一个 Evaluation 漏判后的 unsafe publication；D1-D2 已以 1.1.0 阻断安全 issue，D3 已创建隔离 held-out；D5 已离线证明 unsafe publication 全局停止与公开 snapshot 脱敏 | 阶段 5D 建立不可信输入边界 | 之后才可能在冻结 held-out 上验证真实模型；阶段 6 覆盖会话，阶段 7 覆盖外部 MCP/Tool 内容 | 恶意用户输入、恶意文档、恶意工具结果、评测漏判和越权测试 | 部分完成 |
...
| Q07 | 性能、Token 与成本 | Skill 预算、Tool 超时、Token Usage 基础；5D-2/3 Context 门禁已实现；D5 已按 D4 冻结值实现 I/O 前 call/output/最坏成本预检与响应后 usage/Token/金额/延迟结算 | 阶段 5E 定义运行预算，阶段 6 定义 API SLO | 有限真实运行校准 Usage；5E 统一 Trace，阶段 8 增加监控和成本告警 | p50/p95、Token、工具次数、模型成本和超预算停止 | 部分完成 |
...
错误归因、控制器和 no-I/O 入口，并通过公开 exact-SHA CI/no-I/O 门；下一步是真实
协议门，不能把 Fake SDK 或 Scripted Provider 结果当成真实模型质量。
