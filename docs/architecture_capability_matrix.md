...
| A03 | 模型 Provider 抽象 | 统一 ChatRequest/Response、Registry、能力协商；Zhipu Adapter 真实最小 structured/tool 协议已准入但领域未准入；独立 DeepSeek V4 Pro Adapter 已以 3/3 calls 通过真实最小协议，旧真实领域 held-out 首例暴露多 ToolCall 边界并 fail closed；ADR-0022 已公开验证多调用传输；Fresh-Gate 1/3/4 已公开冻结；V2 真实门首例 1 call/3440 tokens 后由单例 Token 门在下一 I/O 前停止，领域 `admitted=false` 且未注册为默认 | 阶段 3 | 下一步离线裁决真实 Prompt 下预算可达性；旧/V2 held-out 均不重跑 | 同一领域案例、Tool Calling、结构化输出、错误合同、调用/Token/金额停止 | 部分完成 |
...
| A09 | Prompt/Context Engineering | Harness Prompt V0、SKILL.md 指令；5D-2 已实现 trust-typed Context Builder，5D-3 已实现逐轮 Context 门禁；5D-7 已冻结双层语义身份、Evaluation 1.1、安全 blocking policy、隔离 held-out 与独立输入计划；V2 首轮真实输入 3241 tokens，暴露单例 4000-token 合同无法保证工具往返与 Evaluation 可达 | 阶段 5D-5E | 下一步用真实 Context envelope 做零调用预算可达性 TDD；5E 加 Trace，阶段 6 加 Memory，阶段 7 加 Meta，阶段 8 做 Compaction | Prompt 版本、上下文优先级、Token 预算、漂移拒绝、用户/RAG 注入、回归和消融测试 | 部分完成 |
| A10 | 结构化模型输出 | 5D-6a 已建立 Provider-neutral 合同；Zhipu 与 DeepSeek V4 Pro 均已真实通过最小 3-call structured/tool 协议，DeepSeek 协议结果为 1428 tokens；旧真实领域首例在多 ToolCall 边界失败；V2 首例第一次响应可规范化，但第二轮前由 Token 门停止，尚未到结构化 Evaluation | 阶段 5D | 先证明真实多轮 Token 可达性，再决定是否用新门验证结构化 Evaluation；不重跑 V2 | 合法、缺字段、额外字段、截断、非 JSON、Schema 漂移、Thinking 预算、调用预算和修复上限测试 | 部分完成 |
...
| Q07 | 性能、Token 与成本 | Skill 预算、Tool 超时、Token Usage 基础；5D-2/3 Context 门禁已实现；D5 已实现实验账本；DeepSeek 协议为 3 calls/1428 tokens/约 `$0.00221496`；V2 新鲜首例为 1 call/3440 tokens/`$0.00506616`/12125 ms，下一请求被 4000-token 单例门在 I/O 前阻止 | 阶段 5E 定义运行预算，阶段 6 定义 API SLO | 先补真实长度预算可达性 fixture；5E 统一 Trace，阶段 8 增加监控和成本告警 | p50/p95、Token、工具次数、模型成本和超预算停止 | 部分完成 |
