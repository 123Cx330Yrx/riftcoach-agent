...
| A14 | API 与任务持久化 | 5P 已公开完成同步 product slice；6A-1 与 6A-2 已由 `854e52d/32043214500`、`012b066/32046532695` 真库公共验证；6A-3 claim/Worker 已本地实现，真实 PostgreSQL claim CI 待验证，API 尚未实现 | 阶段 5P 提供本地同步切片，阶段 6 加 SQL | 阶段 8 扩展 lease、取消、恢复与迟到结果治理 | receipt/path/Schema/SHA/终态交叉校验；PostgreSQL migration、HTTP 幂等、并发 claim、鉴权、隔离和中断测试 | 部分完成 |
...
| Q08 | 可靠性与故障恢复 | Harness 降级、Tool 重试/熔断、Artifact 哈希；6A 已设计 PostgreSQL durable task/幂等/原子 claim，6A-3 已本地实现 claim、ownership/CAS、退避和 graceful shutdown，真实 claim CI 待验证，hard-crash/Artifact 仍未实现 | 阶段 6 增加持久状态、幂等、短事务与有证据 reconciliation | 阶段 8 增加取消、lease/heartbeat/fencing、检查点、自动恢复和备份 | DB/Artifact 故障、并发 claim、进程中断、重复请求、人工恢复和迟到结果测试 | 部分完成 |
