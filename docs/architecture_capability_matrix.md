...
| A11 | AgentRuntime V1 | 5D 控制链已通过退出审查；5E-1、5E-2 和 5E-3 已公开；两个真实 Skill 共用的同步 `run()`、进程内 `stream()`、typed output、完整 Trace/Usage 与安全失败映射均有证据 | 阶段 5D-5E | 5E-4 完成整体 exit matrix；阶段 6 持久 Session，阶段 8 取消、快照和恢复 | 统一 run/stream、事件、Trace、Usage 和终止原因 | 部分完成 |
...
| Q02 | Trace 与 Observability | 5E-1、5E-2 和 5E-3 已公开；Runtime 时序、Artifact SHA、双终态、两阶段 Trace 提交、实时事件交付、消费者隔离和 parity 均有证据 | 阶段 5E | 5E-4 复核整体证据和限制；G53 沿用安全错误合同 | run_id 串联 Prompt、模型、工具、证据、耗时和决策 | 部分完成 |
...
进程内 `stream()`、背压、关闭隔离和 run/stream parity；当前 5E-4 只做 exit matrix 与退出
决策。5P、5F、阶段 6/8 的 API、SDK 对照、持久事件、cancel/resume、DAG、Memory 和
Multi-Agent 边界不变。
