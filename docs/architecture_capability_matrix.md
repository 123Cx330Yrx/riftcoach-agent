...
| A09 | Prompt/Context Engineering | Harness Prompt V0、SKILL.md 指令；5D-2 已实现 trust-typed Context Builder，5D-3 已实现逐轮 Context 门禁；5D-7 已冻结双层语义身份、Evaluation 1.1、安全 blocking policy、隔离 held-out 与独立输入计划；Fresh-Gate 1 已公开冻结兼容合同，Fresh-Gate 3 又让三个正式新案例分别经真实 Router/Boundary/ContextBuilder 形成 body-free 摘要并由 V1.1 plan 按序绑定 | 阶段 5D-5E | 下一步先公开冻结新资产，再用同一摘要做真实运行前漂移门；5E 加 Trace，阶段 6 加 Memory，阶段 7 加 Meta，阶段 8 做 Compaction | Prompt 版本、上下文优先级、Token 预算、漂移拒绝、用户/RAG 注入、回归和消融测试 | 部分完成 |
| A10 | 结构化模型输出 | 5D-6a 已建立 Provider-neutral 合同；Zhipu 与 DeepSeek V4 Pro 均已真实通过最小 3-call structured/tool 协议，DeepSeek 结果为 1428 tokens、无停止；真实领域首例在 Agent ToolCall 响应规范化阶段失败，因此 Evaluation 仍未运行；多 ToolCall 修复与 Fresh-Gate 1 已公开验证，新鲜 Dataset/输入/Context 已本地冻结但未运行 | 阶段 5D | 完成新资产公开冻结后再验证真实结构化 Evaluation；不以旧题、低层协议或合成 Executor 替代质量证据 | 合法、缺字段、额外字段、截断、非 JSON、Schema 漂移、Thinking 预算、调用预算和修复上限测试 | 部分完成 |
