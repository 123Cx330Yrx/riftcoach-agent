...
| A16 | Multi-Agent 与 DAG | 当前不需要；Stage 8 entry design 已登记 8A/8B 采用门 | 阶段 8 Advanced | 仅在独立上下文、权限和并行收益成立时采用；无收益可 reject，DAG 不预先强制 | Bad Case、同切片对照、消融、成本、失败隔离、污染记录和 ADR | entry design 已冻结，按证据采用 |
