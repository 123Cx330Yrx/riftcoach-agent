...
| A03 | 模型 Provider 抽象 | 统一 ChatRequest/Response、Registry、能力协商；Zhipu Adapter 真实最小 structured/tool 协议已准入但领域未准入；独立 DeepSeek V4 Pro Adapter 已以 3/3 calls 通过真实最小协议，真实领域 held-out 首例暴露多 ToolCall 边界并 fail closed；ADR-0022 已由 `037a47f` / Actions `31817798170` 公开验证多调用严格双向传输，领域仍未准入且未注册为默认；Fresh-Gate 1 已本地实现历史证据链和 development-only no-I/O admission | 阶段 3 | 先完成 Fresh-Gate 1 exact-SHA CI；冻结后才创建新 Dataset/输入/Context，旧 held-out 不重跑，真实调用需单独确认 | 同一领域案例、Tool Calling、结构化输出、错误合同、调用/Token/金额停止 | 部分完成 |
...
| A09 | Prompt/Context Engineering | Harness Prompt V0、SKILL.md 指令；5D-2 已实现 trust-typed Context Builder，5D-3 已实现逐轮 Context 门禁；5D-7 已冻结双层语义身份、Evaluation 1.1、安全 blocking policy、隔离 held-out 与独立输入计划；Fresh-Gate 1 已让三个合成 development case 分别经真实 Router/Boundary/ContextBuilder 形成 body-free 摘要，并由 V1.1 plan 按序绑定 | 阶段 5D-5E | 先完成新 Context commitment 的 exact-SHA CI，再创建新 held-out；5E 加 Trace，阶段 6 加 Memory，阶段 7 加 Meta，阶段 8 做 Compaction | Prompt 版本、上下文优先级、Token 预算、漂移拒绝、用户/RAG 注入、回归和消融测试 | 部分完成 |
| A10 | 结构化模型输出 | 5D-6a 已建立 Provider-neutral 合同；Zhipu 与 DeepSeek V4 Pro 均已真实通过最小 3-call structured/tool 协议，DeepSeek 结果为 1428 tokens、无停止；真实领域首例在 Agent ToolCall 响应规范化阶段失败，因此 Evaluation 仍未运行；多 ToolCall 修复已公开验证，Fresh-Gate 1 兼容合同/no-I/O TDD 已本地完成 | 阶段 5D | 先公开冻结 Fresh-Gate 1，再以新鲜 Dataset 验证真实结构化 Evaluation；不以旧题、低层协议或合成 Executor 替代质量证据 | 合法、缺字段、额外字段、截断、非 JSON、Schema 漂移、Thinking 预算、调用预算和修复上限测试 | 部分完成 |
...
兼容合同、历史证据链和 no-I/O admission，冻结后才创建新考卷；不能把低层协议或离线
修复当成真实模型报告质量。
