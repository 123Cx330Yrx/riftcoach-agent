...
| A11 | AgentRuntime V1 | 5D 控制链已通过退出审查；5E-1 已在本地实现严格 request/result/signal/event/trace、中央 Recorder、完整性明确的 Usage 和原子不可变 Trace Store | 阶段 5D-5E | 5E-2 接 observer 并形成统一 run；5E-3 验证 stream parity；阶段 6 持久 Session，阶段 8 取消、快照和恢复 | 统一 run/stream、事件、Trace、Usage 和终止原因 | 部分完成 |
...
| Q02 | Trace 与 Observability | 5E-1 已在本地新增安全类型化 Signal/Event、严格调用生命周期、complete/partial/unknown Usage、Trace 哈希引用和不可覆盖原子存储；尚未接入真实执行观察点 | 阶段 5E | 5E-2/3 接 Agent/Harness observer 与同步/流式同源执行；G53 API 可用后沿用安全错误合同 | run_id 串联 Prompt、模型、工具、证据、耗时和决策 | 部分完成 |
...
| Q07 | 性能、Token 与成本 | 既有预算/实验账本保留；5E-1 已在本地区分 Token complete/partial/unknown/not_applicable，并只在完整 Usage + 版本化价格表下计算 Decimal 成本 | 阶段 5E 定义运行预算，阶段 6 定义 API SLO | 5E-2 接真实运行观测；G53 使用独立预算；未知计费继续保持 null | p50/p95、Token、工具次数、模型成本、预算可达性和超预算停止 | 部分完成 |
