...
| Q10 | 前端可解释性与可访问性 | 尚无正式产品前端；8E Batch B/C 已公共闭环 profile/routing 与 evidence/SSE/四态 DTO；RQ-091/RQ-092 与 ADR-0061 已冻结多来源两层采用门、Rift Command Center、tokens、客户端/产品状态和 a11y 合同 | Stage 8 8E 首个正式 Web 纵向切片 | Batch D 正按 TDD 做静态 fixture screen、响应式、键盘和克制但高完成度的动效，不提前接 Auth/API | 桌面/移动截图、键盘操作、reduced-motion、loading/empty/error/degraded 状态、forbidden-field 和数据边界展示 | 后端产品合同公共就绪；Batch D authorized/in progress |
