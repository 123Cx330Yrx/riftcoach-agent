...
| A14 | API 与任务持久化 | 5P 已公开完成同步 product slice；6A-1 已由 `854e52d/32043214500` 真库公共验证；6A-2 task contract/idempotent Repository 已本地完成，真库 create/query/concurrency 证据待 public CI；Worker/API 尚未实现 | 阶段 5P 提供本地同步切片，阶段 6 加 SQL | 阶段 8 扩展 lease、取消、恢复与迟到结果治理 | receipt/path/Schema/SHA/终态交叉校验；PostgreSQL migration、HTTP 幂等、并发 claim、鉴权、隔离和中断测试 | 部分完成 |
