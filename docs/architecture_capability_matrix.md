...
| A16 | Multi-Agent 与 DAG | 8A strict gate 已由 `12ad835/32567642315` 公共闭环，冻结串行 baseline、普通受限并行 comparator、角色隔离 Multi-Agent primary candidate，并将 DAG/Agentic Retrieval deferred | 阶段 8 Advanced | 8B 只在独立 Context、工具权限、失败隔离及相对普通并行的增量收益成立时采用；无收益可 reject，DAG 不预先强制 | Bad Case、同切片对照、消融、成本、失败隔离、污染记录、不可覆盖 holdout 和 ADR | 8A complete；8B prepared/未实现 |
