...
| A03 | 模型 Provider 抽象 | 统一 ChatRequest/Response、Registry、能力协商；Zhipu Adapter 真实最小 structured/tool 协议已准入但领域未准入；独立 DeepSeek V4 Pro Adapter 已以 3/3 calls 通过真实最小协议；ADR-0022 已公开验证多 ToolCall 传输；Fresh-Gate 1/3/4 已公开冻结；V2 领域 `admitted=false`；ADR-0025 证明下一调用最低上限 4464；ADR-0026 又冻结两 profile/四阶段 Usage replay 和预算公式，模型质量仍 unknown | 阶段 3 | 先离线实现并公开冻结 V3 development 校准合同，再单独确认最多 8-call 真实校准；旧/V2 held-out 均不重跑 | 同一领域案例、Tool Calling、结构化输出、错误合同、调用/Token/金额停止与可达性证明 | 部分完成 |
...
| A10 | 结构化模型输出 | 5D-6a 已建立 Provider-neutral 合同；Zhipu 与 DeepSeek V4 Pro 均已真实通过最小 3-call structured/tool 协议，DeepSeek 协议结果为 1428 tokens；V2 首例第一次响应可规范化，但第二轮前由 Token 门停止；ADR-0025 已证明当前门不可达；ADR-0026 明确 Evaluation 非法时第 4 call repair 也必须资源可达 | 阶段 5D | 先完成 V3 development 四阶段资源校准，再决定是否建立新门验证结构化 Evaluation；不重跑 V2 | 合法、缺字段、额外字段、截断、非 JSON、Schema 漂移、Thinking 预算、调用预算、可达性和修复上限测试 | 部分完成 |
...
| ID | 能力 | 当前基础 | V1 负责阶段 | 后续深化 | 验收证据 | 状态 |
|---|---|---|---|---|---|---|
| Q01 | 端到端 Evaluation | 报告事实评测、RAG holdout、路由评测；5D-7 已建立分层合同/实验身份、secure offline development、生产 Executor/CLI 与资源/错误控制；V2 已真实单次运行并公开归档，但首例第二轮前由 Token 门停止；ADR-0025 已把资源不可达与模型质量 unknown 分开；ADR-0026 将资源校准与质量采用进一步隔离 | 阶段 5C 增加路由 Eval，5D 增加 Prompt Eval | 先实现/公开冻结 V3 development 资源校准，再单独确认 Usage replay；阶段 8 固定产品回归集和消融 | 数字忠实度、引用、路由、工具选择、实验身份、注入漏判、失败归因、预算可达性与发布安全 | 部分完成 |
...
| Q07 | 性能、Token 与成本 | Skill 预算、Tool 超时、Token Usage 基础；5D-2/3 Context 门禁和 D5 实验账本已实现；DeepSeek 协议为 3 calls/1428 tokens/约 `$0.00221496`；V2 首例为 1 call/3440 tokens/`$0.00506616`/12125 ms；ADR-0025 给出 next-call 4464 门槛；ADR-0026 冻结两 profile/四阶段 replay、逐阶段 input ×1.25、四次 output ceiling、延迟与 `$0.10` 停止规则 | 阶段 5E 定义运行预算，阶段 6 定义 API SLO | 先离线实现并公开冻结校准门；真实 8-call development replay 需单独确认，结果不用于质量；5E 统一 Trace | p50/p95、Token、工具次数、模型成本、预算可达性和超预算停止 | 部分完成 |
