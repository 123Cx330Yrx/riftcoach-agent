...
| A12 | 多模型选择与降级 | Provider Registry 已有；DeepSeek V4 Pro 独立 Adapter 已离线实现，但只作为实验候选，尚无真实准入、任务级选择或自动降级 | 阶段 5F 或真实业务触发点 | 按质量、能力、成本选择，不按厂商数量堆叠；Flash 仅在成本/时延 Bad Case 后评估 | 同一评测集、故障降级、成本和延迟对照 | 部分完成 |
