...
| A11 | AgentRuntime V1 | 5D 控制链及 5E-1 至 5E-4 均已公开完成；两个真实 Skill 共用同步 `run()`、进程内 `stream()`、typed output、完整 Trace/Usage、安全失败映射与 exit matrix；5F-1 至 5F-5 已完成 Pi 0.84.2 审计/隔离/Harness/采用对照，最终由 `f8dea66/32028206103` 公共裁决产品拒绝 Pi、冻结保留 evaluation-only 资产 | 阶段 5D-5E | Stage 8 8C 的 durable event、cancel、lease/fencing、checkpoint、recovery 与 replay 已本地实现，待 exact-SHA 真库/Linux 闭环 | 统一 run/stream、事件、Trace、Usage、终止原因、恢复/迟到结果退出审查，以及 Pi batch/Usage/Trace/sidecar 差异矩阵与采用/归档门 | V1 已完成；8C 本地完成/公共待验证 |
...
| A14 | API 与任务持久化 | 5P 与 6A 已公开完成；8C 已本地增加 0010、durable event/replay、lease/fencing、cancel、checkpoint/recovery 与 package event query | 阶段 5P 提供本地同步切片，阶段 6 加 SQL、异步组合、安全与生命周期 | 8C 等待 exact-SHA PostgreSQL/Linux 闭环；8E 再提供 SSE/正式产品 DTO | receipt/path/Schema/SHA/终态交叉校验；PostgreSQL migration、HTTP 幂等、并发 claim、event replay、owner 隔离、CORS、删除、性能与 Linux composition smoke | V1 已完成；8C 本地深化/公共待验证 |
...
| Q02 | Trace 与 Observability | 5E Runtime Trace 已公开；8C 又本地实现与 Trace 分层的 body-free durable task event、SHA identity、owner cursor replay 和 recovery result | 阶段 5E | 8C 等待真库/Linux 公共门；8E 增加生产指标与 SSE replay；G53 沿用安全错误合同 | run_id 串联版本、模型、工具、证据、耗时、决策、event cursor 和恢复结果 | V1 已完成；8C 本地深化/公共待验证 |
...
| Q08 | 可靠性与故障恢复 | 6A 的 receipt reconciliation/recovery-required/人工 CAS 已公开；8C 已本地实现取消、lease/heartbeat/fencing、safe checkpoint、bounded automatic recovery、replay 与 late-result isolation | 阶段 6 增加持久状态、幂等、短事务、有证据 reconciliation 与安全生命周期 | 8C 仍需 exact-SHA PostgreSQL 17 并发/migration 与 Linux package；备份属于 8E | DB/Artifact 故障、并发 claim、进程中断、重复请求、人工恢复、自动 reclaim、删除补偿和迟到结果测试 | 8C 本地完成/公共待验证 |
