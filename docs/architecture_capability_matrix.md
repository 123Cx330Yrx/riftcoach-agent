...
| A03 | 模型 Provider 抽象 | 统一 ChatRequest/Response、Registry、能力协商；Zhipu Adapter 真实最小 structured/tool 协议已准入但领域未准入；独立 DeepSeek V4 Pro Adapter 已以 3/3 calls 通过真实最小协议，真实领域 held-out 首例暴露多 ToolCall 边界并 fail closed；ADR-0022 已公开验证多调用传输；Fresh-Gate 1/3 已公开冻结兼容合同和新资产；V2 readmission/生产 CLI/prepare-only 已本地通过，领域仍未准入且未注册为默认 | 阶段 3 | 下一步完成入口 exact-SHA CI 与同 SHA no-I/O prepare-only，再单独确认真实调用；旧 held-out 不重跑 | 同一领域案例、Tool Calling、结构化输出、错误合同、调用/Token/金额停止 | 部分完成 |
...
| A09 | Prompt/Context Engineering | Harness Prompt V0、SKILL.md 指令；5D-2 已实现 trust-typed Context Builder，5D-3 已实现逐轮 Context 门禁；5D-7 已冻结双层语义身份、Evaluation 1.1、安全 blocking policy、隔离 held-out 与独立输入计划；Fresh-Gate 1/3 已公开冻结兼容合同和三个正式案例的 body-free 摘要；Fresh-Gate 4 readmission 已在本地重建并拒绝任一逐案例漂移 | 阶段 5D-5E | 下一步公开验证同一漂移门并执行 no-I/O prepare-only；5E 加 Trace，阶段 6 加 Memory，阶段 7 加 Meta，阶段 8 做 Compaction | Prompt 版本、上下文优先级、Token 预算、漂移拒绝、用户/RAG 注入、回归和消融测试 | 部分完成 |
| A10 | 结构化模型输出 | 5D-6a 已建立 Provider-neutral 合同；Zhipu 与 DeepSeek V4 Pro 均已真实通过最小 3-call structured/tool 协议，DeepSeek 结果为 1428 tokens、无停止；旧真实领域首例在 Agent ToolCall 响应规范化阶段失败；多 ToolCall 修复与新资产已公开冻结，V2 运行接缝已本地装配但新题未运行 | 阶段 5D | 先公开验证 V2 接缝与 no-I/O prepare-only，再单独确认真实结构化 Evaluation；不以旧题、低层协议或 Fake Provider 替代质量证据 | 合法、缺字段、额外字段、截断、非 JSON、Schema 漂移、Thinking 预算、调用预算和修复上限测试 | 部分完成 |
...
| ID | 能力 | 当前基础 | V1 负责阶段 | 后续深化 | 验收证据 | 状态 |
|---|---|---|---|---|---|---|
| Q01 | 端到端 Evaluation | 报告事实评测、RAG holdout、路由评测；5D-7 已建立分层合同/实验身份、secure offline development、生产 Executor/CLI 与资源/错误控制；旧真实 held-out 首例失败且已消费，多 ToolCall development 纵向链已通过；Fresh-Gate 1/3 已公开冻结历史链与新资产；Fresh-Gate 4 已在本地把新资产接入 readmission、Fresh result envelope 与生产 CLI，Fake 正常/首错路径通过 | 阶段 5C 增加路由 Eval，5D 增加 Prompt Eval | 下一步入口 exact-SHA CI 与同 SHA no-I/O prepare-only；真实新 held-out 仍需单独确认，阶段 8 固定产品回归集和消融 | 数字忠实度、引用、路由、工具选择、实验身份、注入漏判、失败归因与发布安全 | 部分完成 |
...
| Q03 | Prompt/上下文注入防护 | 工具白名单、Schema、data-only sections、累积预算和实际 ToolExecutionRecord 证据；Batch C 已验证一个 Evaluation 漏判后的 unsafe publication；D1-D2 已以 1.1.0 阻断安全 issue；新三案例注入身份已公开冻结，V2 envelope/CLI 的 Fake Provider 输出已验证不泄漏 marker/Key，但不证明真实模型抗注入 | 阶段 5D 建立不可信输入边界 | 入口公开 CI 与 no-I/O prepare-only 后，另行确认冻结 held-out 的真实模型验证；阶段 6 覆盖会话，阶段 7 覆盖外部 MCP/Tool 内容 | 恶意用户输入、恶意文档、恶意工具结果、评测漏判和越权测试 | 部分完成 |
...
| Q07 | 性能、Token 与成本 | Skill 预算、Tool 超时、Token Usage 基础；5D-2/3 Context 门禁已实现；D5 已实现实验账本；DeepSeek 真实协议记录 3 calls、1428 tokens、约 `$0.00221496` 与 14844 ms 累计 Provider latency | 阶段 5E 定义运行预算，阶段 6 定义 API SLO | 领域 held-out 校准 Usage；5E 统一 Trace，阶段 8 增加监控和成本告警 | p50/p95、Token、工具次数、模型成本和超预算停止 | 部分完成 |
...
与生产 CLI 组合。下一步是该入口的 exact-SHA CI 和同 SHA no-I/O prepare-only，不能把
低层协议、Fake Provider 或离线修复当成真实模型报告质量。
