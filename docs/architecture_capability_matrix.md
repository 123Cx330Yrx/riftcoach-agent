...
| A03 | 模型 Provider 抽象 | 统一 ChatRequest/Response、Registry、能力协商；Zhipu Adapter 真实最小 structured/tool 协议已准入但领域未准入；独立 DeepSeek V4 Pro Adapter 已完成离线映射/错误 TDD，未真实调用或注册为默认 | 阶段 3 | 5D-7 先过 exact-SHA/no-I/O，再以最多 3-call 协议门决定能否进入同任务领域评测 | 同一领域案例、Tool Calling、结构化输出、错误合同、调用/Token/金额停止 | 部分完成 |
...
错误归因、控制器和 no-I/O 入口；下一步仍须先过公开 SHA 与真实协议门，不能把 Fake
SDK 或 Scripted Provider 结果当成真实模型质量。
