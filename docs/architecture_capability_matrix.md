...
| A03 | 模型 Provider 抽象 | 统一 ChatRequest/Response、Registry、能力协商；Zhipu Adapter 真实最小 structured/tool 协议已准入，但 recent-form 真实领域运行在统一响应形成前失败并被拒绝；ADR-0018 已把有界候选更正为 DeepSeek V4 Pro | 阶段 3 | 5D-7 D5 先离线实现独立 DeepSeek Adapter、安全错误归因与预算控制器，再经 exact-SHA CI 决定是否运行协议/领域门 | 同一领域案例、Tool Calling、结构化输出、错误合同、调用/Token/金额停止 | 部分完成 |
...
| A12 | 多模型选择与降级 | Provider Registry 已有；ADR-0018 只选定 DeepSeek V4 Pro 实验候选，Adapter、任务级选择和自动降级均未实现 | 阶段 5F 或真实业务触发点 | 按质量、能力、成本选择，不按厂商数量堆叠；Flash 仅在成本/时延 Bad Case 后评估 | 同一评测集、故障降级、成本和延迟对照 | 部分完成 |
...
| Q07 | 性能、Token 与成本 | Skill 预算、Tool 超时、Token Usage 基础；5D-2 初始 preflight 与 5D-3 累计消息门禁已实现；D4 已冻结每场 4000 total tokens、每请求 1024 output、15/12 call 和 GLM ¥0.50 / DeepSeek Pro $0.10 停止线 | 阶段 5E 定义运行预算，阶段 6 定义 API SLO | 5D-7 D5 离线实现预算 ledger，有限真实运行校准 Usage；阶段 8 增加监控和成本告警 | p50/p95、Token、工具次数、模型成本和超预算停止 | 部分完成 |
...
唯一有界候选更正为 DeepSeek V4 Pro，并冻结成本/停止规则。下一步只做 D5 离线 Adapter、
