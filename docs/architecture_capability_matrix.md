...
| A03 | 模型 Provider 抽象 | 统一 ChatRequest/Response、Registry、能力协商；Zhipu Adapter 真实最小 structured/tool 协议已准入但领域未准入；独立 DeepSeek V4 Pro Adapter 也已在 exact-SHA 上以 3/3 calls 通过真实最小协议，领域 admission/累计资源/逐例停止接缝已完成离线 TDD 与公开 CI，但未运行 held-out 或注册为默认 | 阶段 3 | 5D-7 冻结/装配真实案例计划与生产 Executor/CLI，先通过新的 exact-SHA CI，再执行单次领域准入 | 同一领域案例、Tool Calling、结构化输出、错误合同、调用/Token/金额停止 | 部分完成 |
