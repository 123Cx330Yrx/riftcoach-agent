...
| A09 | Prompt/Context Engineering | Harness Prompt V0、SKILL.md 指令；5D 已完成 trust-typed Context/预算与 Evaluation 1.1；5E Trace 有 prompt profile 字段；5P-2 已实现 verified Prompt Program identity；5P-3 secure factory 已让实际 Evaluator/Reviser 与 Program 1.1 对齐 | 5D-5P | 阶段 6-8 再加入 Memory、Meta、Compaction 与隔离上下文 | Prompt 版本、组件摘要、上下文优先级、Token 预算、漂移拒绝、实际 factory 类型、注入、回归和消融测试 | 部分完成 |
...
| A14 | API 与任务持久化 | CLI/File Run Store；5P-1 至 5P-3 已公开；5P-4 immutable receipt、strict Trace/manifest/final Artifact query 与 Application receipt 接缝已本地通过，尚待公共 CI；FastAPI 尚无代码 | 阶段 5P 提供本地同步切片，阶段 6 加 SQL | 阶段 8 扩展恢复与运行治理 | receipt immutability、路径/Schema/SHA/终态交叉校验、Application 顺序/错误；后续 HTTP、幂等、并发、鉴权、隔离和恢复测试 | 部分完成 |
...
Actions `31998739178` 完成 exact-SHA 公共验证；5P-4 receipt/query 已本地完成并等待公共 CI，
FastAPI 尚未实现，Provider I/O 仍为 0。
