...
以下为历史记录，当前以上述独立诊断行动为准。

2026-09-21中断恢复及本批完成：核实“依然继续”停在clean 354d752、无在途Provider或同提交旧run；Actions35565312170三项success。按claim-scope 1→4→3执行7次完整stop，79804输入+29040输出=108844tokens，未知0，累计Provider至少272。正确全文95/pass；全称负例82/needs_revision→实际修订→96/pass，正文正确使用视野33.75/早死1.5，独立接受但保留首评33.5/33.75措辞及引用精度限制。第三例正确检出合并补刀错误，却把与正例相同的block4泛指补成全称误报；虽实际修订后96/pass，整路径人工拒绝。本批立即停止扩展，无后续scope/历史五例/产品调用。完整原文、7响应公开投影、原回执/哈希及独立请求重建见golden_native_claim_scope_result_354d752.json；原失败不改。

已离线修复开发验证的执行缺口：首评命中目标但另有未标注问题时，在修订前结束该run，保留全部issue，标记未裁决而非自动判假。真实失败回放从3调用变为1调用，避免后续2次的路径已验证（原27739tokens仍是已发生费用）；两合格案例路径保持，真实额外问题/同段局限也有反例覆盖。89项相关测试与实际Runtime组装通过；关闭native入口时完整manifest已同步。产品模型、high、全部预算及原生评估提示/Schema未改；这不是模型语义修复。native为offline_claim_scope_false_positive，产品仍offline，8E仍in_progress。公共检查以本次修复提交的PR7记录为准。

离线合同比较证据已更新为golden_native_contract_options_offline_v3.json：A的原句/上下文锚点仍可承载真实误报，只有审计定位价值；B在既有修订槽内逐项裁决并交完整报告，保留原首评与独立复评。分析者见证仅改block6错误尾句，正确前半句与其余全文逐字保留，五调用保守预约388108。历史五调用见证403828超401920；实际预算器在满额合成usage下第5次前停止，低usage则能5次，不能把全路径预约和当新增硬门或断言历史必然失败。B的错误撤销/未改真错等结构反例均保留semantic_approval=false；未改变首评模型，不能宣称已修正首评误报或绕过开发guard。产品生成占用与下一候选裁决见2026-09-20计划最新节；本批不再付费。实际两状态重评、同版本剩余范围/归因/历史五例、产品来源绑定/Evidence/真实事务/API/Workbench及独立评估/学习仍待完成。前端整体审美/重做、英雄头像、四块联动、专属Training与62主题继续保留。

继续验证产品共享预算：真实typed compiler/本地RAG/Runtime搭配脚本响应，生成两次→首评→编辑→终评共5调用、保守合成用量365638，可完成；多一次首评重评则第6次终评被调用预算拒绝、不发布。仅测试实例注入B，不是新的产品注册、观摩身份/外部Evidence验收或模型质量证明。相关72测试通过。已准备混合真假/纯真错/纯误报三组完整编辑诊断输入，来源和请求hash可重建、预期不入模型；独立审查通过，仍offline_unregistered。唯一下一步：基于冻结诊断计划准备独立回执执行边界并完成同SHA检查，再裁决是否进入单独有界编辑实测；旧失败批不恢复、首评准确率未修复、产品仍不准入。

以下为历史记录，当前以上述中断恢复、离线合同及产品预算结果为准。
...
v3.6 / Coach1.4.6 / Skill0.6.0 / Program3.0.6 / wire3.2.0 的离线改动有两项：重评仅用 replaced/withdrawn，每个旧问题仍须显式映射；旧 retained 逐字段验证仅显式历史回放可用，新请求与验证均拒绝 retained。计算证据改为按统计量命名的数组，win_mean/loss_mean 与 cohort_mean/cohort_median 分开；全部原值、成员、缺失、位置对比与引用编号无损，来源解析和哈希绑定同一投影。默认旧布局不变。收尾另修正历史回放的来源身份：legacy明确绑定v3.5 rows投影，新旧journal分别保持正确的来源/catalog哈希；新增回归验证，旧离线证据保留，新v2证据绑定最终manifest。这消除重评的复制选择分支并改善数值定位，不证明模型语义已经修复。

反证已固化：只把真实 retained 改成 replaced，结构可通过但错误 33.5 仍在。四份同来源完整报告对照区分全组均值、胜局均值和中位数；分析者修正版能走完重评→修订→最终复评的五调用路径，仅为工程证明。161 项相关测试通过，Runtime 实际组装/manifest 已校验；聚合证据 `golden_native_v36_offline_qualification_v2.json` 中 claim-scope/本次恢复/历史五例/原归因的五调用预约分别 380514/382552/400972/380036，均低于 401920；长历史重评输入 58972 < 63936。独立复核已通过新旧合同边界、manifest、7请求、68文件哈希及数值反例；公共 CI 以 PR7 对应提交的检查记录为准，不能用离线结果替代真实质量。

唯一下一步：为下一次有界开发资格批核对新候选准入与同 SHA 公共检查，本轮不再追加付费。下一次开发资格批先正确全文，再本次全称负例，优先核对评估自身数字/来源、问题映射和实际改文，随后补齐同版本范围/归因/历史五例；任一语义或执行失败停批。重新开放开发入口需同步 manifest 并再次取得该 SHA 公共检查；当前 native 为 offline_claim_scope_reassessment_contract，产品仍 offline。每报告 5 次/1 修订/401920 tokens/900 秒，GLM-5.3-flash/high、单次32768/300秒/输入63936、SDK retry0 均保持。
...
2026-09-19 native v3.2实现3a73231/Actions35431889592公共三项全部success后，完整归因正例实测1次、63.704秒完整stop，11562输入+4658输出=16220tokens，无未知用量，累计Provider至少240。模型80/needs_revision：按第4段局部中单语境将早期死亡句判fact_error，同时承认第14段已明确全样本2.5对2.67。实际请求保留两段和完整上下文标准；中单2.5对0.5、全样本2.5对2.6667均复算正确。这是范围解释误报，非断流、额度或资料遗漏。

按已采用完整上下文标准，第4段只把伤害明确限定为中单，早期死亡未明写中单；第14段为同指标的具体全样本限定，非泛泛免责声明。因此人工拒绝该评估，不改冻结正例标签。本批立即停止，未跑归因原错稿、修订/复评或原五例；不能说伤害漏检已修复，也不能撤销a71eb94五例历史通过。原回执/全文/来源哈希保存于golden_native_role_contrasts_result_3a73231.json。未证明新计算或提示分别导致本次范围选择。

native入口已设offline_role_scope_false_positive，产品runner继续offline；新增五份完整范围对照分别覆盖后文明确限定、前后明确一致、前文显式错误范围、错误分组数字、原伤害错误归因。旧控制集/失败不改。离线复现scripts/check_native_product_regression.py --scope-controls核对同一原始数据、完整25段及输入43860–43958<63936，标签不入模型；结果golden_native_scope_controls_offline_v1.json不是新模型质量。停用后32项runner/应用检查通过。

唯一下一步：完成完整报告范围裁决方法的离线审查：具体同指标限定可在前后文消歧；明确局部错误不能被另一组正确数字或免责声明冲掉；真实归因还须逐指标核验量级。保留原生问题清单与全部预算，先用上述完整对照和既有五例证明输入/修订合同，再决定新候选及其同SHA有界验证。v3.2本批不追加付费重试。方案/失败诊断见docs/plans/2026-09-19-role-attribution-review.md。

Stage8E仍in_progress。来源接线已修复但语义资格未取得；任务原子事务/API/Workbench实际消费、独立评估、学习覆盖、前端审美重做/头像、四块联动/专属Training及62主题均沿用restart plan。
...
2026-09-17 整篇typed审查合同离线接通：恢复提交e3f5fcc/Actions35215700732公共检查success；本次实现8bc7d4e/Actions35218637665公共三项也全部success。普通来源事实与比赛推断分开，均值/中位数明确运算及完整成员；错分审查类别可纠正但原文覆盖不可丢，旧问题须逐项保留或引用真实来源处置。完整原生评估直接进入修订，避免展开后的原句/解释重复传输。30项新增检查、161项相关回归通过；两份冻结整篇报告已作明确标记的分析者构造见证，脚本走通负例初评→重评→修订→两次复评，原回执未改。五阶段输入上界60884/63054/58104/60814/62766，均低于63936；五次全额上界合计469462高于总401920，继续按实际结算+下一请求预约准入，不保证任意输出或900秒完成。四场中单被换成五局的反例仍能结构通过，语义未修复；不能据离线成功恢复付费。唯一下一步：按来源合同修复计划第3项，离线比较“首步明确原句对象/范围/运算、次步核算并质疑”与当前同时审查；用完整正反例、错误首评和全请求预算判定方案，不再补字段后直接重跑。无新增Provider请求，累计至少201；四个失败入口仍offline_only，8E/high/全部预算及前端审美、四块联动和62主题后续保持。

- 本地代码：离线完整合同及五调用脚本路径完成，161项相关测试通过；语义流程仍待裁决。
- 所有者理解：已说明结构可用与语义可靠的区别，无新增用户理解验收。
- 参考来源审计：冻结报告、原始响应和失败回执哈希保持；人工构造见证不重标为模型成功。
- 公共作品/部署成熟度：本次实现8bc7d4e/Actions35218637665的pytest、postgres-migrations、packaging-smoke均success；仅工程验证，无生产准入。
...
- 本地代码：来源目录与全文矩阵离线完成，33项相关测试通过；整体评估接线和范围语义待完成。
- 所有者理解：说明资料可见、可引用与推断成立的区别；未代替用户理解验收。
- 参考来源审计：原始资料和回执未改，已纠正通用录像边界完全缺失的过宽判断。
- 公共作品/部署成熟度：9cd6020公共三项success；本次原型待公共检查，未准入生产。
...
- 本地代码：失败诊断已停用；完整只读审计暴露多个独立问题，12项针对性检查通过。
- 所有者理解：已区分原文可见与来源可引用、统计正确与语义正确；无新增理解验收。
- 参考来源审计：全部原响应/请求/回执字节哈希保留，组件审计不修复或重标历史结果。
- 公共作品/部署成熟度：e23c90d公共三项通过但真实诊断失败；本次停用/审计改动待公共检查，无生产准入。
...
- 本地代码：离线原文优先重评及有界单次诊断完成；仍不证明语义质量。
- 所有者理解：已说明首评意见隔离与复用旧首评的诊断边界，未代替用户验收理解。
- 参考来源审计：旧原文、响应、问题与失败均保留；尺寸投影不改历史结果。
- 公共作品/部署成熟度：恢复提交1f2bc62/Actions35074258939三项success；新实现待公共检查，无生产准入。

以下为历史记录，当前以最新canonical为准。

2026-09-16 独立provisional候选真实验证失败并停止本批：c3ff392/Actions35071182777公共三项success。正例两次完整stop（75.468秒/131.156秒），25511输入+23007输出=48518tokens，无未知用量，累计Provider至少200。新入口确实完成首评→完整重评，但最终table block13缺操作数引用、target block10缺跨段scope_source；逐项审查确认目标仍把四场中单换成五局混合样本，训练中位数/整体均值还被附上胜负比较绑定。仅离线补两个结构缺项即可pass，但错误解释未改，明确证明格式修复不解决语义；原响应及失败回执哈希不变。已将新入口也设offline_only，35项受影响检查通过，不运行负例/修订/复评，不自动局部补丁后付费重跑。当前下一行动：联合裁决原始事实、派生统计、旧模板判断、首评意见和最终语义的数据流；离线比较原文优先的独立重评，比较不向第二评灌输首评claim判断但保留全部原文义务/旧issue/审计的方案，联合检查范围继承、运算类型与全链预算；不再把结构可行等同方法有效。8E/high/硬上限与前端及62主题后续保持。

- 本地代码：失败候选入口停止并记录语义反例；进一步修正确定性来源模板的混分路能力/稳定性推断，128项上游及产品链路检查通过。RQ-246旧报告固定为与原manifest一致的快照，旧输入/指纹未改。
- 所有者理解：已说明两次完整返回仍未纠正样本对象；未代替用户验收理解。
- 参考来源审计：两次实际响应、回执与新离线投影分开；不重标失败，不修改冻结预期。
- 公共作品/部署成熟度：c3ff392公共三项通过但真实质量失败；33c03ce/Actions35072891451（停用与审计）及fcf0ac3/Actions35073525804（模板修复）均公共三项success；没有新的真实语义通过或生产准入。
...
- 本地代码：整体离线裁决及独立开发入口完成；可执行不等于模型语义正确。
- 所有者理解：说明首评可纠正与最终严格验收的分工，未代替用户验收理解。
- 参考来源审计：复用原字节回放和尺寸证据；历史失败不重标。
- 公共作品/部署成熟度：f034b01公共三项通过；新入口实现待同SHA公共检查，无生产准入。
...
- 本地代码：失败候选停用；临时首评离线纠正与最终严格校验分离，153项相关检查通过。
- 所有者理解：已说明首评临时性与最终验收的边界；未代替用户做理解验收。
- 参考来源审计：原始响应、输入及失败结果哈希不变；原字节仅离线构造第二步，无新增成功模型结果。
- 公共作品/部署成熟度：387b8ef公共三项通过但真实失败；最新离线修复尚待公共检查，未生产准入。
...
- 本地代码：比较绑定、完整重评及修订复评隔离接线完成，134项相关检查通过。
- 所有者理解：已解释程序核验成员与模型判断语义的分工，未代替用户做理解验收。
- 参考来源审计：真实回执保持原字节；正反例及历史投影只作离线合同与预算证据。
- 公共作品/部署成熟度：前轮ff13c1c/Actions35063171248三项success；本实现待公共验证，无生产或语义准入。
...
- 所有者理解：已区分执行约束与真正修复整体设计；无新增理解验收。
- 参考来源审计：真实首评仅离线原字节回放；原哈希及失败结果保持。
- 公共作品/部署成熟度：本次执行约束尚待公共CI；无新增真实质量验收或部署。

以下为历史记录，当前以最新canonical为准。

2026-09-16 v10真实失败后停止付费连跑：6bcaa86/Actions35049563440公共三项success。两次完整stop共42956返回tokens，无未知用量；累计Provider至少197，本批9轮22请求544606tokens。首评c004/c005/c006引用同一完整段落，最终禁止重复，而纠正禁止删项/缩句/换段，合法操作无法修复，实际以duplicate_context_claim失败；第二报告、修订、复评未开始。人工另确认c010仍把四场中单扩大为五局混合样本，第二次未纠正；标题复核说明多处错配。统一字段仅修表示冲突，不能称完整方法已奏效。只读可执行审计还发现48首评/16修改、64标题/16修改及issue容量不对齐。唯一下一步：离线完成纠正合同可达性裁决，联合证明原文覆盖、稳定目标身份、事实/安全问题保留、可纠正错误族及全请求预算；比较源单位预索引与同预算完整重评，先反例验证再选实现，不局部放宽重复规则后付费重跑。8E/完整上下文标准/high/硬上限/62主题及前端后续不变。详见ADR-0102最新结果节及golden_contextual_reachability_v10.json。

- 本地代码：统一协议698项黄金相关回归及公共三项通过，但真实候选已失败，未解决纠正操作缺口和语义错误。
- 所有者理解：已承认此前方法调整仍未核对首评错误与纠正能力是否匹配；无新增理解验收。
- 参考来源审计：两次原始响应和失败回执冻结；可执行审计验证不可修复重复组，不修改历史结果。
- 公共作品/部署成熟度：6bcaa86公开验证通过；模型质量不通过，未准入或部署。

以下为历史记录，当前以最新canonical为准。

2026-09-16 v10方法调整：013dbe1/Actions35047568110公共三项success；v9首评完整stop但JSON后有额外正文，1次请求26722返回tokens，无未知用量，累计Provider至少195，本批8轮20请求501650tokens。已核实实际Adapter发送json_object，但不发送服务端严格Schema；此前“普通文本输出”说法不准确。局部尾部恢复分支离线64912>63936，已撤下、未实测。当前统一首评与纠正的decision/来源/解释及共享规则，程序只展开重复字段；首评原文、全部问题和引用保留，不吞杂文、不增调用。九份历史首评及一次真实复评的合成尺寸测量56204–61508、首评45830/45902，均在限内；合成投影不改旧失败、不证明语义成功。唯一下一步：完成本实现回归及同SHA公共检查后一次有界两例实测，人工核对解释对象、事实问题及实际修订复评。若失败先裁决整个候选，不继续自动局部补丁连跑。完整上下文标准/high/所有上限/8E与62主题全局后续保持。证据见ADR-0102及golden_contextual_decision_readiness_v10.json。

- 本地代码：统一首评/纠正的单一判断协议及共享规则，原始评估留审计；698项黄金相关回归、编译、治理与差异检查通过，待公共验证。
- 所有者理解：已说明尾部恢复仍偏局部、旧首评持续制造字段冲突；无新增理解验收。
- 参考来源审计：v9原始失败冻结；十份尺寸投影明确非真实模型通过。
- 公共作品/部署成熟度：013dbe1公共三项通过；当前改造未实测、未准入部署。

以下为历史记录，当前以最新canonical为准。

2026-09-16 v9接续首次实际修订复评：68e8670/Actions35005362095公共三项success；v8两例均启动，七次stop共158888返回tokens，无未知用量，累计Provider至少194。本批七轮19请求共474928返回tokens。第一例有效96/pass但人工发现解释将四场中单扩大为五局混合样本，质量不通过。第二例有效80/needs_revision，检出未来外推并实际修订两句、完成两次复评调用；末次因两条新增OP.GG直接事实附scope_source而协议拒绝。v9验证并在journal保留direct附加来源边界，canonical仍direct/null范围，模糊或外推仍禁止该字段；新增保持同一分路/胜负组/样本对象的纠正要求。第二例未改字节离线复评95/pass，不算新真实全对通过。150项相关检查通过，八轮历史形状62994/59650/58322/58072/58512/57218/61474/57570及实际复评59660均在预算内。唯一下一步：v9同SHA公共检查后新身份完整两例，人工核对样本对象和复评结果；8E/完整上下文标准/high/上限与全局后续保持。证据见ADR-0102及golden_contextual_pair_68e8670_v9.json、golden_contextual_readiness_v9.json。

- 本地代码：两例初评有效并实际修订复评；direct补充来源接收与样本对象纠正已补，150项相关测试通过。
- 所有者理解：已区分自动通过与解释对象漂移；无新增理解验收。
- 参考来源审计：保留v8两例完整原始回执；第二例离线复评不替代真实成对验收。
- 公共作品/部署成熟度：68e8670公共三项通过；v9待公共检查与实测，未准入。
...
- 所有者理解：已区分待审片段与解释该片段的同段原文；无新增理解验收。
- 参考来源审计：v6原始失败与完整响应保留；v7离线原字节回放不改历史结果。
- 公共作品/部署成熟度：92aed17公共三项通过；v7尚无新实测或准入。

以下为历史记录，当前以最新canonical为准。

2026-09-16 v6单一判定纠正协议：c453088/Actions34999382329公共三项success，658项黄金回归通过；真实两次stop共49521返回tokens，无未知用量，累计Provider至少183。本批四轮8请求共209963返回tokens。原v5纠正漏填c010/c013四个必填字段，离线补全诊断后另发现c007/c011范围锚点不在对应上下文，未完成初评/第二报告/修订复评。v6改为扁平claim_updates：模型显式decision决定分类/状态/范围，scope_source保留模型选定原文，程序仅展开字段及定位样本标记，不能从reason猜缺失判定。省略quote_ref只保留同一旧原句，不能删错或换源；原始失败不改。显式人工补两项判定后的新协议投影93/pass只作离线诊断，不是模型成功。146项相关检查及五轮真实形状测量通过（62106/59028/57658/57410/57890），标准/high/调用及时间预算保持。唯一下一步：v6同SHA公共检查后新身份完整两例实测，人工核对解释归属和必要修订复评；8E与62主题全局后续保持。详见ADR-0102最新节及golden_contextual_wire_c453088_v6.json、golden_contextual_readiness_v6.json。

- 本地代码：单一显式判定协议及原文定位接线；146项相关检查通过，待公共检查。
- 所有者理解：已说明漏字段与范围引用矛盾，程序不猜判定；无新增理解验收。
- 参考来源审计：v5原始失败保留；人工投影不算实测或准入。
- 公共作品/部署成熟度：c453088公共三项通过；v6尚未实测或准入部署。

以下为历史记录，当前以最新canonical为准。

2026-09-16 v5修复算术/锚点误拒并补评估解释审查：769d651/Actions34997110778三项success，真实两次stop共53707返回tokens、无未知用量，累计Provider至少181。全部8条已反馈错误都有模型编辑，但完整校验仍因同位置胜负分组均值及“输局中”锚点识别不足而拒绝。v5仅允许完整被引用、实际纳入且同位置同胜负的原始行计算均值；胜负组锚点须在同一引用原文中另有明确样本限定。原始响应离线可结构通过94/pass，人工发现c004解释把混合组差值115.88/695.42说成四场中单，故语义仍拒绝；新增解释数值待核对诊断并要求修改claim.explanation，不能只改review_notes。138项相关检查通过；四轮历史纠正形状63912/60834/59464/59214均在63936预检限内，非未来保证。本批3轮6次真实返回160442 tokens，无未知。唯一下一步：v5同SHA公共检查后完整两例新身份实测，逐条核对解释中的分组与数值及修订复评；high/预算/完整上下文标准/8E和62主题后续保持。证据见ADR-0102最新节及golden_contextual_recovery_769d651_v5.json、golden_contextual_readiness_v5.json。

- 本地代码：v5接收修复与解释诊断已验证，待公共检查及新实测。
- 所有者理解：已区分程序误拒与评估解释算错；无新增理解验收。
- 参考来源审计：保留第三轮原始失败；结构离线通过未被当成语义通过。
- 公共作品/部署成熟度：769d651公共三项通过；v5未实测、未准入部署。

以下为历史记录，当前以最新canonical为准。

2026-09-16 修复真实复测暴露的诊断遮蔽：5645afc/Actions34994694662同SHA公共三项success；两次真实stop共47783返回tokens，无未知用量，累计Provider至少179。第二步仍未修复6条首评direct_result带范围字段的冲突，最终拒绝；第二报告、修订和复评仍未运行。根因是诊断只覆盖早期锚点/数值错误，未逐项反馈后续表示冲突。本补丁将当前候选的最终逐条校验与诊断共用claim_errors，以紧凑target/codebook矩阵优先保留全部错误；验证48条同时错误不会被12行/3000字反馈上限遮掉。旧错误响应仍被拒绝，人工投影仅证明修掉已列6项后无其他下游阻断，不算真实通过。124项相关回归通过；三轮实际历史形状均在预算内（63076/62242/58794）。唯一下一步为本补丁同SHA公共检查后，新身份两例完整实测并核对修订复评；标准/high/硬上限/8E和全局后续不变。证据见ADR-0102最新节、golden_contextual_diagnostics_5645afc.json及golden_contextual_readiness_v4_diagnostics.json。

- 本地代码：补齐共享逐项诊断与全部错误清单；124项相关检查通过，待公共检查。
- 所有者理解：已说明执行方漏查的诊断缺陷；无新增理解验收。
- 参考来源审计：两轮真实失败及原字节保留；人工投影不是模型改对的证据。
- 公共作品/部署成熟度：前一提交公共三项通过，当前补丁未实测、未准入部署。

以下为历史记录，当前以最新canonical为准。

2026-09-16 接续真实回执后的v4修复：ad1003d/Actions34992614360同SHA公共三项success。v3真实两次完整stop，25829输入+33123输出=58952返回tokens，无未知用量；累计Provider至少177。第二请求已实际发出，输入预算阻断修复生效；首份报告因22项复核之外又给3个新增claim附重复说明而触发inventory_mismatch，另有“各只1局”漏原文空格，未执行修订或第二份报告。v4只允许补充说明绑定实际新增claim，锚点仅作原文唯一的显示空白恢复，保留完整审计；不能改变数字/文字/对象/范围或吞掉缺项、未知ID。用原始响应直接离线回放可通过93分，原v3失败及字节保持，不等于新真实通过。111项相关测试通过；最新请求形状62348，其他测量均在限内，high及所有调用/时间/Token上限不变。唯一下一步：v4同SHA公共检查后，以新身份完整执行两例及必要修订复评，人工核对真实判断；8E仍in_progress，整体后续保持。详见ADR-0102的2026-09-16节及golden_contextual_recovery_ad1003d_v4.json。

- 本地代码：v3来源/可变性/预算/修订标准修复已实测到纠正响应；v4完成引用格式接收修补及离线验证。
- 所有者理解：已区分实际模型判断与格式阻断，未新增理解验收。
- 参考来源审计：两份真实原始响应和失败回执保留，新增说明与空白恢复可追溯；未修改历史标签。
- 公共作品/部署成熟度：v3公共检查通过，v4待公共检查及新真实验证；未准入部署。

以下为历史记录，当前以最新canonical为准。

2026-09-15 大复盘后接续最新候选：206a4c3/Actions34959615129公共三项已通过；其后真实首评完整返回25333 tokens，但纠正请求在本地输入64300被拒，无修订复评。累计Provider至少175，补记此前状态遗漏的一次。当前v3修复首评direct_result冻结、OP.GG来源不可引用、请求重复以及修订继承旧严格标准的问题；保持用户已确认的完整上下文标准。最新历史形状输入63124、首评47540/47612、其他纠正56860–59198、修订50540，全部是离线测量，未证明模型质量。1.3.25只作回归基线，不折返开发；8E仍in_progress。唯一下一步：完成本补丁工程检查及同SHA公共检查后，使用独立新身份执行冻结两例完整初评→必要修订→复评并逐条人工核对。详见ADR-0102的Recovery节及golden_contextual_readiness_v3.json。

- 本地代码：来源、全部判断纠正、无损表格和统一修订标准已接线；642项黄金切片相关回归、编译、入口预检及治理检查通过。
- 所有者理解：已说明旧基线与当前候选的区别；未新增理解验收。
- 参考来源审计：复用完整冻结输入，原响应逐字节不变；本补丁尚未新增真实请求。
- 公共作品/部署成熟度：v3尚待公共CI和有界实测，不代表生产准入。既有发布链和观摩记录保存成果保留。

全局后续按本次大复盘62主题总表及前端专项附件追踪；包含四块关联、百科式Coach、训练计划、前端整体审美升级/必要重做及英雄头像、Portal/Account、数据/版本/分路、部署认证和学习证据。当前修复不吞并这些后续内容，详见ADR-0102。
...
- 本地代码：完整上下文独立候选接线，129项相关测试、编译、治理和差异检查通过。
- 所有者理解：用户已确认上下文语义标准，无新增理解验收。
- 参考来源审计：冻结报告按新标准独立标识，旧报告、标签、响应不改写。
- 公共作品/部署成熟度：待本实现公共检查和真实观察，无生产准入。

以下为历史记录，当前以最新canonical为准。


2026-09-15效率修正：原始统计改为同请求内独立不可信原文，逐字符保留；十份首评50282–50456、两份历史状态纠正63474/60410，单请求准备门通过。五次最大预留和仍445284>401920，但此前将其当作强制实测门槛是执行方加设条件；现按既有执行器的实际用量结算和下一次请求预约裁决，不改估算器、任何上限或high。已接独立有界纠正五调用入口与初评/复评纠正日志，122项相关验证通过；只证明工程路径，不称语义修好。唯一下一步为本实现同SHA公共检查后一次冻结显式定义正反例完整报告观察，失败停批、原始结果保留，逐条人工复核；每报告5调用/一次修订/401920tokens/900秒，单调用32768/300秒，零SDK重试。原报告1.3.25闭环有效，8E仍in_progress，Workbench四块设计后置；本批尚无新Provider调用，累计至少172。详见ADR-0102最新节及golden_bounded_review_readiness_v1.json。

- 本地代码：独立有界纠正工作流与原文传输已接线，预算执行器保持原合同。
- 所有者理解：纠正预留估计与实际结算的混淆，无新增理解验收。
- 参考来源审计：冻结完整报告与旧响应仅离线复用，未改标签或原字节。
- 公共作品/部署成熟度：本批待同SHA检查与有界开发观察，无生产准入。
...
- 本地代码：离线状态合并及请求构造完成；当前拼装预算不通过，未注册或接入Runtime。
- 所有者理解：保留源引用与语义关系的区别，无新增理解验收。
- 参考来源审计：十份冻结完整报告及两份原始响应仅离线复用，历史字节不变。
- 公共作品/部署成熟度：实现56a5467/Actions34954808947同SHA三项success，无生产准入或部署。
...
- 本地代码：诊断缺陷已修；整篇候选保持否决，新纠正流程仅进入离线验证。
- 所有者理解：区分传输、协议、语义及澄清标准；无新增理解验收。
- 参考来源审计：回放旧原始响应且逐字节确认未改，未新增外部来源或真实调用。
- 公共作品/部署成熟度：诊断实现abcfc0242b30798d9a239338a1a043dd6ae6ef11/Actions34951075015同SHA公共三项success；无语义验收、部署或准入。
...
- 本地代码：真实目标预算已修复；整篇候选因协议和语义失败不采用，待离线流程重划。
- 所有者理解：已明确预算修复、完整返回与可靠评估是不同结论；无新增理解验收。
- 参考来源审计：保存两次新批全部请求、回执、错位证据与独立人工审计；原标签不改。
- 公共作品/部署成熟度：实现153ae2c公共三项通过，不代表语义验收；未准入或部署。
...
- 本地代码：真实目标预算保护已修复并离线核验，待新身份实测。
- 所有者理解：已解释整段估算与模型实际拆分的区别，无新增理解验收。
- 参考来源审计：完整保存57条发现与原始回执，原标签不变。
- 公共作品/部署成熟度：v1公共三项通过；v2待同SHA检查，无准入或部署。
...
- 本地代码：整篇两步原型、回执绑定与五调用控制流已实现；未切换产品默认。
- 所有者理解：已解释两步分工、预算与语义验证的区别；无新增理解验收。
- 参考来源审计：使用冻结十份完整报告及完整原始数据，旧标签/回执不修改。
- 公共作品/部署成熟度：本实现待同SHA公共检查；未部署或准入。
...
- 本地代码：probe-v2已公共验证及真实观察；目标身份绑定离线修复与43项相关测试通过，尚未接入Runtime。
- 所有者理解：明确引用位置正确不证明定义关系正确，无新增理解验收。
- 参考来源审计：沿用ShowMaker五局及冻结四例完整报告，保存逐例真实证据和独立审计。
- 公共作品/部署成熟度：身份绑定实现753267a/Actions34943310239同SHA公共三项success；本次仅文档收口，无生产准入或部署。
...
- 本地代码：独立候选接线和97项相关验证完成；真实语境质量未验证。
- 所有者理解：已解释原句与上下文分离、引用校验与语义判断区别；无新增理解验收。
- 参考来源审计：仅使用既有ShowMaker五局、已修订报告及历史响应；新十例完整语境由执行方审核，非held-out。
- 公共作品/部署成熟度：本实现待同SHA公共验证，无新生产准入或部署。

以下为历史记录。

2026-09-15扩大复盘后的离线实验完成：28份历史响应中23份可无损索引化，全部13份原有效结果保持不变，原无效结果未被放行；同一可转换子集紧凑字符减少33.8%，非tokens/时延结论。选择完整上下文＋段落编号/唯一短片段，否决整段或简单分句作为唯一表示。引用补丁仅允许诊断位置并全量复验，可明确要求重审；实际完整补丁请求53844、原整篇纠正58806，均<64000。原十二例不改标签，新增五组十条语境开发对照，目标判断和整篇结果分开。未来对照回执新增started/finalized/interrupted/not_started，旧回执不动。详细证据和边界见docs/adr/0102-evaluation-reference-protocol-experiment.md。下一步按ADR-0102接入独立候选的原文引用、显式上下文关联与有界纠正，完成实际请求预算和公共验证后优先定义/标题成对实测；不原样续跑v8。本批未注册新Runtime、无新增真实Provider请求（累计至少155不变），不称语义修复或准入。GLM high/32768/300秒、ShowMaker observed及既有Workbench人工稿保持，四块设计后置，8E仍in_progress。

- 本地代码：离线原型、语境开发对照和计数修复完成，66项聚焦/相邻测试及编译/治理通过；实现8d3857251e147ff2ce4f13efd9275a9db3aa7653已由Actions34924328754同SHA公共验证，postgres-migrations、packaging-smoke及pytest三项均success；仅证明本批工程检查通过，不代表模型语义质量通过。
- 所有者理解：已完成扩大范围复盘并获继续指令；没有将执行方方案或新标签写成用户逐项确认。
- 参考来源审计：仅重用28份历史响应及原五局事实，无新来源；Luna执行子任务因模型不可用未产出，由主Agent复核。
- 公共作品/部署成熟度：本批实现8d38572公共三项验证通过；未接入新候选、无真实语义质量通过或部署。
...
- 本地代码：1.3.26及84项回归已公开验证；本轮真实重验未完成，流程可靠性未验收。
- 所有者理解：已回应连续失败，明确整篇重审纠正的错误传播与协议/语义分离；无新增理解验收。
- 参考来源审计：保存新11请求及逐条人工审计；无新外部事实来源。
- 公共作品/部署成熟度：c8450dc同SHA三项success；无真实全套通过、生产准入或部署。
...
- 本地代码：1.3.26候选和84项相关回归完成；新真实对照待运行。
- 所有者理解：已说明显式标题审查、原文锚点与诊断定位原则，无新增理解验收。
- 参考来源审计：沿用冻结数据和18份旧响应，无新外部来源。
- 公共作品/部署成熟度：本实现待同SHA公共检查，无生产准入或部署。
...
- 本地代码：十二例真实验证完成但质量未通过；候选及评测入口已公共验证，下一步为具体失败修复。
- 所有者理解：已解释传输完整、结构有效、目标语义正确三层区别，无新增理解验收。
- 参考来源审计：沿用冻结十二例及ShowMaker五局；逐行算术和目标判定人工核验完成，无新外部来源。
- 公共作品/部署成熟度：d8576be同SHA三项success；真实开发集未通过，无生产准入或部署。
...
- 本地代码：对照入口与28项回归通过；十二例真实验证待执行。
- 所有者理解：已说明单例成功与对照质量、invalid与误报漏检分母区别，无新增理解验收。
- 参考来源审计：冻结十二例和同一ShowMaker五局，无新外部来源。
- 公共作品/部署成熟度：原实现已公共验证，本次入口待同SHA公共检查；未部署或准入。
...
- 本地代码：候选1.3.25及142项相关回归通过；原报告真实自动初评/修订/复评已验收。
- 所有者理解：已解释本次三处修订、有效复评与独立质量准入区别，无新增理解验收。
- 参考来源审计：沿用ShowMaker五局；人工逐行复算与新增对照数字核对完成，无新外部来源。
- 公共作品/部署成熟度：实现c69cb3e三项公共通过；本次为私有开发集证据，无部署或生产准入。
...
- 本地代码：具体数字/来源/原文反馈已通过回归；真实闭环仍待验证。
- 所有者理解：已解释抽象错误码无法有效指导纠正，无新增理解验收。
- 参考来源审计：同一ShowMaker五局和原报告，无新来源。
- 公共作品/部署成熟度：37c65b9公共三项通过，新实现待公共；无部署或准入。
...
- 本地代码：分类提示、重复字段定位和修订阶段输入缩减已通过回归；新版本真实闭环待验证。
- 所有者理解：已解释本次为校验失败及后续输入超限，无新增理解验收。
- 参考来源审计：同一ShowMaker五局和原报告，无新来源。
- 公共作品/部署成熟度：6adc473三项公共通过，新实现待公共；无部署或准入。
...
- 本地代码：单局范围修复及真实修订输入大小验证通过；真实闭环仍待运行。
- 所有者理解：已解释局部别名误拒，无新增理解验收。
- 参考来源审计：同一ShowMaker五局和原报告，无新来源。
...
- 所有者理解：已解释真实语义检出、canonical接受和完整修订区别，无新增理解验收。
- 参考来源审计：仍为同一ShowMaker五局及原报告，无新来源。
- 公共作品/部署成熟度：fd0bdff三项公共通过，新实现待公共；未部署或准入。
...
- 本地代码：独立单次容量接线与聚焦检查通过；真实闭环未验收。
- 所有者理解：已解释单请求容量和整批预算区别，无新增理解验收。
- 参考来源审计：沿用ShowMaker五局及既有官方128K最大输出证据，无新来源。
- 公共作品/部署成熟度：db9567e公共三项通过，新实现待同SHA公共；未部署。
...
- 本地代码：两个实证缺陷已修，离线回归通过；新版本真实语义闭环待验证。
- 所有者理解：已解释完整返回与语义/结构校验的区别，无新增理解验收。
- 参考来源审计：复用ShowMaker五局及原报告，无新来源。
- 公共作品/部署成熟度：上一实现三项公共通过，本实现待同SHA检查；无部署或自动报告准入。
...
- 本地代码：程序侧数值核对及较小评估输出已接线通过；真实闭环待验证。
- 所有者理解：已解释计算支持与语义归属边界，无新增理解验收。
- 参考来源审计：沿用ShowMaker五局及96分稿，无新来源。
- 公共作品/部署成熟度：本实现待同SHA公共检查，无部署或自动报告准入。
...
- 唯一下一步：`8e-productization / candidate-real-golden-slice / in-progress / offline-hardening-and-live-consumption`。先完成临时首评合同的整体准入裁决（已知错误族、原文与问题保护、完整五调用及预算），确认新实现公共检查；若合格再以独立身份验证冻结两例，逐例人工审查。当前两条失败入口均offline_only，不重跑已失败批次。
