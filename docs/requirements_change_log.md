...
| RQ-058 | 2026-08-18 | 执行中 | 用户明确“继续下一步”，解除 `6A-6-security-lifecycle-nfr` 的等待确认 | 只实施已冻结的 task 基座安全、生命周期、背压、结构化可观测性与性能基线：默认关闭 CORS、日志/Secret 脱敏、7/90/30 天 retention、terminal delete 与安全补偿、active delete conflict、owner/global capacity 和 warm-DB/claim benchmark；先红灯测试，再最小实现，使用注入时钟与真实 PostgreSQL CI。不得实现正式 Auth/HTTPS、Session/Memory、SSE、前端、lease/heartbeat/reclaim/cancel/resume、真实 Provider/Riot I/O 或进入 6A-7。 |
