...
| RQ-059 | 2026-08-18 | 执行中 | 用户再次明确“继续吧”，授权 canonical 的 `6A-7-packaging-exit-review` | 只实现可在本地/CI 重建的 FastAPI API + PostgreSQL + 独立 polling Worker packaging、配置/启动命令、无真实 Key/I/O 的 Linux smoke，以及逐项 6A exit matrix/review；先冻结红灯合同，再做最小组合。正式 Auth/HTTPS、Session/Memory、SSE、前端、lease/heartbeat/reclaim/cancel/resume、直接公网部署、LangGraph/Multi-Agent/MCP/新 SDK 和 CI 真实 Riot/Provider 调用均不在本轮。exact-SHA 公共 CI 成功前不得关闭 6A 或交接后续阶段 6 检查点。 |
