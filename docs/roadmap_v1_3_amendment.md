...
2026-09-15 大复盘后恢复：8E顺序不变；本批修复来源索引、纠正可变性、无损请求和修订标准一致性。真实语义与准入未完成；后续四块交互和整体前端升级保留。当前以canonical及ADR-0102 Recovery节为准。
...
RQ-258（2026-09-10）：用户授权持续到真实黄金切片，不改变主阶段顺序。当前新增真实入口尚未验收，四来源投影不等于完整 Coach/训练/live UI；按 `docs/plans/2026-09-10-real-golden-slice.md` 推进，生产默认不变，8E仍in_progress。
...
实现纠正已补齐 RQ-235 身份排重和检索策略绑定，并重新生成开启检索加固路径的预算证明；当前仍等待
修复提交的 exact-SHA 公共 CI。

RQ-237 真实领域观察已在首案 `evidence_required` 后停止；实际查询未留存，白名单缺口仅为假设；未观察到 Provider
故障。白名单修复只在候选离线控制面生效，旧回执不可重跑。

RQ-238 在同一 8E Advanced 边界内固定查询指引的 Context 身份，避免开发探针和候选入口使用不同接线；不把开发样本成功升级为 8-Core 或生产成熟度。

RQ-241（2026-09-06）补全工具错误到 Harness/开发回执的固定安全码传递，并完成同 SHA CI 后的一次生存调整开发观察（2 来源、96 分）。下一步经济调整开发场景和正式接入条件汇总；不改变 v1.3 的 8-Core/8-Advanced 或 8F 退出要求，不重跑旧考卷。详见 [计划](plans/2026-09-06-guided-domain-candidate-assets.md)。

RQ-242 取代“待经济观察”：该场景 3 来源但修订失败；诊断补全 117 测试及 `a64331b` / CI `34022245397` 三任务通过。下一项是候选生成/修订合同对齐的离线设计与回归。开发成功与失败分别保留，不作为 8-Core/8F 或生产完成证据；产品接入差异见同一计划，质量门和既定阶段不变。
