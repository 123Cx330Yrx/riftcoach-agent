...
离线合同比较证据已更新为golden_native_contract_options_offline_v2.json：A的原句/上下文锚点仍可承载真实误报，只有审计定位价值；B在既有修订槽内逐项裁决并交完整报告，保留原首评与独立复评。分析者见证仅改block6错误尾句，正确前半句与其余全文逐字保留，五调用保守预约388070。历史五调用见证403790超401920；实际预算器在满额合成usage下第5次前停止，低usage则能5次，不能把全路径预约和当新增硬门或断言历史必然失败。B的错误撤销/未改真错等结构反例均保留semantic_approval=false；未改变首评模型，不能宣称已修正首评误报或绕过开发guard。产品生成占用与下一候选裁决见2026-09-20计划最新节；本批不再付费。实际两状态重评、同版本剩余范围/归因/历史五例、产品来源绑定/Evidence/真实事务/API/Workbench及独立评估/学习仍待完成。前端整体审美/重做、英雄头像、四块联动、专属Training与62主题继续保留。

继续验证产品共享预算：真实typed compiler/本地RAG/Runtime搭配脚本响应，生成两次→首评→编辑→终评共5调用、保守合成用量365600，可完成；多一次首评重评则第6次终评被调用预算拒绝、不发布。仅测试实例注入B，不是新的产品注册、观摩身份/外部Evidence验收或模型质量证明。相关72测试通过。已准备混合真假/纯真错/纯误报三组完整编辑诊断输入，来源和请求hash可重建、预期不入模型；独立审查通过，仍offline_unregistered。唯一下一步：基于冻结诊断计划准备独立回执执行边界并完成同SHA检查，再裁决是否进入单独有界编辑实测；旧失败批不恢复、首评准确率未修复、产品仍不准入。
