...
current_checkpoint: "8e-productization / candidate-explicit-zhipu-neutral-stream-adapter-seam / candidate-report-contract-alignment / completed-local / pending-public-ci"
status: in_progress
pause_reason: ""
---
...
- 当前短摘要（2026-09-06，RQ-243，取代下方 RQ-242 待对齐动作）：候选初稿/修订共用既有八标题合同；新开发计划 2.0.0、case/run/snapshot 与合同 SHA 独立绑定，旧路径不变。13 项新合同测试及相邻回归共 144 项、16 subtests 通过，待本实现公共 CI。本批真实 API 为 0，不能把脚本修订成功写成真实经济场景已修好。后续为新合同完整预算证明与产品迁移版本对齐；默认模型、GLM-5.2、前端、质量门与 production_media=0 不变，8E 仍 in_progress。
...
- 唯一下一步：`8e-productization / candidate-explicit-zhipu-neutral-stream-adapter-seam / candidate-report-contract-alignment / completed-local / pending-public-ci`。RQ-243 候选生成/修订合同已完成本地对齐及 144 项测试；先取得本实现公共 CI，再推进新合同完整预算证明与产品迁移版本对齐。新合同未发真实请求；RQ-242 经济失败仍保留，不重考旧资产、不改默认模型/前端/GLM-5.2，详见 `docs/plans/2026-09-06-candidate-generation-revision-contract-alignment.md`。
