...
2026-09-25最新工程阻断：8117369e / CI36119731919由平台明确判定超过15分钟，pytest取消；数据库与打包成功，不能称三项全绿。旧成功job已耗13分35秒，新coarse20项本机139.75秒（完整15回放53.66秒）。现去除测试中未使用的重复准备，把完整CI job容量调至25分钟并保留全部检查/增加耗时报告。相关验证和同HEAD公共CI完成后才启动原15真实批；截至本次修改无新Provider调用，业务模型/时间/token预算与质量门不变。
