...
- 唯一下一步：`8e-productization / candidate-real-golden-slice / in-progress / offline-hardening-and-live-consumption`。完成1.5.2离线/独立审查、冻结同版本原15前三项资格批并通过同提交公共CI，执行原正确泛指稿→明确全称错误稿→归因错误稿；最多12调用/997376tokens/2400秒、保守全按GLM价15.843328元。每个真实意见与改文均按完整上下文审查，首个失败停止，旧输入/标签/失败不改；具体次序、预算及后续依赖见活动工作卡。
