...
- 本地代码：显式V2流桥与mixed工具回合修复完成公共及真实兼容观察，部分输出不交付，生产默认不变。
- 所有者理解：已区分早期有输出、完整终态、工具往返和报告质量；不以字符估算未知token。
- 参考来源审计：主Agent与Luna核对Trace/逐请求回执，无隐藏重试；缺少事件间隔不能断言一直持续输出或判网络根因。
- 公共作品/部署成熟度：8a72e2b公共代码通过；真实失败记录待提交，无部署。
