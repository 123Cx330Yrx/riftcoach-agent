...
离线合同比较已完成，证据为golden_native_contract_options_offline_v1.json：A的原句/上下文锚点仍可承载真实误报，只有审计定位价值；B在既有修订槽内保留全文、问题清单和独立复评，正常五调用保守预约387920，但最长历史见证403790超401920。B的错误撤销/未改真错等结构反例均保留semantic_approval=false。下一步不是提额或加第六调用，而是把产品前两次生成与历史最坏输入纳入同一预算门，形成新候选并取得同SHA公共检查；本批不再付费。已完成两例仅为开发证据；实际两状态重评、同版本剩余范围/归因/历史五例、产品来源绑定/Evidence/真实事务/API/Workbench及独立评估/学习仍待完成。前端整体审美/重做、英雄头像、四块联动、专属Training与62主题继续保留。

以下为历史记录，当前以上述中断恢复及本批结果为准。
