...
current_checkpoint: "8e-productization / candidate-explicit-zhipu-neutral-stream-adapter-seam / candidate-guided-domain-assets-v4 / completed-failure-attribution-diagnostics / pending-fresh-observation-decision"
status: in_progress
pause_reason: ""
---
...
- 唯一下一步：`8e-productization / candidate-explicit-zhipu-neutral-stream-adapter-seam / candidate-guided-domain-assets-v4 / completed-failure-attribution-diagnostics / pending-fresh-observation-decision`。V4 资产和运行器已取得同一提交 `b71dfa1` 的公共 CI `34014412211` 三项全绿；随后绑定该提交完成新鲜 G53-3-L：3 次真实调用、协议 admitted、1096 tokens、11750ms。四案例领域观察回执为 `data/evaluation/results/provider_capabilities/zhipu_glm53_flash_guided_domain_v4_rq239_v1.json`，评测层 fail-closed，未形成领域或生产准入。此前公共 CI `34014981605` 的失败已定位为新协议回执缺少公共版本化契约分流；提交 `b7ac850` 增加严格 body-free 回执模型并接入总检，公共 CI `34015592274` 三项全绿。此前“空正文是已确认根因”的说法证据不足；本批补充候选评测失败的脱敏 `failure_code` 通道，保留稳定的 `evaluation_failed` 终态并让后续新鲜观察能区分结构化输出、超时或提供方错误。提交 `f2a7470` 的 exact-SHA 公共 CI `34017153819` 三项全绿。产品工具合同、质量门和旧回执不变。下一步是基于新诊断能力裁决是否另立一次新鲜观察，不降低质量门、不重跑旧考卷。
