...
- 当前子阶段组：5D Python 受限 Agent Loop，entry design 与 5D-1 至 5D-6b 已完成；
  5D-7 Batch A-C 与 Batch D 的 D1、D2、D3 已完成，held-out 只完成创建与生命周期校验，
  尚未运行真实 Provider 或 held-out
- 唯一下一步：继续 5D-7 Batch D 的 D4 候选 Provider 采用门设计；先用新 ADR 固定同任务
  比较、能力前置条件、失败分类、调用/成本上限与停止规则，本轮不自动调用真实 Provider，
  不接入第二 Provider，不运行 held-out
...
| 5D-7 Prompt/Context & Domain E2E Evaluation | 工具选择、事实/引用、注入、质量/成本/延迟评测 | 进行中（Batch A-C 与 Batch D 的 D1-D3 已完成） | Batch A：分层合同与 10 个记录型 development 控制样本；Batch B：组件/案例双层 SHA-256 快照和零调用 admission；Batch C：ADR-0015、7 个 `offline_executable` development 场景及一个真实 unsafe publication；D1-D2：`coach_evaluation@1.1.0` 安全合同、不可修订 blocking policy、7 场 secure offline development 基线，task/failure accuracy 均 1.0、unsafe publication 0、external calls 0；D3：3 场独立 held-out 创建，`calibration_excluded=true`，尚未运行；D4 Provider 采用门仍待设计 |
...
| 进度线 | 当前事实 | 不能混淆为 |
|---|---|---|
| 本地代码 | 阶段 0-4 已形成 V1；阶段 5 完成 5A、5B、5C、5D entry design 与 5D-1 至 5D-6b；5D-7 Batch A-C 已建立 executable development 基线，D1-D2 已迁移并验证 1.1 安全评测控制流，D3 已建立隔离 held-out 文件 | 阶段 5、整个 5D、真实 Provider 领域质量、held-out 运行、第二 Provider 或报告质量准入已完成 |
| 项目理解 | 已区分旧 1.0 与安全 1.1 评测合同、Evaluator 输入证据、不可修订 security blocking policy、development 与 held-out 的防污染生命周期；理解 D3 创建不等于运行 | 7 个离线安全样本通过就等于真实模型抗注入、held-out 已泛化或安全性已获得生产保证 |
| 参考资料 | EchoMind、AGI-Saber、Sea/OpenResearch 已做源码/文档审计并建立选择性映射 | 已经接入或复用了这些项目 |
| GitHub/部署 | 本轮 D1-D3 尚未提交；上一公开基线仍为 Batch C，正式网页仍未部署 | 本地 secure 基线或 held-out 文件已公开等于真实领域能力、最终厂商选型或 Web Agent 可用 |
...
唯一下一步是 5D-7 Batch D 的 D4 候选 Provider 采用门设计：先写新 ADR，固定同任务
比较合同、能力/错误归因、预算和停止规则；不在本轮自动调用真实 Provider、不接入第二
