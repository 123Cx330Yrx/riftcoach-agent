...
- 唯一下一步：`8e-productization / candidate-explicit-zhipu-neutral-stream-adapter-seam / candidate-guided-domain-assets-v4 / completed-failure-attribution-diagnostics / pending-fresh-observation-decision`。旧 V4 绑定 `b71dfa1` / CI `34014412211`，协议 3 次/1096 tokens/11750ms 通过，但四案均执行后在评测层拒绝，无评分，不是后三案跳过。旧回执保持不变，具体触发原因未知。“三次调用上限”和“空正文已确认根因”均撤回。RQ-241 完成 ToolResult→ToolError→Harness→Manifest→开发回执的安全错误码传递，移除无效空正文放宽；新增错误链路测试修复前 3 项失败、修复后通过，相邻共 107 passed、19 subtests passed。`f2a7470` / CI `34017153819` 及文档 `c868c4c` / CI `34017467728` 均三项全绿，本批新增修复等待公共 CI。下一步已明确为 CI 后执行一次已有匿名 survival_adjustment 开发观察，具体边界见 RQ-239 计划；不创建新考卷、不重跑 V4、不修改默认模型或前端。共享 Harness 仅增加诊断，85 分、来源/事实/引用/注入门与 GLM-5.2 回退不变。
