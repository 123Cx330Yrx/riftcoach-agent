...
- 未完成边界：当前 executor 已在显式 evidence-bound 运行中校验 publication manifest，并构造 `PendingEvidenceBundleSnapshot`、publication reference 和摘要；TerminalTurnWriter 能从持久绑定及报告工件重建 `TerminalAssistantTurn` 并幂等写入，Worker 每轮以有界批量调度。B/C 代码闭环完成，仍需公共验证；消息投影失败时保持 pending，绝不假完成。
