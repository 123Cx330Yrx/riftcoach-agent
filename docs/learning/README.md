...
| 阶段 8：Multi-Agent、可靠运行时与产品化 entry design | 准备态/等待授权 | [阶段 8 路线](../roadmap.md) / [双轨修订](../roadmap_v1_3_amendment.md) / [能力矩阵](../architecture_capability_matrix.md) | 当前只有 `stage-8-multi-agent-reliable-runtime-productization-entry-design` 的 canonical 准备态；8-Core/8-Advanced 尚未教学、设计或实现 |
...
- 6B-9 与 Session/Memory V1 已完成公共 PostgreSQL/package 闭环；这仍不表示正式 Auth/RSO、备份副本擦除、
  ...
  patch/freshness 或 Riot+OP.GG 数据融合；Stage 8 当前只处于 entry-design 等待授权状态。
