...
| 8E：Productization | 进行中/coverage planned | [preflight 计划](../plans/2026-08-23-8e-productization-preflight.md) / [Batch B 玩家档案与显式路由](8e-player-profile-selection-explicit-routing-walkthrough.md) / [Batch C Evidence/Product API](8e-evidence-product-api-walkthrough.md) / [ADR-0060](../adr/0060-adopt-postgresql-evidence-snapshots-and-cursor-sse.md) | OP.GG live/public 链与 Batch B `e844bdd/32622696087` 已公共闭环；Batch C 已本地实现 append-only Evidence revision、query-time expiry、四态 DTO 与 cursor SSE，正在完成本地/公共关闭门。整个 coverage 仍 planned；Batch C exact-SHA 公共闭环前不进入 Batch D 前端 |
