...
| 8E：Productization preflight | 进行中/coverage planned | [preflight 计划](../plans/2026-08-23-8e-productization-preflight.md) / [ADR-0056](../adr/0056-adopt-bounded-external-validation-and-player-profile-selection.md) / [ADR-0057](../adr/0057-record-body-free-opgg-schema-drift-diagnostics.md) | RQ-086 已授权；`DK ShowMaker#KR1 / asia / observed` 的 Riot Account/Match gate 通过，真实 OP.GG `mid` replay 被严格 adapter 以 `opgg_meta_result_invalid` 拒绝；adapter 已有 body-free schema-drift diagnostic 与受控 fixture，`c5cbc94` / Actions `32613573022` 三 job 全绿，但 live 字段级诊断、玩家档案选择、legacy 地区审计和前端小批尚未完成 |
