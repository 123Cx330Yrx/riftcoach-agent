...
2026-09-15 大复盘后恢复：当前8E继续最新上下文候选的工程修复、公共验证和有界实测；1.3.25仅历史回归基线。前端整体升级等后续按大复盘追踪。当前事实及唯一下一步见canonical，实施裁决见ADR-0102 Recovery节。
...
2026-09-15接续复盘：修复assessment诊断遮蔽并离线否决未经验证的整篇审查＋范围审查直接接线（测量五调用包络424928>401920、无额外纠正槽）。来源引用正确仍不证明定义关系；不称语义修复或改旧标签。下一步按ADR-0102最新节离线验证保留首评状态的有界语义纠正、完整问题合并与预算，8E仍in_progress；Workbench四块设计后置，Provider至少172且本次无新增。
...
2026-09-15最新执行调整：1.3.26真实重验启动7例、完成6例（5有效正确/1格式无效），第七例纠正300秒超时，后5例未启动；标题语义修复未验证。停止原样追加提示并重跑全套，下一步为[评估流程与局部纠正离线复核](plans/2026-09-15-evaluation-workflow-reassessment.md)。高档及现有容量保持，8E未完成，无准入；下方继续全套等旧下一步均为历史。

2026-09-15后续修复：1.3.26新增逐标题判别、含混原文锚点说明及独立全局/局部错误诊断，84项相关测试通过；尚待公共检查及新身份十二例真实重验。上一批12例失败结果仍有效，不将离线修复当作语义通过。8E保持in_progress，high/32768及既有预算保持。详见[当前执行状态](project_execution_state.md)和[范围对照计划](plans/2026-09-15-golden-scope-controls.md)。

2026-09-15最新对照结果：十二例真实验证已完成但未通过（8有效、4无效；4正例通过、3有效反例检出、1含混标题有效漏放）。18次完整返回共476603 tokens；无本批断流或额度耗尽，不能继续用容量解释质量问题。原ShowMaker报告自动修订闭环保持有效；下一步修复标题断言、含混锚点/邻段范围及纠正诊断，8E仍在进行、无生产准入。详见[十二条对照验收](plans/2026-09-15-golden-scope-controls.md)及[当前执行状态](project_execution_state.md)。以下准备状态和更早下一步为历史。

2026-09-15状态对齐：原ShowMaker报告自动初评/修订/复评已由Coach1.3.25真实验证（有效96/pass，人工核对三处修改及正确数字）；当前进入冻结十二条独立开发对照的真实验证准备。主阶段仍8E，未进入8F；后文“原报告漏检修复未验证”及旧唯一下一步均为历史，最新事实以[当前执行状态](project_execution_state.md)为准。
...
## 当前 RQ-258（2026-09-10）

A/B/C 已公共闭环，用户已授权连续推进至真实黄金切片。当前为 8E 内真实入口/报告/训练/Workbench 联调，尚未完成；以 canonical 状态和 `docs/plans/2026-09-10-real-golden-slice.md` 为准，下方等待授权/真实API=0均为各历史批次事实。生产默认不变。

RQ-258 同日补充：候选入口本地实现实际位置/显式训练目标分离、分位置统计和目标英雄定向匹配；长期画像、目标交互尚待接线，不判定补位。真实Coach与Training/live UI未验收，黄金切片仍进行中，详见 `docs/plans/2026-09-10-real-golden-slice.md`。
...
RQ-215 在 RQ-214 离线预检和同 SHA 公共 CI 后，以一次性授权只发送 1 次真实
`zhipu/glm-5.3-flash` 请求。观察提交为
...
官方 TLS transport 外层 gate 已进入，真实流启动后形成 pending reader，并在 `31ms` 内被
response close 唤醒；取消抛出安全码 `zhipu_stream_close`，iterator/composite 为 `failed`、
SDK stream 为 `closed`，因此结论为 `client_wakeup_close_race`。回执为
...
该证据仍属于 8E/8-Advanced candidate-only 客户端观察，不新增 8-Core 能力，不证明
provider-native close/wakeup、模型一般能力或生产 streaming。候选仍 disabled/未注册，
默认模型、产品 Runtime、Portal、Account、Workbench、Auth、路由和 `production_media=0` 不变。
当前精确 checkpoint 为
...
RQ-217 在 RQ-216 公共 CI 闭环后按用户授权只执行 1 次真实
`zhipu/glm-5.3-flash` 请求，仍属于 8E/8-Advanced 的 candidate-only 客户端观察，不改变
8-Core、8E→8F 顺序或产品默认。实现/观察器/输入计划 SHA 为
`3e028b1217f1274152ba161993287f29188a1b73`，Actions `33727163550` 三 job exact-SHA 全绿。

官方 TLS transport 外层 `before_first_event` gate 进入并形成 pending reader；
`provider_call_count=1`、`transport_request_count=1`、`network_used=true`，
`cancel_status=returned`、`reader_woke=true`，iterator/SDK/composite close report 均为
`closed`，结论为 `client_wakeup_clean`。回执为
...
保持 body-free 且 canonical round-trip 通过；`gate_released=false` 是受控停顿协议的预期条件。

该结果只说明真实流启动后本机客户端的 reader 唤醒和 reader-owned 收尾，不证明
provider-native close/wakeup、底层 HTTP response 独立取消、模型一般能力、生产 streaming、
G53-7、黄金切片或 8F。候选继续 disabled/未注册，`capabilities.streaming=False`；默认模型、
...
新增与相邻本地回归 `123 passed`，no-I/O 准入、compileall、diff check 与治理检查通过。
实现 `c50cf231957bc54201d0207b99110fcf4b2897b3` 的 Actions `33843064715` 三个任务
exact-SHA 全绿；公共 Python 2349、PostgreSQL 201、前端 270，packaging-smoke 通过。
...
纠正批已将检索策略显式绑定到输入计划、协议、预算与候选门控，并纳入 RQ-235 历史排重；检索加固后的
最坏预算为每案 `205000`、全域 `613000`，不改变旧 V3 或产品默认边界。
...
### RQ-242（2026-09-06）经济观察与修订诊断

经济场景已于公共验证的 `7c931f6` 完成：4 调用、3 来源，修订拒绝且无最终评分。安全校验码与开发历史投影已通过 117 测试及 `a64331b` / CI `34022245397` 三任务验证；上条“待经济观察”已取代。下一项是候选生成/修订合同对齐的离线设计与回归，不改模型选型，不自动重跑。8E 仍在进行，正式领域、黄金切片与 8F 未关闭。
