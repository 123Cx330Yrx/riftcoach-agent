...
> `capabilities.streaming` 仍为 `False`，候选仍未注册；下一项是独立的候选 runtime 接线裁决，不能自动重试或进入 G53-7；
>
