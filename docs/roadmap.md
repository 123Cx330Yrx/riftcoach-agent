...
| 6 | API、Session 与 Memory | 如何从脚本变成真正的长期个性化 Coach | EchoMind 迁移重构 | 进行中；6A-1 至 6A-6 已公共完成；6A-7 已定位 Linux import-root 漂移，module-entry 修复待 CI |
...
- 6A-1 至 6A-6 已公开建立 PostgreSQL task、原子 claim、Application/Artifact 接线、异步 HTTP、
  CORS/脱敏、背压、生命周期删除和真实性能边界；6A-7 packaging/exit 已完成本地实现与门禁并等待
  exact-SHA Linux/真库公共验证；首个 run `32145005904` 的 pytest/真库成功、one-off smoke 失败，当前
  已用后续 run 定位 direct-script/wheel 的 Alembic import-root 漂移，当前只做 module-entry 修复，
