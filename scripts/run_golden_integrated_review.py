...
def implementation_identity(*, bounded=False, full_context=False, comparison=False, provisional=False, meaning=False, grounded=False):
    ...
    if bounded or full_context or comparison or provisional or meaning or grounded:
    ...
    if full_context or comparison or provisional or meaning or grounded:
    ...
    if comparison or provisional or meaning or grounded:
        names += ("app/evaluation/golden_reassessment_feasibility.py",
            ...
            "scripts/run_golden_comparison_review.py")
    if provisional:
...
def run(args, *, bounded=False, full_context=False, comparison=False, provisional=False, meaning=False, grounded=False):
    if sum((bounded, full_context, comparison, provisional, meaning, grounded)) > 1:
        raise ValueError("review_mode_conflict")
    if grounded and args.execute:
        from app.evaluation.golden_grounded_reading_review import require_live_qualification
    ...
    if comparison:
        from app.evaluation.golden_comparison_workflow import ComparisonReassessmentWorkflow, EXPERIMENT_ID
        workflow_factory, experiment_id, prefix = ComparisonReassessmentWorkflow, EXPERIMENT_ID, "comparison-review"
    if provisional:
        from app.evaluation.golden_provisional_workflow import ProvisionalReassessmentWorkflow, EXPERIMENT_ID
        workflow_factory, experiment_id, prefix = ProvisionalReassessmentWorkflow, EXPERIMENT_ID, "provisional-review"
    if meaning:
        from app.evaluation.golden_meaning_first_review import MeaningFirstWorkflow, EXPERIMENT_ID, first_request
        workflow_factory, experiment_id, prefix = MeaningFirstWorkflow, EXPERIMENT_ID, "meaning-review"
    if grounded:
        from app.evaluation.golden_grounded_reading_review import GroundedReadingWorkflow, EXPERIMENT_ID, first_request
        workflow_factory, experiment_id, prefix = GroundedReadingWorkflow, EXPERIMENT_ID, "grounded-reading"
    if comparison or provisional or meaning or grounded:
    ...
    plan = dict(experiment_id=experiment_id, implementation=implementation_identity(bounded=bounded, full_context=full_context, comparison=comparison, provisional=provisional, meaning=meaning, grounded=grounded),
...
def main(*, bounded=False, full_context=False, comparison=False, provisional=False, meaning=False, grounded=False):
    ...
    run(p.parse_args(), bounded=bounded, full_context=full_context, comparison=comparison, provisional=provisional, meaning=meaning, grounded=grounded)
