...
def prepare(source: Path, *, scope=False, scope_v2=False, scope_v3=False):
    from app.runtime.coach_contract import SCOPE_COACH_CONTRACT, SCOPE_V2_COACH_CONTRACT, SCOPE_V3_COACH_CONTRACT
    contract = SCOPE_V3_COACH_CONTRACT if scope_v3 else SCOPE_V2_COACH_CONTRACT if scope_v2 else SCOPE_COACH_CONTRACT if scope else CONTRACT
    ...
    assets = ROOT / "examples/runtime_profiles" / ("flash_v2_golden_scope_v3" if scope_v3 else "flash_v2_golden_scope_v2" if scope_v2 else "flash_v2_golden_scope" if scope else "flash_v2_golden_coverage")
...
def run(args):
    ...
    scope = getattr(args, "scope", False) or scope_v2 or scope_v3
    from app.runtime.coach_contract import SCOPE_COACH_CONTRACT, SCOPE_V2_COACH_CONTRACT, SCOPE_V3_COACH_CONTRACT
    contract = SCOPE_V3_COACH_CONTRACT if scope_v3 else SCOPE_V2_COACH_CONTRACT if scope_v2 else SCOPE_COACH_CONTRACT if scope else CONTRACT
    if scope and not (args.report_only or args.controls_only):
        raise ValueError("scope_requires_separate_report_or_controls_run")
    dataset, summary, original = prepare(args.source_run, scope=scope, scope_v2=scope_v2, scope_v3=scope_v3)
    ...
    def components(name):
        ...
        options = {"inference_audit": "scope_v3" if scope_v3 else "scope_v2" if scope_v2 else "scope" if scope else "coverage", "include_generation_facts": True, "include_deterministic_facts": True,
