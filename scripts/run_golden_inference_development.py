...
from app.runtime.coach_contract import FACT_INFERENCE_COACH_CONTRACT, FEEDBACK_COACH_CONTRACT, EXPANDED_COACH_CONTRACT, COVERAGE_COACH_CONTRACT as CONTRACT
...
def prepare(source: Path, *, scope=False, scope_v2=False, scope_v3=False, scope_v4=False, expanded_output=False, scope_v5=False, fact_inference=False):
...
def run(args):
    ...
    scope_v4 = getattr(args, "scope_v4", False) or scope_v5 or fact_inference
    ...
    dataset, summary, original = prepare(args.source_run, scope=scope, scope_v2=scope_v2, scope_v3=scope_v3, scope_v4=scope_v4, expanded_output=expanded_output, scope_v5=scope_v5, fact_inference=fact_inference)
    ...
    def components(name):
        ...
        options = {"inference_audit": "fact_v1" if fact_inference else "scope_v5" if scope_v5 else "scope_v4" if scope_v4 else "scope_v3" if scope_v3 else "scope_v2" if scope_v2 else "scope" if scope else "coverage", "include_generation_facts": True, "include_deterministic_facts": True,
