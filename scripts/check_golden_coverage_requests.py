...
def measure(source, report_path, *, scope=False, scope_v3=False, scope_v4=False, scope_v5=False, fact_inference=False, evidence_scope=False, failed_response=None):
    from app.runtime.coach_contract import EVIDENCE_COACH_CONTRACT, FACT_INFERENCE_COACH_CONTRACT, FEEDBACK_COACH_CONTRACT, SCOPE_COACH_CONTRACT, SCOPE_V3_COACH_CONTRACT, SCOPE_V4_COACH_CONTRACT
    contract = EVIDENCE_COACH_CONTRACT if evidence_scope else FACT_INFERENCE_COACH_CONTRACT if fact_inference else FEEDBACK_COACH_CONTRACT if scope_v5 else SCOPE_V4_COACH_CONTRACT if scope_v4 else SCOPE_V3_COACH_CONTRACT if scope_v3 else SCOPE_COACH_CONTRACT if scope else CONTRACT
    ...
    class Offline(_ReplayProvider):
        def chat(self, request):
            ...
            if scope or scope_v3 or scope_v4 or scope_v5 or fact_inference or evidence_scope:
                for item in evaluation["coverage"]: item["scope_ambiguous"] = False
            if scope_v4 or scope_v5 or fact_inference or evidence_scope:
    ...
    options = dict(runtime=ToolRuntime(registry), inference_audit="evidence_v1" if evidence_scope else "fact_v1" if fact_inference else "scope_v5" if scope_v5 else "scope_v4" if scope_v4 else "scope_v3" if scope_v3 else "scope" if scope else "coverage", include_generation_facts=True, include_deterministic_facts=True,
