...
def measure(source, report_path, *, scope=False, scope_v3=False, scope_v4=False, scope_v5=False, fact_inference=False, evidence_scope=False, evidence_scope_v2=False, evidence_scope_v3=False, evidence_scope_v4=False, failed_response=None):
    from app.runtime.coach_contract import EVIDENCE_V4_COACH_CONTRACT, EVIDENCE_V3_COACH_CONTRACT, EVIDENCE_V2_COACH_CONTRACT, EVIDENCE_COACH_CONTRACT, FACT_INFERENCE_COACH_CONTRACT, FEEDBACK_COACH_CONTRACT, SCOPE_COACH_CONTRACT, SCOPE_V3_COACH_CONTRACT, SCOPE_V4_COACH_CONTRACT
    contract = EVIDENCE_V4_COACH_CONTRACT if evidence_scope_v4 else EVIDENCE_V3_COACH_CONTRACT if evidence_scope_v3 else EVIDENCE_V2_COACH_CONTRACT if evidence_scope_v2 else EVIDENCE_COACH_CONTRACT if evidence_scope else FACT_INFERENCE_COACH_CONTRACT if fact_inference else FEEDBACK_COACH_CONTRACT if scope_v5 else SCOPE_V4_COACH_CONTRACT if scope_v4 else SCOPE_V3_COACH_CONTRACT if scope_v3 else SCOPE_COACH_CONTRACT if scope else CONTRACT
    ...
    class Offline(_ReplayProvider):
        def chat(self, request):
            ...
            if scope or scope_v3 or scope_v4 or scope_v5 or fact_inference or evidence_scope or evidence_scope_v2 or evidence_scope_v3:
                for item in evaluation["coverage"]: item["scope_ambiguous"] = False
            if scope_v4 or scope_v5 or fact_inference or evidence_scope or evidence_scope_v2 or evidence_scope_v3:
                evaluation["coverage"] = [[r["block_id"], "N", "N", False] for r in evaluation["coverage"]]
            if evidence_scope_v4:
                evaluation["reviewed_blocks"] = [r["block_id"] for r in evaluation.pop("coverage")]
                for audit in evaluation["audits"]: audit.pop("status")
    ...
    options = dict(runtime=ToolRuntime(registry), inference_audit="evidence_v4" if evidence_scope_v4 else "evidence_v3" if evidence_scope_v3 else "evidence_v2" if evidence_scope_v2 else "evidence_v1" if evidence_scope else "fact_v1" if fact_inference else "scope_v5" if scope_v5 else "scope_v4" if scope_v4 else "scope_v3" if scope_v3 else "scope" if scope else "coverage", include_generation_facts=True, include_deterministic_facts=True,
