...
class _ReplayProvider(_WorstPathProvider):
    ...
    def chat(self, request):
        ...
        if request.response_contract is not None and request.response_contract.version in ("1.3.0", "1.4.0", "1.5.0", "1.6.0") and ordinal in (6, 9):
            payload = json.loads(response.content)
            payload["audits"] = [{"kind": kind, "status": "not_applicable", "claims": []}
                                 for kind in ("metric_to_ability", "cohort_comparison")]
            if request.response_contract.version in ("1.5.0", "1.6.0"):
                ...
                if request.response_contract.version == "1.6.0":
                    for item in payload["coverage"]: item["scope_ambiguous"] = False
