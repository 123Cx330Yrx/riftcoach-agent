...
@pytest.mark.parametrize("mode", ["comparison", "provisional", "meaning", "grounded", "advisory", "partition"])
def test_preview_is_one_frozen_case_with_no_ci_secrets_or_provider(monkeypatch, tmp_path, case_index, mode):
    ...
    if mode == "partition": expected_id = "golden-computed-partition-review-v1"
...
@pytest.mark.parametrize("mode", ["provisional", "meaning", "grounded", "advisory", "partition"])
def test_provisional_runner_uses_new_workflow_after_ci_without_opening_retired_entry(monkeypatch, tmp_path, mode):
    ...
    from app.evaluation import golden_partition_review as partition_held
    from scripts import run_golden_integrated_review as runner
    from scripts import run_golden_contextual_review as controls
    inputs = inputs_for(report="这四场中单只作本样本比较。[K1]")
    req = make_request(inputs)
    ...
    monkeypatch.setattr(partition_held, "require_live_qualification", lambda: None)
    ...
    def observe(*args, workflow_factory):
        ...
        from app.evaluation.golden_partition_review import ComputedPartitionWorkflow
        assert workflow_factory is {"meaning": MeaningFirstWorkflow, "provisional": ProvisionalReassessmentWorkflow,
            "grounded": GroundedReadingWorkflow, "advisory": ProvisionalReadingWorkflow,
            "partition": ComputedPartitionWorkflow}[mode]
    ...
    args = SimpleNamespace(source_run=tmp_path, base_report=tmp_path, pair=1, execute=True,
        case_index=1, run_id=({"grounded": "grounded-reading-scripted", "advisory": "provisional-reading-scripted",
            "partition": "computed-partition-scripted"}.get(mode, f"{mode}-review-scripted")),
