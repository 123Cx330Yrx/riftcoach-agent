...
**当前动作：** 定点配对成功仅说明指定位置后可区分归因真错。ADR0105紧凑逐段机制的自主全文首例在
300秒超时，没有完整响应/用量；最后保存事件83.484秒不是保证精确的断流时刻。模型语义仍未知。
完整证据`golden_native_block_tool_interruption_a501c33.json`。当前两版live入口已关闭，未继续余下报告。
短连接代理/直连均可达，不能断言代理或Provider根因；新增body-free工具事件计数和时间修复观测盲区。
下一步先用有区分力的长流/终止诊断核查此中断，保留原模型/请求预算，不原样重试、不扩大时限。
若诊断涉及新调用，先固定输入/路径干预及各结果决定；沿用已有有界授权，不发起整批重跑。
传输35项与产品/新机制21项相关检查通过，等待本次公共CI。质量覆盖顺序仍按上面的既有15例，
但任何定点诊断、旧版本成功或结构齐全都不计入自主全文资格。
