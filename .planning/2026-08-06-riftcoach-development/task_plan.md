...
下一步精确HEAD公共CI后执行该对照并逐项人工核对；共享401920tokens/900秒，最多4调用，
每次至多32768/300秒，预留308404tokens。协议/执行失败停止，合法语义失败仍保留完整对照。
若target仍失败停止布局路线，不继续排列组合；若两臂都正确不能声称布局修好；
若只有target正反均正确才投入整链接线/既有覆盖。本批尚0调用，不恢复旧候选或产品入口。
