...
最新6c3a3db实测在完整来源时间已提供的情况下又漏检日期真错，且一条非阻断建议选错指标来源；v2已停止。
...
**预算和授权：** 单任务5调用/401920tokens/900秒，单次32768/300秒，SDK retry0，一次改稿；两模型共享，预算不变。v2首例语义失败已停，1次/17113tokens/估价0.203804元、未知0。后续独立编辑按用户既有连续授权和本轮继续指令单独执行1次，300秒截止、用量未知；两批均未发送第二例、不借用余量。连续授权来源为2026-09-10全授权及后续反复要求诊断修复继续，不能把本轮写成用户逐字新增“两次”批准。新同输入Flash审查对照另冻结最多2次/600秒/164182tokens/估价上界0.2624176元；本提交公共CI后执行，失败即停。默认产品未切换，Luna仅Codex开发协作。

**当前状态和下一动作：** 来源目录不能独立修漏检；独立整稿编辑又在300秒无正文截止，能力未测得，不接入pass后强制编辑。旧74.735秒成功编辑与本次输入不同，不能把多5778字符定为超时根因。补更直接的同输入Flash审查对照，只换模型/transport，复用已有review/observer；错误稿需正确检出日期真错且全部意见有来源，正确参考需通过。若通过，准备现有needs_revision→实际改稿→GLM终评；若失败，关闭替换方案并重新选择任务职责/完整流程，不排列字段或提示重试。原GLM第二例未发送、旧Flash是另一归因稿，均不能冒充已有同题结果。ADR0110和两个新结果/预检保存依据；原15、实际消费和其余8E退出项不变。
