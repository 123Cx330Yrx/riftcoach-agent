...
**当前动作：** ADR0112新opt-in粗来源合同1.5.3已接入现有完整Runtime；角色/预算/观察包装/回执用量/manifest/Trace/显式builder一致，原15仅准备与回放，质量标志仍false。完整应用和尾部实际factory的离线模拟已通过，正在完成相关回归与独立审查。冻结两调用计划、同HEAD公共CI后，执行Flash真实改稿＋GLM fresh终评，逐阶段完整来源双审；失败按首次偏离定位，不重试或改原标签。通过后接同身份原15完整资格审计与自然Agent任务，再解锁真实消费。旧8/15只属旧封存身份，默认Worker不切换。
