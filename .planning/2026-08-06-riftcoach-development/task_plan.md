...
当前唯一行动：离线裁决原文优先、独立语义重评的数据流，对比不向第二评灌输首评claim判断但保留全部原文义务/旧issue/审计的方案；联合检查跨段范围、运算类型、容量和五调用预算。c3ff392正例两次完整返回仍语义错误，本批已停止，三条失败入口均offline_only；不得只补结构缺项后重新付费。累计Provider至少200，本批48518tokens，无未知用量。
