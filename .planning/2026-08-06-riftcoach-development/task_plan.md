...
**当前动作：** 粗来源两控制已成功封存；新工作流的初评/显式重评/编辑/fresh终评离线接通，68项相关验证通过。实际factory模拟发现旧角色路由+预算合同拒绝新投影，因此付费tail入口在任何prepare/密钥前禁用，未调用。下一步一次性注册新opt-in执行合同，接齐角色路由、预算身份、manifest、可信Trace、资格回放与builder，保留默认旧版本及旧8/15；真实factory替身整链/独立审查/同HEAD CI通过后才冻结并开放最多2次真实编辑/终评。详见docs/plans/2026-09-25-source-granularity-diagnosis.md末节，不再新增临时router或伪装旧metadata绕过身份。
