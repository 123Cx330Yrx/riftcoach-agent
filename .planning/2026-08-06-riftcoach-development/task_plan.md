...
2026-09-16：比较候选首评完整返回但因临时结果准入失败，新增1请求16781tokens，累计至少198。已停用真实入口；新临时首评合同离线原字节可继续，153项相关检查通过，实际语义与漏目标补查未验证。

当前唯一行动：联合裁决原始事实/派生统计/旧模板判断/首评意见/最终语义的数据流，对比原文优先且保留全部原文义务/旧issue/审计的独立重评方案；联合检查跨段范围、运算类型、容量和五调用预算。c3ff392正例两次完整返回仍语义错误，本批已停止，三条失败入口均offline_only；不得只补结构缺项后重新付费。累计Provider至少200，本批48518tokens，无未知用量。上游模板已去掉无依据能力/稳定性结论，128项相关检查通过；冻结报告与旧manifest未改，此修复不证明模型语义通过。

全局后续见 `docs/plans/2026-09-16-astra-restart-plan.md`；包括前端整体升级与四块联动。

原节全文逐字符归档于同目录 `history-2026-09-16-pre-restart-sections.md`，只作历史，不执行其旧指针。
