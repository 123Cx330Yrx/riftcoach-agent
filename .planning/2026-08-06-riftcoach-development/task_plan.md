...
当前精确 checkpoint：`8e-productization / candidate-explicit-zhipu-neutral-stream-adapter-seam / candidate-report-contract-budget / completed-local / pending-public-ci`。
...
当前唯一下一步：`8e-productization / candidate-explicit-zhipu-neutral-stream-adapter-seam / candidate-report-contract-budget / completed-local / pending-public-ci`。

RQ-244 已完成新报告合同九调用预算证明与产品迁移版本清单，104 项测试通过；当前待本实现公共 CI，之后直接推进实际 Runtime 的新执行组合离线接线。真实 API=0，不改默认模型或前端；详细见 `docs/plans/2026-09-06-report-contract-budget-and-runtime-migration.md`，下方旧动作均为历史。
