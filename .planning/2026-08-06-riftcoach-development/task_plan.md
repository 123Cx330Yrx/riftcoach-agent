...
不采用直接替换，暂不实现专用适配或live候选；改从审查任务职责和输出负担比较机制。
...
下一步比较现有全文平铺意见与逐段清单的语义职责、输出/恢复负担、原反例和五调用预算，
明确一项针对语义覆盖而非只换格式的最小干预及正反全文验证分支，记录在现有方案裁决。
本批0次付费；不追加数组候选、不循环开关、不提额度，不把上述工程修复当成模型问题解决。
