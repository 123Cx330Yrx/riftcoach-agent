...
问题已从纯格式区分到输出职责：强制逐段重述正确事实制造额外错误，结构覆盖也不证明语义覆盖。独立native-issues候选改回业务结论和实际问题清单；问题仍有全文原段定位、真实来源、解释和修正，完整28段输入/标题/尾句/身份/范围/知识/安全/全部领域标准不减。复用原始响应保留、一次完整重评、修订绑定和5次总预算；旧verbose native入口停用，连同原八旧入口保持离线。没有把旧错误解释删字段后改判成功。

210项相关检查通过，原五份来源/报告/标签完全不变。请求45574–45650，真实坏响应进入重评55930<63936；完整五次脚本预约387042<401920，实际结算+下一请求预约仍决定能否继续；离线不证明召回或语义质量。唯一下一步：native-issues实现同SHA公共三项后，原正例完整审读，合格再逐个执行未来/身份/日期/标题负例→自动修订→复评；失败停批诊断，不按字段付费碰运气。方法及剩余质量边界见docs/plans/2026-09-19-native-problem-list.md。

合格后接现有Runtime/原子保存/Evidence/API/Workbench：生成和审查共享预算、运输与审查状态按任务隔离、保留观摩身份。Stage8E仍in_progress，glm-5.3-flash/high及所有上限、人工工作台稿不变；前端审美重做/头像/四块联动/专属Training及62主题后续完整保留。
