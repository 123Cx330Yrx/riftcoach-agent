...
2026-09-19 native v2在4f2307b/Actions35417428624公共三项success后完成五例实际观察：正确报告1调用通过；未来外推、阅读者身份混淆、官方日期混用分别经3调用完成检出→自动修订→完整复评，已审读实际改文和全部解释。这是三条真实纠错链路，不只是CI或合成证明。标题能力断言也被正确检出和修订，但最后复评失败，整批没有全部质量通过。13次均完整stop，162584输入+69242输出=231826tokens，无未知用量，累计Provider至少220；每次17.266–91.110秒，无本批超时或输出耗尽。

标题复评在JSON后追加“``结束。”导致严格解析失败；独立只读投影还发现第2段错配第4段标题解释、第3段及summary把辅助1负写成0负、第9段位置范围说明不准确。仅去掉尾文会被现有结构校验判pass，明确不是语义修复。原回执保持失败，所有源码/请求/响应/修订/回执哈希和审读见golden_native_review_result_4f2307b.json；旧八失败入口仍停用，工作台人工稿未替换。

已离线修复完整重评的准入断点：一个完整、无重复键JSON对象后的非JSON尾文只允许作为临时旧意见，全部原值、尾文、原问题和原始hash进入既有唯一完整重评，最终响应仍严格校验。多对象/截断/冲突键/非有限数及高危注入保持终止，第二次坏响应不再试。真实失败原字节已纳入离线夹具；4/5调用完整路径、旧问题显式处置和全预算通过，180项相关检查通过。没有改领域提示、high、调用/输出/时间上限、估算器或完整上下文标准；离线重评可达不代表模型已纠正错误解释。

唯一下一步：本恢复实现同SHA公共CI后，用正确报告和标题负例作新的有界验证，逐条审查真实解释及修订复评；已有三类成功保持原SHA证据，不重新计为新实现成功。任一失败结束新批并保留，不补字段付费碰运气。只有复评语义合格才继续共享生成/审查预算的Runtime与原子保存/Evidence/API/Workbench接线；Stage8E仍in_progress，前端审美重做/头像/四块联动/专属Training与62主题后续保持。
