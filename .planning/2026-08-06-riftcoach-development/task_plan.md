...
**当前动作：** ADR0104分层候选已实现；38项聚焦回归、治理检查和完整产品组装通过，精确CI `35689438741` 通过，三例真实开发控制已冻结。
它把阻断issues与完整上下文下的advisories分开，保留建议而不触发修订；案例3的实际改稿经逐字差异与来源核对确认只修正真实补刀错误。下一步是按既有去重清单做同版本完整覆盖；任一真实错误漏检、advisory误升级、协议失败或实际改稿伤及正确段，都停止该候选批次并回到机制诊断。
