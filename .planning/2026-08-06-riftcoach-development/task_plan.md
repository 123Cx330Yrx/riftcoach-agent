...
**具体实现：** ADR0106复用ADR0105完整段落清单，但独立审查只提交reviews/score/verdict；
仅有旧评估的重评要求issue_resolutions，仍逐项映射。单一工具Schema，显式buffered-tools并记录进程路由。
严格保留原响应与内部投影区别，编辑仅接收真实阻断，不改完整上下文采用标准、来源和资源合同。
...
**当前动作：** eadb930 / CI35710759810的buffered正反配对完整返回，判断分别85/revise及95/pass；
后者因缺初审无用的旧问题映射字段失败，配对不合格。2调用/32656tokens，无未知。
先完成阶段合同、原始journal/编辑/重评/产品五调用相关离线验证，精确HEAD CI后执行归因负例完整链路，
随后正例和既有控制，逐例核对改文。不再重复Schema文字诊断。原诊断/失败记录见attribution-miss-diagnosis。
阶段合同是接口减负，不证明模型准确率；旧生产入口仍关闭，不能把离线兼容回放改称新实测成功。
