...
**预算和授权：** 单任务5调用/401920tokens/900秒，单次32768/300秒，SDK retry0，一次改稿；两模型共享，预算不变。用户“确认继续”批准v2两例，首例语义失败已停止，仅1次/17113tokens/估价0.203804元、未知0，第二例未发。原v1及更早授权均已结束，不挪用未用次数；已有新批准备但尚未授权或执行，见下方当前动作。默认Flash/high未切换；Luna仅Codex开发协作。

**当前状态和下一动作：** 已完成ADR0110来源职责核查及最小离线实现。保留固定目录原型，不作为漏检修复：完整错误正文加正确目录后，旧pass仍能过结构校验。原15仅证明全文与源输入保留；两次生成/工具、needs_revision、编辑、终评的实际Runtime替身路径可达，不是新pass后核验流程或产品采用。已准备独立Flash编辑能力正负配对：不送旧意见、不加新目录，只比较实际修错与正确全文保留；新批最多2次/600秒，上界估价0.2599216元，计划hash在golden_independent_edit_preparation_v1.json。当前未付费、等待当前提交公共CI及这批具体授权。任何失败停止，不追加提示重试；两例接受才进入实际编辑稿普通GLM终评，不先铺产品新状态机或拼旧成功。已有v2仍关闭，不挪其剩余一次。
