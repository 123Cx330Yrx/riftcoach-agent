...
**预算和授权：** 单任务5调用/401920tokens/900秒，单次32768/300秒，SDK retry0，一次改稿；两模型共享，预算不变。用户“确认继续”批准v2两例，首例语义失败已停止，仅1次/17113tokens/估价0.203804元、未知0，第二例未发。原v1及更早授权均已结束，不挪用未用次数；当前没有新付费计划。默认Flash/high未切换；Luna仅Codex开发协作。

**当前状态和下一动作：** v2在6c3a3db / CI35835943718上95/pass漏检；完整来源/请求回放已排除字段缺失、传错稿、编号偏移及预算问题。正确建议使用不相干来源的独立失败亦已记录。已关闭该批execute，7项入口/预检测试通过，原失败和计划不改。元数据和单条引用工程修复保留，撤回其足以解决审查的假设。下一步按日期诊断文档核查“程序来源事实、模型分析”的职责方案，保留完整语义审查，不移除原错误报告的回归要求；先看实际七节报告合同、改稿/终评、原15资格和5次共享预算可否容纳，再决定是否值得最小原型和实测。禁止把另加一次审查当作免费补丁，不再排列v3字段/提示变体。
