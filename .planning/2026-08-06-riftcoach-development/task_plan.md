...
**当前状态和下一动作：** 日期v2在061482e7 / CI35851847210三项成功后实际完整返回，漏两处真错，关闭且不采用审查器替换。来源目录不能独立修漏检，整稿编辑未完成，旧正例标签遗漏已修正但新参考尚未实测。下一步核对完整任务职责和预算，选择真正可执行的路径：额外来源专审+现有全流程需6调用，不能只看单次可行；用专审替换首评则业务问题可能首次在终评暴露。核查生成/检索是否能复用已有准备机制，及合同的真实代价，再决定实现。旧source-first为复杂全稿重评、typed source_checks仅离线，不冒充专审实测。未准备第三个付费变体，原15、实际消费和其余8E退出项保留。
