# 2026-09-23 固定来源呈现不是语义纠错，首评直发路径仍需能力验证

- 新原型与实际原失败回放证明：正确来源目录不会自动消除正文日期冲突；当前合法pass仍能漏掉它。决定不以目录组装作为漏检充分修法。
- 不可晚于editor返回再替换文本，否则既有expected_recheck不再绑定实际输出；不应通过重写私有期望状态逃过检查。修订只验证来源栏原样保留，首评/终评/发布成稿一致。
- 目录中的[K编号]甚至来源名内方括号会绕过正文引用存在门；长固定栏会稀释全篇70%长度规则。新原型转义目录数据，另对正文检查，不把这些工程门声称为来源蕴含判断。
- 旧独立编辑并非全失败：6c22e93编辑本例正确，复杂两状态Flash终评失败；当前GLM首评pass后的Flash独立编辑/普通GLM终评仍是未证假设。5次仅能容纳无恢复、两次生成的路径，不能暗加第六次或伪造needs_revision。
- 新配对不同时启用来源栏，以便失败能否定独立编辑补漏的方案；正反全文、实际改动、旧意见隔离与失败停止均已准备，能力未实测前不扩建新产品控制流。

# 2026-09-23 元数据充分仍漏检，正确建议仍可能引用不相干指标

- 6c3a3db v2已证明新增引用时间实际进入请求，但95/pass遗漏原日期真错。因此“修好投影即足以修好模型审查”被本例否定；v1检出/v2漏检的各一次观察不支持因果退化结论或稳定率。
- block14的早期死亡均值2.5/0.5正确，选中的9/13却是CS均值；1只含混合组早期死亡，21仅为范围边界。文字正确、编号合法与所选证据支持是三件事，不能互相替代，也不能把建议的引用错误改写成原报告错误。
- 当前整任务2生成/工具+首评+改稿+终评已占5次。再叠一次审查需要重新考虑整个调用/时限分配，不能当作无成本补丁。
- 程序来源事实/模型分析职责是下一离线方案，不是已证明修法；单纯附上正确来源或删掉错误段都不消除正文语义风险。已有Evidence/Knowledge可复用，无需另建来源系统。

# 2026-09-23 完整输入含时间，但单条来源解析不含时间

- 883c23e真实首例已正确检出日期错误；失败是所选K1—K5条目缺少retrieved_at，不是编号偏移或模型再次漏检。总目录source25有依据不能自动替代issue选出的26—30；使用既有来源支持标准，不新增验收条件。
- 同一时间元数据在生成context按chunk关联，在审查投影却仅放顶层，导致可引用粒度不同。共享citation_retrieval_fields补全逐条来源，保留多次/缓存/未知，不把一条全局日期套给全部知识。旧source值和失败回执不可回写。
- 本轮指纹错误来自对role/native两组合使用同一构造器；两者共享来源不代表执行合同相同。恢复时应按resolver实际选用的构造器更新，不能只照摘要里的通用代码。
...
- buffered两次结构完整只能支持下一测试，不能推出充分修复。新反例后停止该分支；模型和服务端工具转换的责任尚未分离。
- JSON正文持续活动到300秒而无正式内容，只能记本次截止/语义未知；不能称网络断流或输出token上限已耗尽。
- 初审无旧意见却要求空映射字段是可移除的合同负担。阶段拆分已实现，但它未解决所有格式故障。
- 诊断脚本修好usage并不等于实际runner修好。后者漏接已由真实失败暴露并修正，导出同时核对未组装响应的已知usage，旧回执不回写。
- 本轮又出现本机忽略文件掩盖CI测试依赖，已加提交fixture/禁读约束；不能声称执行老毛病已全部消失。
- 一段一个工具结果在现8工具上限不可达；位置数组可无损但恢复/追溯和模型表现未证实，不从离线往返推断质量或延迟。

# 2026-09-22 执行方法自身的缺口

- 旧canonical机器唯一下一步停留在旧临时首评合同，而开头已有多轮新行动；治理检查只核checkpoint字符串，
  不能发现行动语义过期。已整理成短入口并人工核对，不声称治理脚本能检出全部语义漂移。
- 第一版方法把未来发布资格绑定缺口放在当前误报前，属于依赖/优先级错误；已改为发布前独立依赖。
- 第一版将单例不能准入等同单例无诊断价值；已改为用结果能否改变技术决定判断，而非用它能否直接验收产品判断。
- 相关原文与SHA保存在docs/archive/2026-09-22-execution-method/；系统执行方法及当前工作卡按canonical入口。

以下为历史发现。

# RiftCoach 持续开发发现

2026-09-16：本轮真实失败在首评准入，第二步比较设计尚未受到模型检验。14来源上限、错误片段、漏目标是独立现象；单纯放宽来源上限仍会在定位校验处失败。临时首评必须按可识别原文义务进入重评，而非先证明自身是合格最终结果。新离线接口覆盖一般首评字段错误，保留原字节/问题，不能由此推断模型会补齐漏目标或选择正确样本。未知来源仍终止；详细证据见golden_provisional_reassessment_v1.json。

以下为历史记录，当前以最新canonical为准。

2026-09-16：比较操作数必须独立于整句来源，否则同句的全体数量和中单比较会被错误判为混组。新绑定逐项核对实际成员，保留合法额外事实来源。错误cohort选择、同数字异指标选择、解释矛盾和空绑定仍可能结构通过，属于真实语义审查义务；不把它们写成已检测成功。首版成员接口及预算中间结果v1/v2留作拒绝证据，最终v4保存完整两例、十份历史及带绑定修订尺寸。Luna只读检查因服务404不支持模型未启动，未得到独立审核结论。

以下为历史记录，当前以最新canonical为准。

2026-09-16：旧补丁状态空间与可用操作不匹配可用完整重评解决，但原文覆盖不证明解释对象一致。真实c010重组后仍结构pass，已作为显式反例；范围/角色/胜负组/指标与解释关系是下一项合同工作，不能再以结构通过恢复实测。

以下为历史记录，当前以最新canonical为准。

2026-09-16：此前offline-only只写在文档，命令仍可执行失败候选。现真实入口在所有I/O之前拒绝；纠正入口识别同audit整段重复及必要修改容量超限。只识别已知不可能状态，不以无阻断证明语义正确。真实v10原字节回放确认第二调用未发生，详情见golden_contextual_admission_v10.json。

以下为历史记录，当前以最新canonical为准。

2026-09-16 v10结果：同段整段重复已反馈但协议禁止删项/缩句/换源，因此不存在合法纠正；即使去掉重复限制，c010混组解释仍未修复。另有首评/纠正容量及标题说明身份不对齐。问题不只是模型遵循提示，必须验证整套允许状态和可用操作。可执行审计见golden_contextual_reachability_v10.json，原回执不改。

以下为历史记录，当前以最新canonical为准。

2026-09-16：json_object已实际发送，但strict Schema是本地合同，不是服务端约束。v9尾部杂文只是最新症状；九份旧首评各有6–14处direct范围字段冲突，纠正单独简化没有消除首评的问题来源。统一两步表示及规则，保留完整事实/问题/来源。尾部恢复64912超预算已撤下；新十份尺寸投影56204–61508不是质量通过，详情见ADR-0102 V10。

以下为历史记录，当前以最新canonical为准。

2026-09-16：v8第一例96/pass但解释错误改用五局混合样本；第二例80/needs_revision后真实修订两句并返回复评，因direct附来源范围拒绝。v9保留该来源审计而不改变分类，并补同对象审查要求。详见canonical及ADR-0102最新节。

以下为历史记录，当前以最新canonical为准。

2026-09-16：新wire漏旧Markdown结尾处理，诊断仍指挥旧字段，实际队列来源要求未完整迁入。已修接线并编号真实来源；原模型错引request仍被拒绝，人工投影不是成功。详见canonical及ADR-0102最新节。

以下为历史记录，当前以最新canonical为准。

2026-09-16：v6完整纠正因首评引用片段遗漏同段“两局胜局”而阻断。v7对sample/negated的缺省范围引用使用原所在完整段落，保持待审片段、明确模型判定及显式跨段来源。原字节离线95/pass。详见canonical及ADR-0102最新节。

以下为历史记录，当前以最新canonical为准。

2026-09-16：v5真实纠正中两条claim漏填分类/状态/范围，另两条锚点与上下文冲突。v6由显式decision展开关联字段，程序只定位模型选择的原文；不从说明文字猜判定。详见canonical及ADR-0102最新节。

以下为历史记录，当前以最新canonical为准。

2026-09-16：最新真实响应8项模型编辑均在，但程序无法识别被引用原始行的胜负分组均值与有单局限定的“输局中”；修复后94/pass仅结构通过，人工另发现c004解释混组。补解释数值诊断。详见canonical及ADR-0102最新节。

以下为历史记录，当前以最新canonical为准。

2026-09-16 诊断完整性修复：上一轮漏查诊断覆盖：首个锚点错误遮住6条后续direct_result范围冲突。已将逐项校验和诊断共享，并验证48条错误不被反馈上限隐藏；旧错误响应仍拒绝。 详见canonical与ADR-0102最新节。

以下为历史记录，当前以最新canonical为准。

2026-09-16：v3两次真实stop、58952 tokens：22复核无缺项，3条新增说明触发额外ID拒绝；另有一处锚点空格缺失。v4仅有界绑定补充说明与恢复唯一原文显示空白，原始失败不改，离线整体验证通过。累计Provider至少177。 详见canonical与ADR-0102。

以下为历史记录，当前以最新canonical为准。

2026-09-15 大复盘后恢复：确认四处连接缺陷：冻结数值组件被改会破坏历史加载；首评direct_result锁住纠正；OP.GG原文未入引用索引；修订继承旧局部定义门槛。最新失败补记25333返回tokens及一次Provider请求，累计至少175。无损表格将扩展后的最新纠正形状降至63124。详见ADR-0102 Recovery节。
...
## 2026-09-04：RQ-231 V3 有界修订设计发现

- [root-cause] RQ-230 的实质失败是事实核验为假与 `80<85`；零修订预算只是让
  `needs_revision` 立即收敛为拒绝。共享 Harness 默认支持一次修订，因此 V2 未覆盖完整产品评测闭环。
- [decision] 保持全部硬门，V3 只显式打开最多一次修订，并在修订后使用同一严格评测合同复评；
  阻断性安全问题仍不可修订。
- [observability] 当前评测 Schema 已有固定 category/severity，但没有固定章节字段；所以安全诊断
  可公开类别/严重度计数，不能从 quote 或 explanation 猜章节。
- [budget] 最坏调用路径是 `4 + 2 + 1 + 2 = 9` 次/案、27 次/域；Token 输入会随评测和修订增长，
...
- 事实纠正：RQ-237 回执不含实际查询正文，因此不能从 unmapped 反推出“早期死亡相关资料”。
  白名单补词只证明离线样例识别改善，不证明真实失败已修复；终端显示乱码也不等同于文件乱码。

## RQ-238 findings (2026-09-06)

- 候选指引由工厂重建 schema 1.1 Prompt/Context 快照，核对文件摘要和逐案承诺后才允许 Provider I/O。
- Windows 文本读取会规范 CRLF，快照重建先规范 LF，避免同一内容产生不同身份摘要。
- guided_03 证明真实模型可在该指引下完成本地 Evidence 链，但只有一个匿名 demo；95 分不外推为全领域质量。

- 开发验证新增14种中文/英文查询可走完整证据链，已核对引用K1与持久Evidence来源及语料文件相符；
  零来源时即便脚本模型声称有引用也没有发布最终报告。该结论只证明给定工具查询的工程控制流。
- 仍缺少模型自主选择查询在真实开发对话中的稳定性证据；不再把手写查询的成功等同于这个能力。

- 开发观察01补充真实证据：两条自主查询59/65字符，分别含economy和survival+vision别名，但完整查询
...
- 零网络结构对照：同一非空 Markdown 经 CoachDraft 与初稿 citation 校验可通过结构边界，但不改正文交给 validate_revised_report 会因缺固定标题拒绝。这只复现生成/修订的格式合同不对称，不证明报告可发布或本次真实错误就是缺标题。

## RQ-243：具名报告合同（2026-09-06）

- 生成指引和修订指引由同一标题常量生成，初稿复用旧验证器；没有为消除失败删除固定结构或 70% 篇幅要求，也没有自动修改模型正文。
...
公共收口：实现 `cecde250e131a0e81585e2e91ec261fad6817700` / Actions `34026629061` 已核对同 SHA，pytest、postgres-migrations、packaging-smoke 全部 success。RQ-245 离线接线完成公共闭环，下一步改为新产品版本独立真实验收准备，取代上段待 CI；不自动发送真实请求、启用默认或改前端，GLM-5.2 与旧结果保持。
...
## RQ-247：引用/评测修复与资源调整（2026-09-06）

RQ-247 取代上方待协议动作：修复前协议3调用通过、产品13调用，前两案96/95分，经济3来源但引用失败且无有效评分，精确评测触发原因未知；旧记录不回填。本地3个失败用例复现并修复评测一致性/引用修订缺口，新显式组合1.1.0、程序2.1.0、评测1.2.0绑定原有纠正/修订额度与安全诊断。用户要求适度释放资源，改为high/8192/60秒、Agent120秒/调用计时480秒；9调用、事实/引用/注入/85分门不降。新增36项，聚焦/相邻269项及38子测试通过；修复期新增真实请求0；实现 `679b203b5dfe65f64bc586e618caaf850cafd2db` / Actions `34030008563` 已核对同 SHA，pytest、postgres-migrations、packaging-smoke 三任务全部成功；下一步仅准备修复版 high 档、带安全诊断的有界开发观察入口（零网络），不重跑旧正式验收或自动另立考卷。默认、GLM-5.2、前端、production_media=0不变，8E仍in_progress。细节与八维学习复用RQ-246计划的RQ-247追加节。

## RQ-248：修复版产品开发观察入口与检索同义词缺陷（2026-09-06）
...
## RQ-249：四场景完整开发批次（2026-09-06）

用户明确要求全套测试并以“开始”授权本轮：补齐记忆专项，离线与同SHA公共验证通过后，直接执行经济→整体→生存→记忆四案真实开发批次，无需逐案再授权。四案使用同一实现及high/8192/60秒合同，每案最多9调用/649728 tokens，整批最多36调用/2598912 tokens；质量失败记录后继续独立后案，认证/额度/连接/超时或执行中断等共性故障停止。复用已知两局demo，不是新held-out，不拿旧96/95分拼接；不改默认、前端、GLM-5.2或旧回执，8E in_progress、production_media=0。RQ-248“只做一次经济观察”的下一步被本条取代；本地实现完成：新增18项，聚焦与相邻共180项通过，编译、零网络预检、治理及diff检查通过；当前待本实现公共CI，随后直接执行已授权真实批次，真实请求0。八维学习复用RQ-246计划的RQ-249节。

RQ-250（2026-09-06）：RQ-250工具批量预算修复完成本地、公共和完整真实开发验证。独立组合1.2.0/Skill0.4.0/程序2.2.0将本地工具总量从3调整为8，原报告/质量/9次模型调用预算不变；旧合同摘要及RQ-249失败回执保留。实现772179eba46ca02c99bc7d920e3ab048e39d0468 / Actions34033770356三任务同SHA全绿，新增13项/相邻196项、公共后端2701项通过。新完整四案经济96、整体97、生存96、记忆97，均3来源，事实/引用/注入检查通过，记忆预算偏好确认通过；17次真实调用，输入72595/输出14676 tokens。连同前一批定位问题的4次，本轮合计21调用/101133 tokens。当前开发验证已收口，下一步准备产品采用与真实数据黄金切片的衔接，不重复此开发批、不自动启默认或另立正式考卷。8E仍in_progress、production_media=0；这是已知两局合成demo的开发证据，不是独立领域准入或公共生产成熟度；GLM-5.2、前端与主仓库用户修改均不动。
...
### 1cd694d完整跑通、人工复核与后续对象边界（2026-09-10）

1cd694d / Actions34474893782三job成功（3022 passed/152 skipped/127 subtests、PostgreSQL208）。新鲜协议3请求1007/90通过；1.3.6黄金运行5模型/2检索、140297ms完整，初稿1983字符在56.656秒收口，82分发现引用互换/中位数误差，经一次修订复评95分并自动published。真实生成截止问题本次已跨过，OP.GG事实→条件建议已出现；不能由一次成功宣称延迟可靠性完全解决。主Agent复核仍发现视野分推断意识、混合位置发育“塌陷”两项，Luna针对复核确认；原始报告人工未通过。已另出2249字符人工校订观摩副本，未重评、不冒充原运行产物。新增8次，累计68次Provider尝试，五次历史超时用量未知。本人身份/训练目标未答复，Training持久化与live Workbench未验收，8E保持in_progress。下一步依据用户明确本人或观摩对象完成目标绑定，保留两项自动质量漏检为后续修正，不将观摩对象写为本人。

实现1cd694d4af9679e0646daa749a90a7cc57c1b6d7；运行golden_20260910_compact_1cd694d；Bundle09aa4e73ae70169497a330c371b37eb83afd57d451c8238d3575eb4fed5d9f50，Trace5dd2e2899175939fd3a6a131c8882f0c055e1ee55b7ff5de44f2c51310cb0808。五请求父墙钟分别20.453/56.656/28.875/14.265/19.797秒，全部终态/Usage/EOF/close完整；对应工具、初稿、评估、修订、复评。Coach输入49993/输出8170；连协议输入51000/输出8260，本批全部可观测，不含旧未知用量。实际来源仍Riot0/static1/official_patch1/OP.GG2工具+2初始化+2目录。此次用户修复接续批合计14次Provider尝试（前计时诊断6+本版8），不是68次新增。

自动报告SHA aade1720b7b2633cfb7842c175609d102b3504a71207a7ed8f70e86b3ff5fcae，82→95明确修正K2/K3对应错位和中位数8.805→约8.8。OP.GG中路辛德拉T2/rank12（previous18）、薇古丝T2/rank14/win52%，辅助卡蜜尔T4/rank35/win46%与输入快照一致并形成条件建议；来源时间与版本/段位/区域局限均存在。该项比原仅附录来源有进展，但质量门95不等于人工全部通过。
