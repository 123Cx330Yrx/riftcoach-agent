# 2026-09-25 完成证据与真实来源责任分离

- 后台原result直接证据是task_observation_host_deadline；用户报告的更早触发是Codex额度耗尽。不能倒置因果或称GLM输出额度耗尽。逐例durable完成机制在本批真实起效。
- 原observed:1“较稳定”正确稿96/pass。observed:2的无据未来外推被正确检出；6/15/19只有汇总/均值，模型扩写单局伤害/all_pairs/经济排序，引用覆盖不足。完整来源有正确值，语义检出与引用责任分别判断。
- 29不是全原始事实根，省略版本/部分指标；29+38够本例不能代表全资料。离线变体追加完整facts_and_provenance来源，所有原数据不变，不让host自动补引用或把宽根当事实证明。
- 独立反例发现未知尾部的声明预算可能低于实际已发请求，现改按已发request上界核账；已完成known批不受影响。ready门防无人接审的新例，不解决已开始例中途Codex离线；有限时等待仍受原batch时钟约束。
...
# 2026-09-24 任务结果观测需要两个层次的绑定

- 原始transport字节摘要与排序后公开JSON的序列化摘要不同；必须从未改builder恢复对象顺序，逐值相等且原byte SHA相等，不能重新hash公开投影冒充原请求。
- host阶段摘要应canonical化以跨write_new_json落盘保持，但原始请求byte hash不得canonical化替换。语义主张、回执来源和序列化表示是三种不同约束。
- 审查与最终任务质量分开后仍需防止“编辑碰巧修好”隐去首评漏检；具体缺陷不合并，旧准入门不吃新task_outcome。继续观察只限附属解释，错误修法/混真假issue不能通融。
- 每次before_send不足以绑定最后host之后的状态；真实执行器成功前也须检查HEAD、冻结identity和case/batch时限，包含必要回放/记账，不在收尾重置时间。

# 2026-09-24 一次正确定位但解释有错的首评，被既有尾部成功限制传播

- 原错误review未经删改仍传入Flash；实际稿修正真全称、没有错误27–43，最终GLM95/pass也无新事实错误。只能称该附属错误未传播，不能据无该句反推编辑显式发现/撤销了它。
- 真正产品Harness保存每轮evaluation，但只有最终attempt评分/实际终稿用于交付；中间错误与最终质量可分别测量。当前原15逐stage gate仍保持，tail不能冒充原资格通过。
- 用户采用的完整上下文标准保留真实事实/无依据外推/评估矛盾阻断；没有证据要求已处理的内部缺陷永久否决整链。后继实验可前瞻定义任务级观测，但不能悄改历史host或取消原15/最终来源门。
- 选择先测完整既有链路，暂不建数值协议。新对照要针对错误传播、漏检pass、误改正确稿、错误最终评估等机制，不只寻找更短提示或一次高分；需考虑实际2生成与尾部共享完整预算。

# 2026-09-24 审查自身新增数字错误与报告目标判断分开

- 两例目标判断均正确，第二例的错误是审查explanation自己声称中单视野27–43，实际25–43。来源5/洛克/视野25在完整输入，公开tool arguments已经错，解析无损，不能归因于输入遗漏/协议/断流/预算。
- sources1+3可重组中单早死1.5、均值方向；12/16可重组中单视野33.75。引用重组合法，但均值不决定逐行范围；并且新增下界27本身错，即使补源ID也不能修正。
- 只收缩advisory没有消除真实issues中的事实生成责任。已有computed有mean/median/胜负差/完整成员而没有min/max；缺预计算不等于缺原始数据，增加min/max也不机械保证模型使用。
- 主审最初注意到范围引用不充分，独立复核进一步指出实际25反例；最终裁决以共同复核的真实事实错误为准，不仅做机械source_ids检查。这次独立复核提供了实质增量。
- “错误review是否可被后续编辑/终评纠正”与“review自身资格通过”是两个问题；当前失败批不重开，原数字不删除，不能把只验证终稿容错冒充原15审查通过。下一方案应审查整个Agent消费，而非默认只能继续改reviewer。

# 2026-09-24 上下文因素未形成区别；建议扩写责任单独缩减

- 2×2四条件均无目标误报，不能选整段替换/删引言为修法，也不能把新工具交付与历史波动分离归因。按冻结分支停4，没有重复到pass。
- 完整审查2可接受/2不可接受；后两种缺口是新增建议的引用/推断责任，不是原正确报告错。来源引用需容许明确重组：04的全败局与完整辅助行可支持中单CS/gold方向量级，02没有该辅助CS行则不足。
- Review输出的消费者与责任不一致：非阻断自由解释没有API/成稿/编辑消费者，却产生第二份事实文本。选仅block的可选澄清标记，保留出口以面对ADR0104反证；不是删旧输出或自动判语义正确。
- 1.5.1实际存在Note与ToolDelivery两个请求身份；版本号不足以选择回放。关闭的context诊断与delivery审计须取冻结candidate，不能动态绑定1.5.2。历史Trace信任与当前执行许可分别维护。
- 接下来直接用原15正确/真错/混合例检验收缩是否导致错放行或错阻断；不以字段减少、标记合法或一次正确稿通过证明稳定。

# 2026-09-24 明确交付冲突与历史身份漂移

- RoleNote user重复schema头与system工具要求冲突；后继去头builder可修确定性问题，不能归因为误报。旧去重实测反例仍成立。
- 新candidate必须绑定delivery/manifest/request摘要；相同业务版本不等同相同请求，当前资格拒绝旧回执。
- prepare_comparison旧请求＋当前candidate曾混用；旧批预览必须绑定冻结计划，不再排当前队列。
- 公开投影排序dict，而旧compact hash保留插入顺序；读回计划不能冒称重建原字节。原导出整体SHA及原请求重建支持历史回放，预览明确projection。
- 五调用自由文本首审路线可表达，但干净不改稿分支仍交同一失败结构审查，没有核心误报作用机制。不能因首审通过就采用或把失败后移。

# 2026-09-24 普通真实纠错成功与正确全文误报并存

- 当前1.5.1组合真实首评检出复合归因错误、Flash改稿、GLM终评96/pass，逐项人工/独立
  审查接受，足以证明该冻结错误稿的3调用纠错链；不证明自然生成、独立补漏或总体稳定。
- 同system/schema/metadata/来源下，正确全文的泛指句仍被升级为全称事实错误，缺severity
  另记协议失败。第一次偏离已在模型原始输出，恢复未发；补字段、加额度不针对这次误报。
- 对成功终评与失败首评作结构比较：删除报告blocks和三个派生hash后，全部业务data一致。
  23段逐字对应，但引言删除引起段号移动，解释具体化与随机性也未隔离，不能归因于某一项。
  同请求再测仅能在翻转时证明该请求下判断不一致；不翻转也不能推出稳定，不能重试到pass。
- 引用按实际明示句柄核验，不机械要求每条重复数字ID：computed根中的成员能沿明确编号
  追到单局，K2显式句柄可作其实际支持的方法类比；“computed_evidence MIDDLE
  deaths_before_15”也能精确到2.50/0.50。编号1/21本身不足不能扩大21，但完整明示引用
  与只选CS/gold且没有对应伤害依据不同。保留旧失败，不补写模型选择。
- notes精简与角色分工已运行，不是通用误报充分修法。下一方案必须用有区分力的能力
  证据选择，且考虑2动态生成＋首评＋编辑＋终评的五调用整任务；不为局部能力再叠第六次。
- 本机拒绝hash多抄一个字符安全停批，是执行失误；原result不改，另记审计。以后由文件
  计算hash。read_role_calls相对根混用是真实消费缺陷，已修并验证，不与语义失败混记。

# 2026-09-24 实际成稿质量与完整审查输出质量必须分别裁决

资料修复后的真实报告可接受，生成/审查/落盘均保留来源各自时间，未知值与过期指标边界未丢。但GLM的可选建议只选CS/gold根、文字扩写damage；完整输入有数值，原始输出已缺引用，不能归因于编号映射、缺字段、超时或预算。只补显示或自动补引用会篡改实际选择。

该建议未应用于成稿。应保留“成稿语义可接受、完整评审引用未通过”，不能为了维持旧失败叙述把建议缺口倒算成成稿错误，也不能因pass/published就略过建议自身的依据。第一条K1扩写的充分性疑点与第二条可直接复现的引用缺口区分，未发明原稿措辞门槛。

所有原15仍可恢复：每例12个role×胜负×指标根可归为3组，但已有aggregate、5个match和完整computed根。因此增加粗根不增加表达能力；替换引用粒度可能降低选择负担，但改变的是新请求合同，不能事后扩大旧ID含义，也不能由离线拼组推导语义可靠。下一机制决策要看实际消费/失败覆盖，避免又回到一句提示一个实测。

# 2026-09-24 OP.GG来源可用性与来源元数据被混为一层

原bundle保留OP.GG两个digest及9/10检索时间；进入模型的source_context对不可用资料只输出digest/reason。已确认是资料投影丢字段，不是源头完全没有时间。过期或不匹配的指标不可用于建议，不意味着其已知来源时间必须丢弃；现分别保留元数据与排除指标，原15和旧输入不变。

显式编辑本次正常完整且只修知识日期，说明不能把编辑输出短当作来源核验充分修法。它未提交第二条修改，host没有漏应用；公开reason还把OP.GG日期当已知。旧输入不足的负例保留，上游资料不能事后替旧模型补证；不能证明模型内部因果或由一次局部改对推出可靠性。

# 2026-09-24 来源编辑职责与工程反例

五调用合同下，2动态生成＋完整首评＋来源专审＋编辑＋终评需6次；替代完整首评会失去提前发现业务真错后的编辑机会。ADR0011按需选工具和实际AgentRun来源收集不能被预填prompt等价替代。选择来源核验并入一次显式编辑能力原型，尚未采用产品路由，不将短输出当作超时修复证明。

独立审查找到可复现工程缺口：子串标题存在不代表章节存在或顺序正确；报告字数合法不代表完整终评请求可装入。现检查真实标题及顺序、从实际成稿重建完整请求并验证预算；输入视图一致性与人工全文裁决也已补齐。原稿keep仍能保留已知错误并被旧审查判pass，故操作合法绝不自动推导语义通过。模型内部漏检机制仍未知。

# 2026-09-23 同请求换模型不足以补漏，完整职责必须装进任务预算

- Flash在当前错误日期稿上完整95/pass，漏两处；此前另一归因稿的通过不能迁移。本例不足以证明模型整体优劣或内部成因，足以拒绝直接替换作为修法。建议正确与所选引用完整仍须分开核对。
- 2生成+来源专审+完整首评+编辑+终评=6；不能把独立单次专审可行写成整任务可行。替换首评会把首次业务错误发现推到终评，需要明确其修订机会代价。
- source-first旧实现是保覆盖义务的复杂全稿重评，typed source_checks在复杂全稿内且只有离线证据。旧失败中还含来源无可引用地址的工程缺口，不能当作独立来源专审已失败。
- 两次生成不是固定合同次数，但现有准备器只从AgentRun实际工具执行汇集KnowledgeEvidence，成功发布又要求知识来源；当前实际链路通常至少模型选工具一次、成稿一次。用预填prompt省调用不足以保证同源证据，强禁动态检索还会背离ADR0011的AgentLoop采用理由。

# 2026-09-23 人工参考自身的遗漏会污染“误报”判断

- 单删知识日期的原参考仍给两条不可用OP.GG记录赋予9月10日检索日期，原输入没有其依据。不能把另一个来源的检索时间移给OP.GG，也不能把“实际日期未知”说成“已证明日期不同”。主Agent和先前独立审查都沿用了不完整标签；已在新批付费前纠正。
- 两处最小删除后独立复核全部七节、数值/时间线/引用/身份/条件练习，未发现第三个阻断；不能把未获源内证实的背景定性一律升级为新OP.GG门，也不为“差距不大”自设阈值。完整裁决留在reference audit。
- 原GLM v1/v2漏检、Flash全稿编辑截止事实仍成立。旧稿/回执不修改，未执行Flash计划以0请求撤回；新参考是人工控制，绝非真实模型改稿/资格证据。

# 2026-09-23 全稿编辑未完成；先补同输入审查能力比较

- 独立编辑1次/300秒截止，持续事件但没有正文/终态/usage。语义结果和费用未知；不是可证明的断流或输出额度耗尽。不采用该pass后分支，不延时续跑。
- 旧独立编辑74.735秒成功不支持把本次超时归咎于任务类型本身。报告、来源、政策不同，输入字符增加也不能独立解释原因。
- 此前遗漏了更小的能力比较：Flash曾通过的是旧归因稿，不是日期稿；对当前日期稿采用完全相同审查请求才能比较两个模型在此例的行为。纯内存重建确认请求字节可保持一致，GLM第二份参考并未实测。
- 同题Flash审查若成立，可复用现有真实needs_revision→编辑→终评，不先增加pass后新状态机；若不成立，停止该替换路线。两例不能证明优劣、稳定性或错误独立，原15与实际产品消费仍需后续证据。
- 序列化JSON键排序可改变字节摘要但不改变数据。验证原工件需要原请求序列化器重建+原hash+内容三者核对，不对公开排序副本重新序列化后冒称原始请求hash。
