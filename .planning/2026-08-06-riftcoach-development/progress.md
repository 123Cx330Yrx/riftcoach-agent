...
2026-09-16 公共验证补记：33c03ce/Actions35072891451及fcf0ac3/Actions35073525804三项均success。后者对应上游模板修复及历史验收快照保持；原真实语义失败不变，无新增Provider调用。

2026-09-16 上游来源审查扩展：确认实际请求包含旧确定性模板的混分路能力/稳定性判断；已修正新报告生成，128项相关检查通过。历史RQ-246输入改用原字节快照，旧manifest不变；本轮失败请求原哈希仍匹配。模型语义未解决，未继续付费，下一步联合审查事实、模板和两次评估的数据流。


2026-09-16 独立provisional候选真实验证失败并停止本批：c3ff392/Actions35071182777公共三项success。正例两次完整stop（75.468秒/131.156秒），25511输入+23007输出=48518tokens，无未知用量，累计Provider至少200。新入口确实完成首评→完整重评，但最终table block13缺操作数引用、target block10缺跨段scope_source；逐项审查确认目标仍把四场中单换成五局混合样本，训练中位数/整体均值还被附上胜负比较绑定。仅离线补两个结构缺项即可pass，但错误解释未改，明确证明格式修复不解决语义；原响应及失败回执哈希不变。已将新入口也设offline_only，35项受影响检查通过，不运行负例/修订/复评，不自动局部补丁后付费重跑。当前下一行动：离线裁决原文优先、独立语义重评的数据流，比较不向第二评灌输首评claim判断但保留全部原文义务/旧issue/审计的方案，联合检查范围继承、运算类型与全链预算；不再把结构可行等同方法有效。8E/high/硬上限与前端及62主题后续保持。

以下为历史记录，当前以最新canonical为准。

2026-09-16 临时首评整体合同裁决完成：前轮f034b01/Actions35067580216三项success。核对首评错误族、原文字符覆盖、旧issue处置、比较成员及五调用路径；补查漏句/跨段限定/新外推问题、容量前拒绝和新入口接线检查通过。新增独立provisional-review开发入口，每次一例、沿用原始预算及high；旧contextual/comparison入口保持offline_only。158项相关检查及新增入口聚焦9项通过（共159项不同测试）。离线57206及历史58800–62944请求尺寸复用，提示与预算未改。允许同实现公共CI后一次冻结正反例开发验证，首例逐项语义合格后才运行次例；任何失败停批，不自动局部修补后付费重跑。当前尚无新增真实请求，累计仍至少198；8E及前端/62主题后续保持。

以下为历史记录，当前以最新canonical为准。

2026-09-16 比较候选真实首评失败并完成离线入口修复：387b8ef/Actions35065251865三项success；正例第一调用约59.2秒完整stop，11162输入+5619输出=16781tokens，无未知用量，累计Provider至少198。首评14个来源超过最终schema的12个，另有block14引用片段写错、block10目标遗漏，尚未执行第二步/负例/修订/复评。根因是把首评临时判断当作最终合格结果准入，非断流或额度耗尽。已停本批并将比较入口设offline_only（旧contextual也保持停用）。新独立离线临时首评合同保留全部原值和诊断，有效来源照常保护；错误片段但真实block则要求全段覆盖，不猜未知block、不吞坏JSON、不删除旧issue；最终schema/语义标准不放宽。原响应不改即可构造57206的第二步请求，十份历史投影58800–62944；153项相关检查通过，含离线五调用和入口阻断。真实语义仍未验证，8E/high/硬上限/前端及62主题后续保持。

以下为历史记录，当前以最新canonical为准。

2026-09-16 比较关系合同已接入独立完整重评工作流：程序生成完整样本/位置胜负组、指标均值及逐行关系；模型在claim内绑定cohort/metric/operand_refs并保留唯一说明。计算成员与整句其他事实引用分离，避免误拒混合陈述。原文覆盖、旧issue处置及最终校验仍在；五调用初评→重评→修订→复评串联测试通过，绑定依据进入修订。134项相关检查通过。两例第二步58502/58576，十份历史58608–62718，修订形状56892，均在既定上限内；首评完整表格视图逐项还原核验，无删证据。实际旧c010选错cohort仍可结构pass，保留为语义反例；本轮尚未真实验证。旧失败入口继续offline_only，新入口每次只执行一例，待同SHA公共CI后按既有授权进行成对开发验证。累计Provider仍至少197；high/所有预算/完整上下文标准/8E及全局后续保持。

以下为历史记录，当前以最新canonical为准。

2026-09-16：完整重评离线原型完成，99项相关检查通过。保留原文覆盖及旧问题处置，v10结构可行但c010混组语义仍不合格；两例与十份历史完整请求形状在限内，大形状超限前拒绝。没有新增真实请求或产品接线；继续语义合同裁决。

以下为历史记录，当前以最新canonical为准。

2026-09-16：执行约束已实现，134项相关检查通过；真实首评离线回放保留原字节并在第二调用前停止。本次真实Provider请求为0，累计仍至少197、既定恢复序列22请求544606tokens。整体纠正方案和混组语义问题未解决，继续离线合同裁决，未恢复实测。

以下为历史记录，当前以最新canonical为准。

2026-09-16 v10结果：6bcaa86公共三项通过；真实2次完整stop合计42956tokens后duplicate_context_claim失败，累计至少197，本批22请求544606tokens，无新增未知用量；第二例/修订/复评未开始。已完成只读纠正可达性审计，确认不可能的重复修复与独立混组语义错误，停止付费连跑并转离线合同裁决。详见canonical及ADR-0102最新结果节。

以下为历史记录，当前以最新canonical为准。

2026-09-16：013dbe1公共三项success；v9仅一次完整返回26722tokens后因额外正文失败，累计至少195，本批20请求501650tokens。当前v10统一首评/纠正协议并通过十份离线尺寸测量；本次调整尚无新增Provider请求，不重标旧失败。待回归、同SHA公共验证与一次新有界成对验证。详见canonical及ADR-0102 V10。

以下为历史记录，当前以最新canonical为准。

2026-09-16：68e8670公共三项通过；两例7次完整stop共158888tokens，累计至少194。v9第二例原字节离线95/pass、150项相关检查和实际复评预算测量通过，尚无成对质量验收。详见canonical及ADR-0102最新节。

以下为历史记录，当前以最新canonical为准。

2026-09-16：c7e7322公共三项通过；真实2次stop共53637tokens，累计至少187。v8有149项相关测试和七轮预算测量，待公共检查和完整实测。详见canonical及ADR-0102最新节。

以下为历史记录，当前以最新canonical为准。

2026-09-16：92aed17公共三项通过，真实2次stop/52440tokens后停止，累计至少185。v7有147项相关检查及六轮形状预算证据，待同SHA公共检查和完整新实测。详见canonical及ADR-0102最新节。

以下为历史记录，当前以最新canonical为准。

2026-09-16：c453088公共三项及658黄金回归通过；真实49521tokens/2次stop后协议拒绝，累计至少183。v6的146项相关检查与五轮预算测量通过，待同SHA公共检查及新实测。详见canonical及ADR-0102最新节。

以下为历史记录，当前以最新canonical为准。

2026-09-16：769d651公共三项通过；真实53707tokens/2请求后拒绝，累计至少181。v5的138项检查及四轮预算测量通过，待同SHA公共检查与新身份实测。详见canonical及ADR-0102最新节。

以下为历史记录，当前以最新canonical为准。

2026-09-16 诊断完整性修复：5645afc公共三项通过；真实两次stop共47783 tokens仍未完成评估。累计Provider至少179。本诊断补丁124项检查及三轮真实形状预算测量通过，待公共检查后完整实测。 详见canonical与ADR-0102最新节。

以下为历史记录，当前以最新canonical为准。

2026-09-16：v3公共三项通过且纠正请求已实际发出；v4原始响应离线回放及111项相关回归通过，待同SHA公共检查和新身份完整复测。语义与准入仍未完成。 详见canonical与ADR-0102。

以下为历史记录，当前以最新canonical为准。

2026-09-15 大复盘后恢复：v3本地修复与642项黄金切片相关回归、编译、预检和治理检查通过；离线测量覆盖最新真实失败形状、三份旧形状、首评和修订。真实语义未验收、当前补丁未新增Provider调用，累计至少175。下一步按canonical完成公共检查和有界实测。
...
最终复验：164 passed（9.83s）；编译、git diff --check、项目治理检查通过。CLI双目标预检为network_allowed=false/provider_enabled=false，OP.GG最多5个分路查询及对应会话/目录预算已声明。此次仅本地修复，没有新的真实观察或公共CI。

完成候选入口按实际位置定向OP.GG查询、榜单外目标选择、请求/响应/未匹配区分、分位置指标样本数、显式多位置训练目标及辅助不套补刀目标；候选事实JSON无损压缩并恢复完整检索。聚焦与相邻164项通过（最终复验另记），保存v14输入离线测量节省1710单位、无字段省略。未新增外部调用、未写数据库或改前端；真实Coach/官方版本/静态对齐/Training持久化/live UI仍待完成，长期画像及目标交互另有明确待办。

## RQ-258（2026-09-10）持续推进黄金切片

恢复确认隔离分支干净后新增入口草稿，24项聚焦测试通过，但不足以覆盖执行器。已进行多次真实开发读取，部分运行失败未有完整回执，不能声称精确累计外部调用数。v10 有四来源投影、Provider 3次、Harness拒绝，无合格报告或live UI。原回执不删除，禁止作为黄金切片成功证据。当前修复持久计划与真实执行边界，再进行离线链路和产品消费工作；未提交，未改生产默认。
...
- 领域观察已执行，首案 2 次调用、6624 Token；工具成功但来源为零，topic=unmapped、未补查，
  evidence_required 拒绝，后两案跳过，未进入评分。最新代码 dde1f84 的公共 CI 33966158145 三任务成功。
- 撤回“实际查询为早期死亡相关资料”与“仅为终端编码问题”的断言：文件字符核验正常，实际查询正文
  没有留存；该短语仅是离线开发样例，不能作为真实词句根因。相关状态和路线文本已纠正。
- 本次纠正文档不新增模型请求，不改旧考卷或回执；后续应先用独立开发样例验证检索改善，再作新验收。

## 开发链路验证完成

- 新增 tests/test_coaching_retrieval_development_chain.py：14 正例、3 零来源、1 低评分和3不扩展边界，
  共21项通过；相邻旧版本、执行器、查询恢复和公共回执总检合计112项通过（14.15秒）。
- 真实本地语料、执行器、Evidence文件摘要和七段报告持久化均参与；模型查询、正文和评分是脚本输入，
  不是 GLM 真实质量证据。网络连接在测试内被禁止；主仓库、运行时、旧考卷与回执未改。
- 当前批次本地完成；下一步核验开发集模型自主查询，不直接再开一套正式考卷。

## 自主查询开发观察01

- 新增开发探针，先通过24项本地测试（探针3项+开发链路21项）再执行一次；读取匿名demo两局数据，
  不读取旧held-out问题。生产代码HEAD dde1f84有公共CI，探针单独记录本地SHA，不声称公共已验证。
- 2次真实GLM调用、6861 Token、32126ms；两条查询59/65字符，主题分别为economy和survival+vision，
  均未匹配恢复规则；两次检索成功但零来源，评分前拒绝。没有自动重试或新考卷。
- 回执只含安全计数/枚举和身份，正文临时目录已随运行正常退出清除；实际查询原文仍未知。
- 下一步开发专用知识库指引的离线设计/测试，不再用手写查询成功代替GLM自主查询证据。
...
## RQ-243：生成/修订合同本地对齐（2026-09-06）

- 增加显式 `coach-markdown-generation-revision-v1`，同一八标题指引进入生成与修订；默认旧入口/Prompt Program 不变，新开发计划 2.0.0 与三场景身份独立，回执记录合同 ID/SHA。
- 新合同 13 项；两组测试 75/80，通过的唯一用例共 144 项、16 subtests。三代表场景完成离线一次修订控制流；缺标题/过短/未知引用拒绝、Context 漂移/旧 fixture/未启用指引在调用前拒绝；旧预算、资产、回执及 Prompt Program 回归通过。
- 本批真实调用 0，代码/计划/学习说明已完成，下一步本实现公共 CI；旧经济失败不回写，不变更前端/默认模型/GLM-5.2 或质量门。
- 公共收口：实现 `57c73e53a794bc2a4ccab72d878057f378515b5b` / CI `34023099294` 的 pytest、postgres-migrations、packaging-smoke 均 success。最终入口复核又确认新计划缺少显式合同会在调用前拒绝，25 项聚焦相邻回归通过。下一步新合同完整预算证明与产品迁移版本对齐，不重跑本批已通过测试或真实经济场景。

## RQ-244：新报告合同预算与迁移清单（2026-09-06）

- 新零网络检查器直接执行新具名合同的开发入口，每案经过预算墙完成九请求、一次报告修订、两次评测格式修复；模拟 Usage 按完整输入估算及每次 4096 输出结算，不用零用量绕过资源门。
- 三场景预留为 202346/202310/202250，205000 墙内余量 2654/2690/2750；独立预算 JSON 可重建，104 项聚焦/相邻测试通过。真实调用为 0，模拟最终 80 分按原质量门拒绝，不宣称经济真实失败已解决。
- 产品运行档案/Skill/程序/Trace/检索/回退/lease 迁移表和八维学习说明完成；本批未改 app、旧资产、GLM-5.2 或前端。实现 `1cd2deb` / CI `34025448118` 三任务同 SHA 全绿，下一步新产品执行组合离线接线。
...
## ADR-0099 B：共享事务接线复核（2026-09-08）

- 从证据仓库提取同一 Session 的快照写入 helper；任务正常完成与过期恢复改为复用锁内终态更新 helper，保留 lifecycle event 与外层事务边界。修正 reconcile 事件 reason 与旧行为一致。
