...
新证据golden_native_contract_options_offline_v2.json以committed fixtures复现，保留旧失败。v1分析者稿替换整段时不必要地删掉正确中单前半句，v2只修错误尾句、其余逐字保留；原模型报告和回执未动。五调用保守预约388070；历史五调用403790，满额合成usage在第5次之前被真实预算器拒绝，低usage则可完成5次。不能把全路径预约和升级为产品额外硬门。历史来源指纹同时绑定旧真实a04df23请求，所有统计编号、完整尾文保留。

最终72项相关测试通过，含实际产品compiler/RAG/Runtime测试实例：正常5调用满额合成用量365600可完成，多一次重评则第6次被调用上限阻断、不发布。产品默认/manifest未改，不冒称新候选注册或exact历史产品重放。ec00f36/Actions35584803783公共三项通过；后续提交公共检查以PR7对应SHA为准。三组完整编辑诊断已冻结：真实混合/真实全称/分析者提取纯误报，分别41104/40362/40154输入上界，预期只留host；独立审查身份、标签、预算、仅改尾句均通过。
